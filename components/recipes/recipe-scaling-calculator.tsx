...
export function RecipeScalingCalculator({ recipe }: Props) {
  ...
  const portionInfo = coursePosition && guests > 0
  ...
  const handleCopy = () => {
    ...
    const courseLabel = coursePosition ? COURSE_POSITION_LABELS[coursePosition as CoursePosition] : ''
    const header = [
      `${recipe.name}`,
    ...
    lines.push(hasOverrides
      ? (smartMode ? 'INGREDIENTS (smart-scaled, chef-adjusted):' : 'INGREDIENTS (linear, chef-adjusted):')
      : (smartMode ? 'INGREDIENTS (smart-scaled):' : 'INGREDIENTS (linear):')
    )
  ...
  return (
    <Card className={open ? 'border-brand-400 shadow-sm' : 'border-stone-200'}>
      <CardHeader>
      ...
      {open && (
        <CardContent className="space-y-5">
          ...
          <div className="flex flex-wrap gap-4 items-end">
            <div className="flex-1 min-w-[160px] max-w-[220px]">
              ...
              <select
                ...
                className="w-full border border-stone-300 rounded-md px-3 py-2 text-sm bg-white focus:border-brand-500 focus:outline-none"
              >
                <option value="">Select course…</option>
                {COURSE_POSITIONS.map(pos => (
                  <option key={pos} value={pos}>{COURSE_POSITION_LABELS[pos]}</option>
                ))}
              </select>
            </div>
          ...
          {portionInfo && (
            <div className="rounded-lg border border-brand-200 bg-brand-50 p-4 space-y-2">
              <p className="text-xs font-semibold text-brand-700 uppercase tracking-wide">
                Portion Standard — {coursePosition && COURSE_POSITION_LABELS[coursePosition as CoursePosition]}
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                <div>
                  <span className="text-stone-500">Per guest: </span>
                  <span className="font-semibold text-stone-900">{portionInfo.portionOz} oz</span>
                  <p className="text-xs text-stone-500 mt-0.5">{portionInfo.portionNote}</p>
                </div>
                <div>
                  <span className="text-stone-500">Total to prepare: </span>
                  <span className="font-semibold text-stone-900">{portionInfo.totalLabel}</span>
              ...
              {!scaleFactor && (
                <p className="text-xs text-amber-700 mt-1">
                  To scale ingredients, set a base yield on this recipe (e.g. &quot;8 servings&quot;).
                  The portion standard above is a reference for how much to prepare total.
                </p>
              )}
            </div>
          )}
          ...
          {effectiveScale && guests > 0 && (
            <div className="space-y-3">
              ...
              {smartMode && effectiveScale > 1.5 && (
                <div className="text-xs text-stone-500 bg-stone-50 border border-stone-200 rounded p-2">
                  <strong>Smart scaling:</strong> Spices, aromatics, and condiments scale below the linear amount
                  because their flavor compounds concentrate in larger batches. Proteins and liquids always scale
                  linearly. Hover any ingredient to see why.
                </div>
              )}
              ...
              <div className="bg-stone-50 rounded-lg border border-stone-200 divide-y divide-stone-100">
                {recipe.ingredients.map((ri) => {
                  ...
                  const diffPercent = isSubLinear && !isOverridden
                    ? Math.round((1 - smartQty / linearQty) * 100)
                    : 0

                  return (
                    <div
                      key={ri.id}
                      className="flex items-start gap-3 px-4 py-2.5 text-sm hover:bg-white transition-colors relative"
                    ...
                    >
                      ...
                      <div className="w-36 shrink-0 flex items-center gap-1">
                        <input
                          ...
                          className={`w-20 text-right tabular-nums text-sm font-semibold rounded border px-1.5 py-0.5 focus:outline-none focus:ring-1 ${
                            isOverridden
                              ? 'border-brand-400 bg-brand-50 text-brand-900 focus:ring-brand-400'
                              : 'border-transparent bg-transparent text-stone-900 hover:border-stone-300 focus:border-stone-400 focus:ring-stone-300'
                          }`}
                        />
                      ...
                      <div className="flex-1 min-w-0">
                        <span className="text-stone-800">{ri.ingredient.name}</span>
                        {ri.preparation_notes && (
                          <span className="text-stone-400 ml-1.5 text-xs">({ri.preparation_notes})</span>
                        )}
                        {ri.is_optional && (
                      ...
                      {smartMode && hoveredIngredient === ri.id && (
                        <div className="absolute right-0 top-full z-10 mt-1 w-72 bg-stone-900 text-white text-xs rounded-lg p-3 shadow-xl pointer-events-none">
                          <p className="font-semibold mb-1">{ri.ingredient.category} → ×{exponent.toFixed(2)} exponent</p>
                          <p>{smartReason}</p>
                          {isSubLinear && (
                            <p className="mt-1.5 text-stone-300">
                              Linear would give: {formatScaledQty(linearQty)} {ri.unit}.
                              Smart gives: {formatScaledQty(smartQty)} {ri.unit}.
                            </p>
                          )}
                        </div>
                      )}
          ...
          {!effectiveScale && guests > 0 && !portionInfo && (
            <div className="text-sm text-stone-500 bg-stone-50 rounded p-3">
