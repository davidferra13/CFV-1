...
import { getVersionHistory, type DocumentVersion, type SnapshotEntityType } from '@/lib/versioning/snapshot'
    ...
    <div className="border border-stone-200 rounded-lg overflow-hidden">
      ...
      {expanded && (
        <div className="divide-y divide-stone-100">
          ...
          {!isPending && versions.length === 0 && <div className="px-4 py-3 text-sm text-stone-500">No version history yet.</div>}
          {versions.map((v, i) => (
            <div key={v.id} className="flex items-center justify-between px-4 py-3">
              <div>
                ...
                <p className="text-xs text-stone-500 mt-1">{format(new Date(v.created_at), 'MMM d, yyyy h:mm a')}</p>
                {v.change_summary && <p className="text-xs text-stone-600 mt-0.5">{v.change_summary}</p>}
              </div>
              {onRestore && i > 0 && (
                <Button
                  size="sm"
