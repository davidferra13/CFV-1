...
} from '@/lib/proposals/template-actions'
...
export function VisualBuilder({ templates, menus }: Props) {
  ...
  function handleCreate() {
    startTransition(async () => {
      ...
      setShowCreate(false)
      setForm({ name: '', coverPhotoUrl: '', description: '', defaultMenuId: '', basePriceCents: 0, includedServices: '' })
    })
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-stone-900">Proposal Templates</h2>
      ...
      {showCreate && (
        <Card className="border-stone-300">
          ...
          <CardContent className="space-y-3">
            ...
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1.5">Default Menu</label>
              <select
          ...
          <Card key={t.id} className="overflow-hidden">
            ...
            <CardContent className="py-3">
              <div className="flex items-start justify-between">
                ...
                <span className="text-sm font-semibold text-brand-600">{formatCents(t.basePriceCents)}</span>
              </div>
              {Array.isArray(t.includedServices) && (t.includedServices as string[]).length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {(t.includedServices as string[]).map((s, i) => (
                    <span key={i} className="inline-flex items-center px-2 py-0.5 rounded-full text-xs bg-stone-100 text-stone-600">
                      {s}
                    </span>
                  ))}
                </div>
      ...
      {items.length === 0 && !showCreate && (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-stone-500">No templates yet. Create your first proposal template.</p>
        </div>
      )}
    </div>
  )
