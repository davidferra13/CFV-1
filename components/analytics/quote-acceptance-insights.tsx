  ...
  return (
    <Card className="p-5">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-sm font-semibold text-stone-900">Quote Performance (Last 90 Days)</h3>
        <Link href="/quotes" className="text-xs text-brand-600 hover:text-brand-700 font-medium">
          All Quotes →
        </Link>
      </div>
        ...
        <div className="mb-4 space-y-1">
          ...
          {data.expiringThisWeek.map(q => (
            <Link
              key={q.id}
              href={`/quotes/${q.id}`}
              className="flex items-center justify-between text-xs text-stone-700 hover:text-stone-900"
            >
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-stone-900">{data.acceptanceRate}%</div>
              <p className="text-xs text-stone-500 mt-0.5">acceptance rate</p>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-stone-900">{data.avgTimeToDaysDecision}d</div>
              <p className="text-xs text-stone-500 mt-0.5">avg time to decision</p>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-stone-900">{data.totalSent}</div>
          ...
          {(data.avgAcceptedValueCents > 0 || data.avgRejectedValueCents > 0) && (
            <div className="grid grid-cols-2 gap-2 mb-4">
              <div className="bg-emerald-50 rounded-lg px-3 py-2 text-center">
                ...
                <p className="text-sm font-bold text-emerald-700">{formatCurrency(data.avgAcceptedValueCents)}</p>
              </div>
              <div className="bg-stone-50 rounded-lg px-3 py-2 text-center">
                <p className="text-xs text-stone-500 font-medium">Avg Rejected</p>
                <p className="text-sm font-bold text-stone-600">{formatCurrency(data.avgRejectedValueCents)}</p>
              </div>
            </div>
          )}
            ...
            <div className="border-t border-stone-100 pt-3">
              ...
              <div className="space-y-1">
                {data.byPricingModel.map(m => (
                  <div key={m.model} className="flex justify-between items-center text-xs text-stone-600">
                    <span className="capitalize">{m.model.replace('_', ' ')}</span>
                    <span className="font-medium">{m.acceptanceRate}% ({m.decided} decided)</span>
                  </div>
                ))}
              </div>
            </div>
