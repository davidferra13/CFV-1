  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-stone-400">{data.previousYearLabel} → {data.currentYearLabel}</span>
      </div>
...
function YoYCard({ metric, isCurrency = false }: { metric: YoYMetric; isCurrency?: boolean }) {
  ...
  const trendColor =
    metric.changeDirection === 'up' ? 'text-emerald-600' :
    metric.changeDirection === 'down' ? 'text-red-500' :
    'text-stone-400'

  const TrendIcon =
    metric.changeDirection === 'up' ? TrendingUp :
    metric.changeDirection === 'down' ? TrendingDown :
    Minus

  return (
    <Card className="p-3">
      <p className="text-[10px] text-stone-500 font-medium uppercase tracking-wide truncate">{metric.label}</p>
      <p className="text-sm font-bold text-stone-900 mt-1 truncate">{fmt(metric.currentYear)}</p>
      ...
      <p className="text-[10px] text-stone-400 mt-0.5 truncate">{fmt(metric.previousYear)} prior yr</p>
    </Card>
  )
}
