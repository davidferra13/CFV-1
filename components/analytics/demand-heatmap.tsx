...
export function DemandHeatmap({ data: heatmapData }: DemandHeatmapProps) {
  ...
  function getIntensityClass(count: number): string {
    ...
    if (ratio >= 0.3) return 'bg-brand-300 text-stone-900'
    if (ratio >= 0.15) return 'bg-brand-200 text-stone-900'
    if (ratio > 0) return 'bg-brand-100 text-stone-700'
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card>
          <CardContent className="flex items-center gap-3 py-4">
            ...
            <div>
              <p className="text-2xl font-bold text-stone-900">{totalPredicted}</p>
        ...
        <Card>
          <CardContent className="flex items-center gap-3 py-4">
            ...
            <div>
              <p className="text-2xl font-bold text-stone-900">{totalActual}</p>
        ...
        <Card>
          <CardContent className="flex items-center gap-3 py-4">
            ...
            <div>
              <p className="text-2xl font-bold text-stone-900">{avgConfidence}%</p>
            ...
            <div className="space-y-6">
              ...
              {years.map(year => (
                <div key={year} className="space-y-1">
                  <p className="text-xs font-semibold text-stone-700 mb-1">{year}</p>

                  {/* Predicted */}
                  <div className="grid gap-1" style={{ gridTemplateColumns: 'auto repeat(12, 1fr)' }}>
                    <div className="text-xs text-stone-400 text-right pr-2 self-center w-10">Pred</div>
                  ...
                  {/* Actual */}
                  <div className="grid gap-1" style={{ gridTemplateColumns: 'auto repeat(12, 1fr)' }}>
                    <div className="text-xs text-stone-400 text-right pr-2 self-center w-10">Actual</div>
                    {MONTH_LABELS.map((_, monthIdx) => {
                      const point = getDataPoint(year, monthIdx + 1)
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-stone-100">
                ...
                <div className="flex gap-0.5">
                  <div className="w-6 h-4 rounded-sm bg-brand-100" />
                  <div className="w-6 h-4 rounded-sm bg-brand-200" />
                  <div className="w-6 h-4 rounded-sm bg-brand-300" />
