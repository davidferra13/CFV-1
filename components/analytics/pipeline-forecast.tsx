        ...
        <CardContent className="py-4">
          <div className="flex items-center gap-3">
            ...
            <div className="text-right">
              ...
              <p className="text-xs text-stone-400">{totalCount} event{totalCount !== 1 ? 's' : ''}</p>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={Math.max(200, pipeline.length * 60)}>
              ...
              >
                ...
                />
                <Tooltip
                  formatter={((value: number | undefined, name: string | undefined) => [`${(value ?? 0).toLocaleString()}`, name ?? '']) as any}
                />
                <Legend />
                <Bar
                  dataKey="Raw Revenue"
        ...
        <CardContent>
          {pipeline.length === 0 ? (
            ...
            <div className="space-y-2">
              {pipeline.map(stage => {
                const weightPct = stage.totalCents > 0
