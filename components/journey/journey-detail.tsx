...
export function JourneyDetail({
  ...
  const reflectionCount = useMemo(
    () => entries.filter(entry => entry.mistakes_made.length > 0 || entry.proud_moments.length > 0).length,
    [entries],
  )
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
            ...
            <Badge variant={journeyState.status === 'completed' ? 'success' : journeyState.status === 'in_progress' ? 'info' : 'default'}>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <div className="rounded-lg border border-stone-200 bg-stone-50 p-3">
              ...
              <p className="text-xl font-semibold text-stone-900 mt-1">{entries.length}</p>
            </div>
            <div className="rounded-lg border border-stone-200 bg-stone-50 p-3">
              <p className="text-xs uppercase tracking-wide text-stone-500">Highlights</p>
              <p className="text-xl font-semibold text-stone-900 mt-1">{highlightCount}</p>
            </div>
            <div className="rounded-lg border border-stone-200 bg-stone-50 p-3">
              <p className="text-xs uppercase tracking-wide text-stone-500">Ideas</p>
              <p className="text-xl font-semibold text-stone-900 mt-1">{ideas.length}</p>
            </div>
            <div className="rounded-lg border border-stone-200 bg-stone-50 p-3">
              <p className="text-xs uppercase tracking-wide text-stone-500">Adopted</p>
              <p className="text-xl font-semibold text-stone-900 mt-1">{adoptedCount}</p>
            </div>
            <div className="rounded-lg border border-stone-200 bg-stone-50 p-3">
              <p className="text-xs uppercase tracking-wide text-stone-500">Media</p>
              <p className="text-xl font-semibold text-stone-900 mt-1">{media.length}</p>
            </div>
            <div className="rounded-lg border border-stone-200 bg-stone-50 p-3">
              <p className="text-xs uppercase tracking-wide text-stone-500">Recipe Logs</p>
              <p className="text-xl font-semibold text-stone-900 mt-1">{recipeLinks.length}</p>
            </div>
          </div>
