    ...
    startTransition(async () => {
      try {
        ...
        if (editingId) {
          ...
          setMedia(prev => {
            const next = sortMedia(prev.map(item => item.id === result.media.id ? result.media : item))
            onMediaChange?.(next)
            return next
          })
        } else {
  ...
  const handleDelete = (item: ChefJourneyMedia) => {
    ...
    startTransition(async () => {
      try {
        ...
        setError(deleteError instanceof Error ? deleteError.message : 'Failed to delete journal media')
      }
    })
  }
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="border border-stone-200 rounded-lg p-4 space-y-4 bg-stone-50/50">
            ...
            <div>
              ...
              >
                {(Object.keys(MEDIA_LABELS) as ChefJournalMediaType[]).map(type => (
                  <option key={type} value={type}>{MEDIA_LABELS[type]}</option>
                ))}
              </select>
            </div>
            <Input
          ...
          {form.media_type === 'photo' && (
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1.5">Upload Photo (optional)</label>
              <input
                type="file"
              ...
              />
              <p className="mt-1 text-xs text-stone-500">Upload JPEG, PNG, HEIC, or WebP (max 15MB).</p>
              {photoPreviewUrl && (
          ...
          <Input
            ...
            placeholder="https://..."
            helperText={form.media_type === 'photo' ? 'Paste a URL or upload a photo above.' : undefined}
          />
      ...
      <div className="rounded-lg border border-stone-200 p-3 space-y-3">
        <div className="flex items-center justify-between">
          <p className="text-sm font-medium text-stone-900">Scrapbook</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {photoItems.map(item => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="block rounded-lg overflow-hidden border border-stone-200 bg-stone-50 hover:opacity-90"
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={item.media_url}
          ...
          media.map(item => (
            <div key={item.id} className="border border-stone-200 rounded-lg p-4 space-y-3">
              <div className="flex items-start justify-between gap-3">
                <div className="space-y-1">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-xs text-stone-500">{formatDisplayDate(item.taken_on)}</span>
                  </div>
                  {item.entry_id && (
              ...
              {item.media_type === 'video' && (
                <video controls className="w-full rounded-lg border border-stone-200 bg-black/80 max-h-80">
              ...
              {item.media_type === 'document' && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center gap-2 rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-sm text-stone-700 hover:bg-stone-100"
                >
              ...
              {item.caption && <p className="text-sm text-stone-700 whitespace-pre-wrap">{item.caption}</p>}

              {(item.location_label || (item.latitude !== null && item.longitude !== null)) && (
                <div className="space-y-2">
                  {item.location_label && (
                    <div className="inline-flex items-center gap-1.5 rounded-md border border-stone-200 bg-stone-50 px-2 py-1 text-xs text-stone-600">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex text-xs text-brand-600 hover:text-brand-700"
              >
