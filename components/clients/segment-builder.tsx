...
export function SegmentBuilder() {
  ...
  const [filters, setFilters] = useState<Filter[]>([
    { field: 'days_since_last_event', op: 'gt', value: '90' },
  ])
  const [color, setColor] = useState('#6366f1')
  const [isPending, startTransition] = useTransition()

  function addFilter() {
    setFilters((prev) => [...prev, { field: 'status', op: 'eq', value: 'active' }])
  }
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      try {
        ...
        setName('')
        setDescription('')
        setFilters([{ field: 'days_since_last_event', op: 'gt', value: '90' }])
      } catch (err: any) {
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-stone-300 mb-1">Segment Name *</label>
            <input
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="e.g., High-Value Dormant"
          ...
          <div>
            <label className="block text-sm font-medium text-stone-300 mb-1">Color</label>
            <input
              type="color"
              value={color}
              onChange={(e) => setColor(e.target.value)}
              className="h-10 w-20 rounded border border-stone-600"
            />
          </div>
        </div>
        <div>
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            <label className="text-sm font-medium text-stone-300">Filters</label>
          ...
          {filters.map((f, i) => (
            <div key={i} className="flex gap-2 mb-2 items-center">
              <select
                ...
                className="border border-stone-600 rounded-lg px-3 py-2 text-sm flex-1"
              >
                {FIELD_OPTIONS.map((o) => (
              ...
              <select
                ...
                className="border border-stone-600 rounded-lg px-3 py-2 text-sm w-36"
              >
                {OP_OPTIONS.map((o) => (
                  <option key={o.value} value={o.value}>
                    {o.label}
              ...
              <input
                ...
                className="border border-stone-600 rounded-lg px-3 py-2 text-sm w-28"
                placeholder="Value"
              />
