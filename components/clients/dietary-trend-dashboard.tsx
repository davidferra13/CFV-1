...
function BarChart({
  ...
  if (items.length === 0) {
    return (
      <p className="text-sm text-stone-400 py-2">No data yet</p>
    )
  }
    ...
    <div className="space-y-1.5">
      {items.map((item) => (
        <div key={item.value} className="flex items-center gap-2">
          <span className="text-sm w-28 truncate capitalize text-stone-700">
            {item.value}
          </span>
        ...
        <div className="p-4 rounded-lg border border-stone-200 bg-white">
          ...
          <div className="flex flex-wrap gap-2">
            {trends.risingTrends.map((trend) => (
              ...
              >
                <span className="text-sm capitalize font-medium text-green-800">
                  {trend.value}
                </span>
                <Badge
                  variant={
                    trend.change_type === 'allergy_added' ? 'error' : 'warning'
                  }
                >
