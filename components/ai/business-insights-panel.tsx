...
> = {
  thriving: { badge: 'success', bg: 'bg-green-50 border-green-200' },
  healthy: { badge: 'info', bg: 'bg-blue-50 border-blue-200' },
  needs_attention: { badge: 'warning', bg: 'bg-amber-50 border-amber-200' },
  at_risk: { badge: 'error', bg: 'bg-red-50 border-red-200' },
}
      ...
      <div className="bg-white border border-stone-200 rounded-lg p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-stone-700">Business Insights</span>
  ...
  return (
    <div className="bg-white border border-stone-200 rounded-lg p-4 space-y-3">
      ...
      <div className={`border rounded-lg p-3 ${health.bg}`}>
        <p className="text-sm font-medium text-stone-800">{result.headline}</p>
      </div>
          ...
          return (
            <div key={i} className="border border-stone-100 rounded p-3 space-y-1">
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-xs font-medium text-stone-700">{card.title}</span>
                <span className="text-[10px] text-stone-400 bg-stone-100 rounded px-1">
                  {card.category}
                </span>
              </div>
              <p className="text-xs text-stone-600">{card.insight}</p>
              <p className="text-xs text-brand-700 font-medium">→ {card.action}</p>
            </div>
          )
        })}
      </div>
