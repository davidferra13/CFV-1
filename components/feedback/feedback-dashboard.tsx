...
import { getAllFeedback, getFeedbackStats, toggleFeedbackPublic } from '@/lib/feedback/feedback-actions'
...
function RatingDisplay({ value, label }: { value: number; label: string }) {
  return (
    <div className="text-center">
      <div className="text-3xl font-bold text-amber-600">{value > 0 ? value.toFixed(1) : 'N/A'}</div>
    ...
    <div className="space-y-6">
      ...
      {stats && (
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-7">
          ...
          <div className="text-center">
            <div className={`text-3xl font-bold ${stats.npsScore >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              {stats.npsScore}
            </div>
      ...
      <div className="flex items-end gap-3 rounded-lg border border-gray-200 bg-gray-50 p-4">
        <div>
          <label htmlFor="dateFrom" className="block text-xs font-medium text-gray-600">From</label>
          <input
            id="dateFrom"
            type="date"
            value={dateFrom}
        ...
        <div>
          <label htmlFor="dateTo" className="block text-xs font-medium text-gray-600">To</label>
          <input
            id="dateTo"
            type="date"
            value={dateTo}
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    title={f.is_public ? 'Public (click to hide)' : 'Private (click to make public)'}
                  >
