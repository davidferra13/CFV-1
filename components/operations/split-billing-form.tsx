...
export function SplitBillingForm({
  ...
  const defaultSplits: SplitEntry[] = currentSplits && currentSplits.length > 0
    ? currentSplits
    : clients.length > 0
  ...
  const isValid = totalPercentage === 100 && splits.length > 0 && splits.every(s => s.clientId && s.percentage > 0)
      ...
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-1">
            Total: <span className="font-medium text-stone-900">${(totalCents / 100).toFixed(2)}</span>
          </p>
        </div>
              ...
              <div className="w-28">
                <div className="relative">
                  <input
                    ...
                    className="w-full border border-stone-300 rounded-lg px-3 py-2 pr-8 text-sm text-right focus:outline-none focus:ring-2 focus:ring-brand-500"
                  />
