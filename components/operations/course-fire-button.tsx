...
export function CourseFireButton({ courseId, currentStatus, onStatusChange }: CourseFireButtonProps) {
  ...
  function handleClick() {
    ...
    startTransition(async () => {
      try {
        ...
        toast.success(`Course ${action.label.toLowerCase()}${action.nextStatus === 'fired' ? 'd' : ''}`)
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : `Failed to ${action.label.toLowerCase()}`
        toast.error(message)
      }
    })
  }
