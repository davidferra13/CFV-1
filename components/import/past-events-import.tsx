...
function rowToInput(row: EventRow): HistoricalEventInput {
  ...
  return {
    ...
    service_style:
      (row.service_style as HistoricalEventInput['service_style']) || null,
    amount_paid_cents: amountCents,
    payment_method:
      (row.payment_method as HistoricalEventInput['payment_method']) || null,
...
export function PastEventsImport({
  ...
  const validRows = rows.filter(
    r => r.event_date && (r.client_id || r.client_name.trim())
  )
          ...
          <nav className="-mb-px flex gap-6">
            {(['manual', 'csv'] as InputMode[]).map(m => (
              <button
                key={m}
                type="button"
                onClick={() => { setInputMode(m); setCsvError(null); setCsvWarnings([]) }}
        ...
        {inputMode === 'csv' && (
          <div className="space-y-4">
            <Alert variant="info" title="Paste a CSV or spreadsheet export">
              Copy columns from Google Sheets, Excel, or Numbers and paste below.
              Recognized columns: Date, Client, Occasion, Guests, City, Amount, Payment Method, Notes.
            ...
            <textarea
              ...
              rows={10}
              className="w-full text-sm font-mono border border-stone-300 rounded-lg px-3 py-2 text-stone-900 focus:outline-none focus:ring-2 focus:ring-brand-500 resize-y"
            />
            ...
            <div className="flex items-center gap-3">
              ...
              <label className="cursor-pointer inline-flex items-center gap-2 text-sm text-brand-600 hover:text-brand-800 border border-dashed border-brand-300 hover:border-brand-500 rounded-lg px-4 py-2 transition-colors">
                <input
                  ref={fileInputRef}
                  type="file"
                  accept=".csv,.tsv,.txt"
        ...
        {inputMode === 'manual' && (
          <div className="space-y-4">
            <Alert variant="info" title="Log your past events">
              Fill in your historical gigs. Only date + client are required — everything
              else is optional. These events are logged as done and won&apos;t go through
              your normal booking workflow.
            </Alert>
            ...
            <div className="hidden sm:grid grid-cols-[160px_200px_1fr_80px_120px_120px_80px] gap-2 px-1">
              {['Date *', 'Client *', 'Occasion', 'Guests', 'City', 'Paid ($)', 'Method'].map(h => (
                <span key={h} className="text-xs font-semibold text-stone-500 uppercase tracking-wide">
                  {h}
                </span>
              ))}
            </div>
                ...
                <Card key={row.id} className="p-3 sm:p-4">
                  <div className="flex items-start gap-2 sm:gap-3">
                    ...
                    <div className="flex-1 space-y-2 sm:space-y-0 sm:grid sm:grid-cols-[160px_200px_1fr_80px_120px_120px_80px] sm:gap-2">
                      ...
                      <input
                        ...
                        className="w-full text-sm border border-stone-300 rounded-lg px-3 py-2 text-stone-900 focus:outline-none focus:ring-2 focus:ring-brand-500"
                      />
                      ...
                      <div>
                        {existingClients.length > 0 ? (
                          <select
                            ...
                            className="w-full text-sm border border-stone-300 rounded-lg px-2 py-2 bg-white text-stone-900 focus:outline-none focus:ring-2 focus:ring-brand-500"
                          >
                        ...
                        {(!row.client_id) && (
                          <input
                            ...
                            className={`w-full text-sm border border-stone-300 rounded-lg px-3 py-2 text-stone-900 focus:outline-none focus:ring-2 focus:ring-brand-500 ${existingClients.length > 0 ? 'mt-1' : ''}`}
                          />
                        )}
                      </div>
                  ...
                  <div className="mt-2 ml-7 sm:ml-8">
                    <input
                      ...
                      className="w-full text-sm border border-stone-200 rounded-lg px-3 py-1.5 text-stone-700 placeholder-stone-400 focus:outline-none focus:ring-1 focus:ring-brand-500 bg-stone-50"
                    />
                  </div>
                </Card>
              ))}
            ...
            <div className="flex gap-3">
              <button
                type="button"
                onClick={addRow}
                className="text-sm text-brand-600 hover:text-brand-800 border border-dashed border-brand-300 hover:border-brand-500 rounded-lg px-4 py-2 transition-colors"
              >
                + Add another event
              </button>
            </div>
    ...
    return (
      <div className="space-y-5">
        <Alert variant="info" title="Review before saving">
          ...
          {withPayment.length > 0 && ` Payment records will be saved for ${withPayment.length} event${withPayment.length !== 1 ? 's' : ''}.`}
        </Alert>
            ...
            <tbody>
              {inputs.map((input, i) => (
                <tr key={i} className="border-b border-stone-100">
                  <td className="py-2 pr-4 text-stone-900 whitespace-nowrap">{input.event_date}</td>
                  <td className="py-2 pr-4 text-stone-900">{input.client_name || '—'}</td>
                  <td className="py-2 pr-4 text-stone-600">{input.occasion || '—'}</td>
                  <td className="py-2 pr-4 text-stone-600">{input.guest_count ?? '—'}</td>
            ...
            {totalCents > 0 && (
              <tfoot>
                <tr className="border-t-2 border-stone-200">
                  <td colSpan={4} className="py-2 text-sm font-medium text-stone-700">
                    Total recorded payments
                  </td>
                  <td className="py-2 font-semibold text-stone-900">
        ...
        {withPayment.length > 0 && (
          <Alert variant="warning" title="Double-check your payment amounts">
            Payment amounts can&apos;t be changed after saving. Everything else about
            these events can be edited later, but not the payment figures.
          </Alert>
        )}
  ...
  if (phase === 'saving') {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-stone-600">Saving your events...</p>
      </div>
    )
  }
