...
function getDistributionWidth(course: MenuCourse, option: MenuOption) {
  ...
  return course.totalResponses > 0
    ? Math.max((option.responseCount / course.totalResponses) * 100, 4)
    : 0
}
              ...
              >
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {iteration.lockedCourseCount === iteration.courseCount &&
                      iteration.courseCount > 0 && <Badge variant="success">Finalized</Badge>}
                  </div>
          ...
          >
            ...
            <div className="mt-4 space-y-3">
              {course.options.map((option) => {
                ...
                const isSelectable =
                  Boolean(profileToken) && !course.lockedOptionId && !course.isClosed
                      ...
                      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {option.optionType === 'opt_out' && (
                              <Badge variant="warning">Opt out</Badge>
                            )}
                            {isManager &&
                              isRecommendedFinal &&
                              !course.lockedOptionId &&
                              isSelection && <Badge variant="info">Final pick</Badge>}
                          </div>
                          {description && (
