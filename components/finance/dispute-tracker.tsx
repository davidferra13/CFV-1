...
export function DisputeTracker({ initialDisputes }: Props) {
  ...
  const activeCount = disputes.filter((d) => d.status === 'open' || d.status === 'under_review').length
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        ...
        <Card>
          <CardContent className="py-3">
            ...
            <p className="text-2xl font-semibold text-stone-900">{formatCents(totalAtRiskCents)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="py-3">
            <p className="text-xs text-stone-500">Total Disputes</p>
            <p className="text-2xl font-semibold text-stone-600">{disputes.length}</p>
          </CardContent>
        </Card>
      </div>
        ...
        <Card className="border-stone-300">
          <CardContent className="py-4 space-y-3">
            <Input
              ...
              onChange={(e) => setNewDispute({ ...newDispute, amountCents: Math.round(parseFloat(e.target.value || '0') * 100) })}
            />
            <Input
              label="Reason"
            ...
            <div className="flex gap-2">
              <Button size="sm" onClick={handleCreate} loading={isPending}>Create</Button>
            ...
            <CardContent className="py-4">
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-sm font-medium text-stone-900">{formatCents(d.amountCents)}</span>
                    <Badge variant={STATUS_BADGE[d.status]}>{d.status.replace('_', ' ')}</Badge>
                  </div>
                  {d.reason && <p className="text-sm text-stone-600">{d.reason}</p>}
                  <p className="text-xs text-stone-400 mt-1">
                    Opened: {new Date(d.openedAt).toLocaleDateString()}
                    {d.resolvedAt && ` · Resolved: ${new Date(d.resolvedAt).toLocaleDateString()}`}
                  </p>
                  {d.evidenceNotes && (
                    <p className="text-xs text-stone-500 mt-2 p-2 bg-stone-50 rounded">
                ...
                {(d.status === 'open' || d.status === 'under_review') && (
                  <div className="flex gap-1">
                    <Button size="sm" variant="ghost" onClick={() => { setEditingId(d.id); setEvidenceNotes(d.evidenceNotes || '') }}>
                      Add Evidence
                    </Button>
                    <Button size="sm" variant="secondary" onClick={() => handleResolve(d.id, 'won')} disabled={isPending}>
                      Won
                    </Button>
                    <Button size="sm" variant="danger" onClick={() => handleResolve(d.id, 'lost')} disabled={isPending}>
                      Lost
                    </Button>
                  </div>
                )}
              ...
              {editingId === d.id && (
                <div className="mt-3 pt-3 border-t border-stone-100 space-y-2">
                  ...
                  />
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => handleAddEvidence(d.id)} loading={isPending}>Save Evidence</Button>
