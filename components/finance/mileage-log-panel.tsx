    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm text-stone-600">
          ...
          <span className="text-emerald-700 font-medium">${(totalDeduction / 100).toFixed(2)} deduction</span>
      ...
      <div className="space-y-1.5">
        {entries.map(entry => (
          <div key={entry.id} className="flex items-center justify-between text-sm border border-stone-100 rounded-md px-3 py-2">
            <div className="min-w-0">
              <p className="font-medium text-stone-900 truncate">{entry.description}</p>
              <p className="text-xs text-stone-400">{format(new Date(entry.tripDate), 'MMM d, yyyy')}</p>
            </div>
            <div className="flex items-center gap-3 shrink-0 ml-3">
              <span className="text-xs text-stone-600">{entry.miles} mi</span>
              <span className="text-xs text-emerald-700 font-medium">${(entry.deductionCents / 100).toFixed(2)}</span>
              <button onClick={() => handleDelete(entry.id)} className="text-stone-300 hover:text-red-500 transition-colors">
        ...
        <form onSubmit={handleAdd} className="border border-stone-200 rounded-md p-3 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input name="tripDate" type="date" required defaultValue={new Date().toISOString().slice(0, 10)} className="w-full text-sm border border-stone-200 rounded px-2 py-1.5 mt-0.5" />
            </div>
            <div>
              <label className="text-xs text-stone-600">Miles</label>
              <input name="miles" type="number" step="0.1" min="0.1" required placeholder="0.0" className="w-full text-sm border border-stone-200 rounded px-2 py-1.5 mt-0.5" />
            </div>
          </div>
          <div>
            <label className="text-xs text-stone-600">Description</label>
            <input name="description" required placeholder="e.g. Drive to venue" className="w-full text-sm border border-stone-200 rounded px-2 py-1.5 mt-0.5" />
          </div>
