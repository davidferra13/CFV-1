    ...
    <div className="space-y-6 max-w-3xl mx-auto" id="partner-contribution-report">
      ...
      <Card className="p-8 print:shadow-none print:border-none">
        <div className="text-center border-b border-stone-700 pb-6 mb-6">
          ...
          <h1 className="text-2xl font-bold text-stone-100">{chef?.name || 'Your Private Chef'}</h1>
        ...
        <div className="flex justify-between items-start flex-wrap gap-4">
          <div>
            ...
            {partner.contact_name && (
              <p className="text-stone-400 mt-2 text-sm">Contact: {partner.contact_name}</p>
            )}
            {partner.description && (
          ...
          <div className="text-right text-sm text-stone-500">
            <p>Generated</p>
            <p className="font-medium text-stone-100">{format(new Date(), 'MMMM d, yyyy')}</p>
          </div>
        </div>
      </Card>
      ...
      {/* Summary Stats */}
      <Card className="p-8 print:shadow-none print:border-none">
        <h3 className="text-lg font-semibold text-stone-100 mb-5">
          Our Contribution to Your Venue
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          <div>
            <p className="text-3xl font-bold text-stone-100">{stats.total_events}</p>
            <p className="text-sm text-stone-500 mt-1">Events Served</p>
          </div>
          <div>
            <p className="text-3xl font-bold text-stone-100">{stats.completed_events}</p>
            <p className="text-sm text-stone-500 mt-1">Completed</p>
          </div>
          <div>
            <p className="text-3xl font-bold text-stone-100">{stats.total_guests}</p>
      ...
      {partner.locations.length > 0 && (
        <Card className="p-8 print:shadow-none print:border-none">
          <h3 className="text-lg font-semibold text-stone-100 mb-4">By Location</h3>
          <div className="space-y-6">
            {partner.locations.map((loc) => {
              ...
              return (
                <div key={loc.id} className="border border-stone-700 rounded-lg p-5">
                  <div className="flex justify-between items-start flex-wrap gap-2 mb-3">
                    ...
                    <div className="text-right text-sm">
                      <p className="font-medium text-stone-100">
                        {locEvents.length} events · {locGuests} guests
                      </p>
                      {locRevenue > 0 && (
                        <p className="text-emerald-600 font-medium">
                          {formatCents(locRevenue)} in catering value
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <table className="w-full text-sm mt-3">
                    ...
                    <tbody>
                      {locEvents.map((evt) => (
                        <tr key={evt.id} className="border-b border-stone-800 last:border-0">
