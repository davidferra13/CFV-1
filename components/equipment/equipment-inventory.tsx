        ...
        .map(([category, items]) => (
          <div key={category} className="space-y-1">
            <h3 className="text-sm font-medium text-gray-600 capitalize flex items-center gap-2">
              <span
                className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${CATEGORY_COLORS[category as EquipmentCategory] || ''}`}
              >
                {category}
              </span>
                  ...
                  {editingId === eq.id ? (
                    ...
                    <>
                      <div className="flex-1 min-w-0">
                        ...
                        {eq.notes && <p className="text-xs text-gray-500 truncate">{eq.notes}</p>}
                      </div>
