...
export default function PackingChecklist({ checklist, showReturnColumn = false }: PackingChecklistProps) {
  ...
  const handleTogglePacked = useCallback((itemId: string) => {
    ...
    setItems((prev) =>
      prev.map((i) => (i.id === itemId ? { ...i, is_packed: !i.is_packed } : i))
    )
    setError(null)
  ...
  const handleToggleReturned = useCallback((itemId: string) => {
    const previousItems = items
    setItems((prev) =>
                ...
                {catItems.map((item) => (
                  <li key={item.id} className="flex items-center gap-2 text-sm">
                    <span className="inline-block w-4 h-4 border border-gray-400 rounded-sm flex-shrink-0">
                      {item.is_packed && <span className="block w-full h-full bg-gray-400 rounded-sm" />}
                    </span>
        ...
        .map(([category, catItems]) => (
          <div key={category} className="space-y-1">
            <h3 className="text-sm font-medium text-gray-600 flex items-center gap-2">
              <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${CATEGORY_COLORS[category] || CATEGORY_COLORS.other}`}>
                {category}
              </span>
