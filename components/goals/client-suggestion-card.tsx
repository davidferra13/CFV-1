...
export function ClientSuggestionCard({ suggestion }: ClientSuggestionCardProps) {
  ...
  return (
    <div className={`rounded-lg border p-3 flex items-start justify-between gap-3 transition-opacity ${isPending ? 'opacity-50' : 'opacity-100'} ${suggestion.status !== 'pending' ? 'border-stone-100 bg-stone-50' : 'border-stone-200 bg-white'}`}>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-stone-900 truncate">{suggestion.clientName}</p>
        <p className="text-xs text-stone-500 mt-0.5">{suggestion.reason}</p>
        {suggestion.avgSpendCents > 0 && (
          <p className="text-xs text-stone-400 mt-0.5">Lifetime value: {dollars(suggestion.lifetimeValueCents)}</p>
        )}
      ...
      {isActionable && (
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button
            ...
            disabled={isPending}
            className="rounded-md p-1.5 text-stone-400 hover:text-stone-600 hover:bg-stone-100 disabled:opacity-50 transition-colors"
