...
const CHANNEL_OPTIONS: Array<{ value: Channel; label: string; icon: string; note?: string }> = [
  ...
  { value: 'call_note',      label: 'Log a call',     icon: '📞', note: 'Saves a record — does not place a call' },
  { value: 'instagram_note', label: 'Instagram note', icon: '📸', note: 'Saves a record — copy and paste into Instagram' },
]
...
export function DirectOutreachPanel({
  ...
  const bodyPlaceholder =
    channel === 'email'          ? 'Write your message here…' :
    channel === 'sms'            ? 'Keep it brief (160 chars max for one segment)' :
    channel === 'call_note'      ? 'What did you discuss? Any follow-ups?' :
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Compose form */}
      {open && !sent && (
        <form onSubmit={handleSend} className="space-y-3 rounded-lg border border-stone-200 p-4 bg-stone-50">
          {/* Channel selector */}
          <div className="flex gap-2 flex-wrap">
            {CHANNEL_OPTIONS.map((opt) => {
              const unavailable =
                (opt.value === 'email' && !clientEmail) ||
                (opt.value === 'sms' && !clientPhone)
              return (
                <button
                  key={opt.value}
                  type="button"
          ...
          <div>
            <label className="block text-xs font-medium text-stone-600 mb-1">
              {channel === 'call_note' ? 'Call notes' :
               channel === 'instagram_note' ? 'Instagram DM content' :
               'Message'}
            </label>
            <textarea
              className="w-full rounded-md border border-stone-200 bg-white px-3 py-2 text-sm min-h-[100px] resize-y"
              value={body}
          ...
          <div className="flex gap-2">
            <Button type="submit" size="sm" disabled={saving || !body.trim()}>
              {saving ? 'Sending…' :
               channel === 'call_note' ? 'Log call' :
               channel === 'instagram_note' ? 'Log note' :
               'Send'}
            </Button>
            <Button
              type="button"
      ...
      {history.length > 0 && (
        <div className="space-y-1.5 mt-2">
          <p className="text-xs font-medium text-stone-400 uppercase tracking-wide">Outreach history</p>
          {history.map((record) => (
            <div
              key={record.id}
              className="flex items-start gap-2 text-xs text-stone-600 rounded-md border border-stone-100 bg-white px-3 py-2"
            >
              <span className="shrink-0">{CHANNEL_ICON[record.channel] ?? '•'}</span>
              <div className="min-w-0 flex-1">
                {record.subject && (
                  <p className="font-medium text-stone-800 truncate">{record.subject}</p>
                )}
