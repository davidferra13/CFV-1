    ...
    startTransition(async () => {
      try {
        ...
        setTests(prev =>
          prev.map(t =>
            t.id === testId
              ? { ...t, winner, resolvedAt: new Date().toISOString() }
              : t
          )
        )
      ...
      <CardContent className="space-y-6">
        ...
        {showCreateForm && (
          <div className="rounded-lg border border-brand-200 bg-brand-50/30 p-4 space-y-4">
            <h4 className="text-sm font-semibold text-stone-900">Create A/B Test</h4>

            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1.5">Campaign</label>
              <select
            ...
            <div>
              ...
              <p className="text-xs text-stone-500 mt-1">
                {testPercent}% of recipients will receive each variant. The remaining {100 - testPercent * 2}% will receive the winner.
              </p>
            </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {activeTests.map(test => (
                <div key={test.id} className="rounded-lg border border-stone-200 p-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-3">
                    <div className="rounded-lg bg-stone-50 p-3">
                      ...
                      <p className="text-sm text-stone-900">{test.variantASubject}</p>
                    </div>
                    <div className="rounded-lg bg-stone-50 p-3">
                      <p className="text-xs font-medium text-stone-500 mb-1">Variant B</p>
                      <p className="text-sm text-stone-900">{test.variantBSubject}</p>
                    </div>
                  </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {resolvedTests.map(test => (
                <div key={test.id} className="flex items-center gap-3 rounded-lg border border-stone-200 p-3">
