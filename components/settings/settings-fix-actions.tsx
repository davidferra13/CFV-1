...
function TaskCard({ task, primary = false }: { task: SettingsFixTask; primary?: boolean }) {
  const toneStyles = SETTINGS_TONE_STYLES[mapToSettingsTone(task.tone)]

  return (
    <div
      className={`relative overflow-hidden rounded-xl border bg-[image:var(--card-gradient)] shadow-[var(--shadow-card)] ${toneStyles.panel}`}
    >
      <div className={primary ? 'p-5 sm:p-6' : 'p-4 sm:p-5'}>
        ...
        <div className="mt-4 space-y-2">
          <h2
            className={
              primary
                ? 'text-2xl font-semibold text-stone-50'
...
export function SettingsFixActions({ tasks }: { tasks: SettingsFixTask[] }) {
  if (tasks.length === 0) {
    return (
      <section className="rounded-xl border border-stone-700/80 bg-[var(--glass-subtle-bg)] p-5 shadow-[var(--shadow-card)] sm:p-6">
        <div className="section-label mb-3">Fix This Setting</div>
        <div className="space-y-2">
          <h2 className="text-2xl font-semibold text-stone-50">
            Core configuration looks healthy.
          </h2>
          <p className="max-w-2xl text-sm leading-6 text-stone-300">
            No missing profile, intake, availability, booking, or review blockers were detected from
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-stone-300">
            These actions come from missing or broken setup state, so you can fix the exact setting
            instead of opening a directory first.
          </p>
        </div>
        <Link
          href="#settings-directory"
