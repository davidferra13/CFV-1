...
export function BusinessHoursEditor({ config, chefId }: Props) {
  ...
  const defaultData = useMemo(
    () => ({
      ...
      outsideMessage: defaultOutsideMessage,
      emergencyEnabled: config?.emergency_enabled ?? true,
      emergencyHours: config?.emergency_window_hours ?? 24,
    }),
    [config, defaultOutsideMessage]
  )

  const currentData = useMemo(
    () => ({ timezone, schedule, outsideMessage, emergencyEnabled, emergencyHours }),
    [timezone, schedule, outsideMessage, emergencyEnabled, emergencyHours]
  )

  const protection = useProtectedForm({
    surfaceId: 'business-hours',
    recordId: null,
    tenantId: chefId,
    defaultData,
    currentData,
  ...
  function applyDraftData(data: Record<string, unknown>) {
    ...
    if (typeof data.outsideMessage === 'string') setOutsideMessage(data.outsideMessage)
    if (typeof data.emergencyEnabled === 'boolean') setEmergencyEnabled(data.emergencyEnabled)
    if (typeof data.emergencyHours === 'number') setEmergencyHours(data.emergencyHours)
  }
    ...
    >
      <div className="bg-stone-900 border border-stone-800 rounded-lg p-6">
        ...
        <p className="text-stone-400 text-sm mb-4">
          Messages received outside your hours get an automatic reply. Day-of-event messages always
          go through.
        </p>
