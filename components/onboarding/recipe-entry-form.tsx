...
const CATEGORIES = [
  ...
  'bread', 'pasta', 'soup', 'salad', 'appetizer', 'condiment', 'beverage', 'other',
] as const
...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  pasta: 'bg-amber-100 text-amber-800',
  soup: 'bg-cyan-100 text-cyan-800',
  salad: 'bg-emerald-100 text-emerald-800',
  appetizer: 'bg-violet-100 text-violet-800',
  condiment: 'bg-stone-100 text-stone-800',
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="flex h-10 w-full rounded-md border border-stone-300 bg-white px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stone-400 capitalize"
                >
                  {CATEGORIES.map(c => (
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                rows={4}
                className="flex w-full rounded-md border border-stone-300 bg-white px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stone-400 resize-y"
              />
            </div>
              ...
              <ul className="divide-y divide-stone-100 max-h-96 overflow-y-auto">
                {recipes.map(r => (
                  <li key={r.id} className="py-2.5 flex items-center justify-between gap-2">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 capitalize ${CATEGORY_COLORS[r.category] ?? 'bg-stone-100 text-stone-700'}`}>
                      {r.category}
                    </span>
                  </li>
                ))}
