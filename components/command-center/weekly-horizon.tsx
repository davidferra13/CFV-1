  ...
  return (
    <div
      className={`flex flex-col items-center gap-1 py-2 px-1 rounded-lg transition-colors ${
        day.isToday ? 'bg-brand-950/50 border border-brand-800/40' : 'hover:bg-stone-800/30'
      }`}
    >
      <span
      ...
      <span className={`text-sm font-medium ${day.isToday ? 'text-brand-300' : 'text-stone-300'}`}>
        {dayNum}
      </span>
      {hasEvents ? (
        <div className="flex gap-0.5">
          {Array.from({ length: Math.min(day.eventCount, 4) }).map((_, i) => (
            <div
              key={i}
              className={`w-1.5 h-1.5 rounded-full ${isBusy ? 'bg-amber-400' : 'bg-brand-400'}`}
            />
          ))}
          {day.eventCount > 4 && (
