...
function legTypeBadgeVariant(type: string): 'default' | 'info' | 'warning' {
  switch (type) {
    case 'specialty_sourcing': return 'warning'
    case 'service_travel': return 'info'
    case 'return_home': return 'info'
    default: return 'default'
  }
}
  ...
  return (
    <div className="flex items-center justify-between py-1.5 border-b border-stone-100 last:border-0">
      <div className="flex items-start gap-2">
        ...
        <div>
          <p className={`text-sm font-medium ${ingredient.status === 'sourced' ? 'line-through text-stone-400' : 'text-stone-800'}`}>
          ...
          <div className="flex gap-2 text-xs text-stone-400 mt-0.5">
            ...
            {ingredient.store_name && (
              <span>@ {ingredient.store_name}</span>
            )}
            {ingredient.notes && (
              <span className="italic">{ingredient.notes}</span>
            )}
          </div>
        </div>
      </div>
      <select
        ...
        className="text-xs border border-stone-200 rounded px-1.5 py-1 bg-white focus:outline-none focus:ring-1 focus:ring-brand-500"
      >
    ...
    <Card className="overflow-hidden">
      ...
      >
        <div className="flex items-start gap-3">
          {/* Date bubble */}
          <div className="flex-shrink-0 text-center bg-stone-100 rounded-lg px-2 py-1 min-w-[48px]">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge variant={legStatusVariant(leg.status)}>
                {LEG_STATUS_LABELS[leg.status]}
              </Badge>
              {leg.linked_event_ids.length > 0 && (
                <Badge variant="info">
                  Shared · {leg.linked_event_ids.length + 1} events
                </Badge>
              )}
            </div>
            ...
            <div className="flex gap-3 mt-1 text-xs text-stone-500">
              {leg.departure_time && (
                <span>Depart: {formatLegTime(leg.departure_time)}</span>
              )}
              ...
              {hasIngredients && (
                <span>{leg.ingredients.filter((i) => i.status === 'sourced').length}/{leg.ingredients.length} sourced</span>
              )}
            </div>
        ...
        <div className="px-4 pb-4 border-t border-stone-100 space-y-4">
            ...
            <div className="mt-3">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="text-sm font-medium text-stone-800">{leg.origin_label || 'Origin'}</p>
                    {leg.origin_address && (
                ...
                {leg.stops.map((stop, i) => (
                  <div key={i} className="flex items-start gap-2 ml-2.5">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="flex-shrink-0 w-5 h-5 rounded-full bg-stone-100 text-stone-600 text-xs font-bold flex items-center justify-center">
                        {i + 1}
                      </span>
                    </div>
                    <div className="ml-1">
                ...
                {(leg.destination_label || leg.destination_address) && (
                  <div className="flex items-start gap-2 ml-2.5">
                    ...
                    <div className="ml-1">
                      <p className="text-sm font-medium text-stone-800">{leg.destination_label || 'Destination'}</p>
              ...
              {(leg.total_drive_minutes || leg.total_stop_minutes) && (
                <div className="flex gap-4 mt-2 text-xs text-stone-500 bg-stone-50 rounded px-3 py-1.5">
                  ...
                  {leg.total_estimated_minutes && (
                    <span className="font-medium">Total: {formatMinutes(leg.total_estimated_minutes)}</span>
                  )}
                </div>
              )}
            </div>
          ...
          {leg.purpose_notes && (
            <div>
              ...
              <p className="text-sm text-stone-700 whitespace-pre-wrap">{leg.purpose_notes}</p>
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-2 pt-2 border-t border-stone-100">
            ...
            {leg.status !== 'cancelled' && leg.status !== 'completed' && (
              <Button
                size="sm"
                variant="ghost"
                onClick={() => onStatusChange(leg.id, 'cancelled')}
              >
                Cancel trip
              </Button>
            )}
...
function EmptyState({ onAdd }: { onAdd: () => void }) {
  return (
    <div className="text-center py-12 space-y-3">
      ...
      <h3 className="text-lg font-semibold text-stone-700">No travel legs planned yet</h3>
...
export function TravelPlanClient({
  ...
  const handleSave = (savedLeg: TravelLeg) => {
    setLegs((prev) => {
      ...
      if (exists) {
        return prev.map((l) =>
          l.id === savedLeg.id ? { ...l, ...savedLeg } : l
        )
      }
      // New leg: add with empty ingredients
      return [...prev, { ...savedLeg, ingredients: [], linked_event_ids: savedLeg.linked_event_ids ?? [] }].sort(
        (a, b) =>
      ...
      setLegs((prev) =>
        prev.map((leg) => ({
          ...
          ingredients: leg.ingredients.map((ing) =>
            ing.id === ingredientId
              ? { ...ing, status, sourced_at: status === 'sourced' ? new Date().toISOString() : null }
              : ing
          ),
        }))
      )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-stone-900">Travel Plan</h2>
        ...
        <div className="flex gap-2">
          {!isFormOpen && (
            <Button onClick={() => setEditingLeg(null)}>
              + Add trip
            </Button>
          )}
        </div>
      ...
      {!isFormOpen && plan.nearbyEvents.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm">
          <p className="font-medium text-amber-800">
            You have {plan.nearbyEvents.length} other event{plan.nearbyEvents.length !== 1 ? 's' : ''} this week:
          </p>
      ...
      {/* Empty state */}
      {legs.length === 0 && !isFormOpen && (
        <EmptyState onAdd={() => setEditingLeg(null)} />
      )}
      ...
      {cancelledLegs.length > 0 && (
        <details>
          <summary className="cursor-pointer text-sm font-medium text-stone-400 hover:text-stone-600">
      ...
      {isPending && (
        <p className="text-xs text-stone-400 text-center">Saving…</p>
      )}
    </div>
  )
}
