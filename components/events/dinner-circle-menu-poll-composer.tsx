...
export function DinnerCircleMenuPollComposer({
  ...
  const availableCourses = Array.from(
    new Set(dishes.map((dish) => dish.course?.trim()).filter((value): value is string => Boolean(value)))
  ...
  const handlePublish = () => {
    ...
    startTransition(async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : 'Failed to publish Dinner Circle menu polling')
      }
    })
  }
    ...
    <div className="space-y-5">
          ...
          <div key={course.id} className="rounded-2xl border border-stone-800 bg-stone-900/70 p-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-stone-500">
                  {course.dishIds.length} dish option{course.dishIds.length === 1 ? '' : 's'} selected
                </p>
              </div>
              {courses.length > 1 && (
                <Button
            ...
            <div className="mt-4 grid gap-3 lg:grid-cols-2">
              <Input
                ...
                onChange={(event) =>
                  updateCourse(course.id, (current) => ({ ...current, courseName: event.target.value }))
                }
              ...
              <Input
                ...
                onChange={(event) =>
                  updateCourse(course.id, (current) => ({ ...current, question: event.target.value }))
                }
            ...
            <div className="mt-4 rounded-xl border border-stone-800 bg-stone-950/60 p-4">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    updateCourse(course.id, (current) => ({ ...current, dishSearch: event.target.value }))
                  }
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="text-sm font-medium text-stone-100">{dish.name}</span>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <Button
          variant="secondary"
          onClick={() =>
            setCourses((current) => [...current, createEmptyCourseDraft(availableCourses[current.length] ?? '')])
          }
        >
          Add course
        </Button>
        <Button variant="primary" onClick={handlePublish} loading={isPending} disabled={dishes.length === 0}>
          {publishLabel}
        </Button>
      </div>
    </div>
