...
const COLUMNS: KanbanColumn[] = [
  { key: 'draft', label: 'Draft', statuses: ['draft'], headerClass: 'bg-stone-800 text-stone-300' },
  {
    ...
    statuses: ['proposed'],
    headerClass: 'bg-violet-900 text-violet-200',
  },
  {
    key: 'accepted',
    label: 'Accepted',
    statuses: ['accepted'],
    headerClass: 'bg-blue-900 text-blue-200',
  },
  { key: 'paid', label: 'Paid', statuses: ['paid'], headerClass: 'bg-indigo-900 text-indigo-200' },
  {
    key: 'confirmed',
    label: 'Confirmed',
    statuses: ['confirmed'],
    headerClass: 'bg-teal-900 text-teal-200',
  },
  {
    key: 'in_progress',
    label: 'In Progress',
    statuses: ['in_progress'],
    headerClass: 'bg-amber-900 text-amber-200',
  },
  {
    key: 'done',
    label: 'Done',
    statuses: ['completed', 'cancelled'],
    headerClass: 'bg-stone-800 text-stone-500',
  },
]
        ...
        {COLUMNS.map((col) => {
          ...
          return (
            <div key={col.key} className="w-60 flex flex-col gap-2">
              {/* Column header */}
              <div
                className={`flex items-center justify-between rounded-md px-3 py-2 ${col.headerClass}`}
              >
              ...
              <div className="flex flex-col gap-2 min-h-[80px]">
                ...
                {colEvents.length === 0 && (
                  <div className="rounded-md border border-dashed border-stone-700 px-3 py-4 text-center">
...
function KanbanCard({ event }: { event: Event }) {
  ...
  return (
    ...
    >
      <p className="text-xs font-semibold text-stone-100 truncate leading-snug">
