...
function parseEventDateTime(eventDate: string, serveTime?: string | null): Date | null {
  ...
  const normalizedTime = trimmedTime.replace(/\./g, '').replace(/\s+/g, ' ').toUpperCase()
...
export function ChefEventCountdown({ eventDate, serveTime, status }: Props) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-0.5 text-lg font-semibold tabular-nums text-amber-100">{countdown.value}</p>
