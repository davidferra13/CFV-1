...
export function SocialPlannerClient({
  ...
  const selectedPost = useMemo(
    () => rows.find((row) => row.id === selectedPostId),
    [rows, selectedPostId]
  )
  const selectedLinks = useMemo(
    () => assetLinks.filter((link) => link.post_id === selectedPostId),
    [assetLinks, selectedPostId]
  )
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Annual Queue</CardTitle>
        </CardHeader>
        <CardContent className="flex items-center gap-4">
          <div className="text-sm text-stone-400">
            Posts: {summary.totalPosts} | Year: {settings.target_year}
          </div>
          ...
          <Button onClick={onGenerate} loading={isPending}>
            Generate/Regenerate
          </Button>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-3">
          <div className="grid gap-2 md:grid-cols-3">
            <input
              ...
              multiple
              accept="image/*,video/*"
              className="rounded border border-stone-600 px-2 py-2 text-sm"
            ...
            />
            <Button onClick={onUploadAssets} loading={isPending}>
              Upload
            </Button>
          </div>
          <div className="overflow-auto border border-stone-700 rounded-lg max-h-48">
            <table className="min-w-full text-xs">
              ...
              <tbody>
                {vault.slice(0, 200).map((asset) => (
                  <tr key={asset.id}>
                    <td className="px-2 py-1">{asset.asset_name}</td>
                    <td className="px-2 py-1">{asset.asset_kind}</td>
                    <td className="px-2 py-1">{asset.asset_tags.join(', ')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Card>
        ...
        <CardContent className="overflow-auto max-h-64">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {rows.slice(0, 300).map((post) => (
                <tr key={post.id}>
                  <td className="px-2 py-1">{post.post_code}</td>
                  <td className="px-2 py-1">{post.title}</td>
                  <td className="px-2 py-1">
                    {post.preflight_ready ? (
                      <Badge variant="success">Ready</Badge>
                    ) : (
                      <Badge variant="warning">{post.preflight_missing_items.length} missing</Badge>
                    )}
                  </td>
                  <td className="px-2 py-1">
                    <select
                      value={post.status}
                      onChange={(e) =>
                        onQuickStatusChange(post.id, e.target.value as SocialPostStatus)
                      }
                      className="rounded border border-stone-600 px-1 py-0.5"
                    >
                      {Object.entries({
                        idea: 'Idea',
                        draft: 'Draft',
      ...
      {selectedPost && editor && (
        <Card>
          <CardHeader>
            <CardTitle>Edit {selectedPost.post_code}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="grid gap-2 md:grid-cols-3">
              ...
              <Input
                label="CTA"
                value={editor.cta}
                onChange={(e) => setEditor({ ...editor, cta: e.target.value })}
              />
              <Input
                label="Location tag"
                value={editor.location_tag}
                onChange={(e) => setEditor({ ...editor, location_tag: e.target.value })}
              />
              <Input
                label="Hashtags"
                value={editor.hashtags}
                onChange={(e) => setEditor({ ...editor, hashtags: e.target.value })}
              />
              <Input
                label="Mention tags"
                value={editor.mention_handles}
                onChange={(e) => setEditor({ ...editor, mention_handles: e.target.value })}
              />
              <Input
                label="Media URL (optional)"
                value={editor.media_url}
                onChange={(e) => setEditor({ ...editor, media_url: e.target.value })}
              />
            </div>
            <Textarea
              label="Alt text"
              value={editor.alt_text}
              onChange={(e) => setEditor({ ...editor, alt_text: e.target.value })}
            />
            <Textarea
              label="Master caption"
              value={editor.caption_master}
              onChange={(e) => setEditor({ ...editor, caption_master: e.target.value })}
            />
            <div className="flex gap-2 items-end">
              <select
                className="rounded border border-stone-600 px-2 py-2 text-sm min-w-72"
                value={assetToAttach}
                onChange={(e) => setAssetToAttach(e.target.value)}
              >
              ...
              <Button size="sm" onClick={onAttach} loading={isPending}>
                Attach + Primary
              </Button>
            </div>
            <div className="space-y-1">
              {selectedLinks.map((link) => {
                ...
                return (
                  <div
                    key={link.id}
                    className="text-xs border rounded px-2 py-1 flex items-center justify-between"
                  >
                    {asset.asset_name}
                    <button
                      type="button"
                      className="text-red-600"
                      onClick={() => onDetachAsset(link.id)}
                    >
                      Detach
                    </button>
                  </div>
            ...
            <Button onClick={onSavePost} loading={isPending}>
              Save Post
            </Button>
          </CardContent>
        </Card>
