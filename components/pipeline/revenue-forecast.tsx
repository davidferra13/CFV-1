    ...
    return (
      <Card className="p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-semibold text-stone-900 text-sm">Pipeline Forecast</h3>
        </div>
        <p className="text-sm text-stone-500">No open inquiries or events in the pipeline.</p>
        <Link href="/inquiries" className="text-sm text-brand-600 hover:underline mt-2 inline-block">
          View Pipeline →
        </Link>
      </Card>
    )
  ...
  return (
    <Card className="p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href="/inquiries" className="text-xs text-stone-500 hover:text-brand-600 transition-colors">
          {totalCount} open →
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        <div className="bg-stone-50 rounded-lg p-3">
          <p className="text-xs text-stone-600 font-medium mb-0.5">Best Case</p>
          <p className="text-xl font-bold text-stone-800">{formatCurrency(bestCaseCents)}</p>
      ...
      {stagesWithAmount.length > 0 && (
        <div className="space-y-1.5">
          {stagesWithAmount.map((stage) => {
            const barWidth = bestCaseCents > 0
              ? Math.round((stage.expectedCents / bestCaseCents) * 100)
              : 0
            return (
              <div key={stage.stage}>
                <div className="flex items-center justify-between text-xs mb-0.5">
                  ...
                  <span className="text-stone-500">
                    ...
                    <span className="text-stone-400 ml-1">({Math.round(stage.probability * 100)}%)</span>
                  </span>
                </div>
