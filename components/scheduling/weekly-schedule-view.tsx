...
const DAY_TYPE_COLORS: Record<string, string> = {
  event: 'bg-brand-50 border-brand-200',
  prep: 'bg-amber-50 border-amber-200',
  admin: 'bg-stone-50 border-stone-200',
  free: 'bg-white border-stone-100',
}
    ...
    <div className="space-y-6">
            ...
            >
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <div className={`text-xs font-semibold uppercase tracking-wide ${isToday ? 'text-brand-600' : 'text-stone-500'}`}>
                      {day.dayOfWeek.slice(0, 3)}
                    </div>
                    <div className={`text-lg font-bold ${isToday ? 'text-brand-600' : 'text-stone-900'}`}>
                      {new Date(day.date + 'T12:00:00').getDate()}
                    </div>
                  </div>
                  <span className={`text-xs px-1.5 py-0.5 rounded ${
                    day.dayType === 'event' ? 'bg-brand-100 text-brand-700' :
                    day.dayType === 'prep' ? 'bg-amber-100 text-amber-700' :
                ...
                {day.events.map((event) => (
                  <Link key={event.id} href={`/events/${event.id}`} className="block">
                    <div className="bg-white rounded p-2 border border-stone-200 hover:shadow-sm transition-shadow">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className={`text-xs px-1 py-0.5 rounded ${PREP_STATUS_COLORS[event.prepStatus]}`}>
                          {event.prepStatus === 'ready' ? 'Ready' :
                           event.prepStatus === 'partial' ? 'Partial' : 'Not started'}
                        </span>
                      </div>
                    </div>
                  </Link>
                ...
                {day.dayType === 'prep' && day.isPrepDayFor && (
                  <div className="text-xs text-amber-700 bg-amber-100 rounded p-1.5">
      ...
      <div className="flex gap-4 text-xs text-stone-500 justify-center">
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded bg-brand-100 border border-brand-200" /> Event
        </span>
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded bg-amber-100 border border-amber-200" /> Prep
        </span>
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded bg-white border border-stone-200" /> Free
        </span>
      </div>
    </div>
  )
