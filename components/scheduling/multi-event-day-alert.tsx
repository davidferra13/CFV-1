  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-stone-700">Double-Booked Days</h3>
      </div>
      <div className="space-y-2">
        {days.map(day => (
          <div
          ...
          >
            <p className="text-xs font-semibold text-amber-900 mb-1.5">
              {format(new Date(day.date + 'T12:00:00'), 'EEEE, MMMM d')} — {day.events.length} events
            </p>
            <ul className="space-y-1">
              {day.events.map(ev => (
                <li key={ev.id} className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-[10px] text-amber-600 capitalize shrink-0">{ev.status}</span>
                </li>
              ))}
            </ul>
          </div>
