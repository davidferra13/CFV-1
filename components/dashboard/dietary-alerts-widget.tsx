...
export function DietaryAlertsWidget() {
  ...
  useEffect(() => {
    startTransition(async () => {
      try {
        const [alertStats, alerts] = await Promise.all([
          getAlertStats(),
          getDietaryAlerts(true),
        ])
        setStats(alertStats)
    ...
    <div className="p-4 rounded-lg border border-stone-200 bg-white">
      <div className="flex items-center justify-between mb-3">
        ...
        {stats.unacknowledged > 0 && (
          <Badge variant="error">{stats.unacknowledged}</Badge>
        )}
      </div>
      ...
      {stats.unacknowledged === 0 ? (
        ...
        <>
          ...
          <div className="flex gap-3 mb-3 text-xs text-stone-500">
            ...
            {stats.warning > 0 && (
              <span className="text-amber-600">{stats.warning} warning</span>
            )}
            {stats.info > 0 && (
              <span className="text-blue-600">{stats.info} info</span>
            )}
          </div>
              ...
              {criticalAlerts.map((alert) => (
                ...
                >
                  <Badge
                    variant={
                      severityBadge[
                        alert.severity as keyof typeof severityBadge
                      ] || 'default'
                    }
                  >
                    {alert.severity}
