      ...
      <Card className="border-emerald-200 bg-emerald-50">
        ...
        <CardContent>
          <div className="text-center py-2">
            ...
            <p className="text-xs text-emerald-600 mt-1">No outstanding tasks across your upcoming events</p>
          </div>
        </CardContent>
      </Card>
    )
    ...
    <Card>
      <CardHeader>
        ...
        <div className="flex flex-wrap gap-2 mt-1">
          ...
          {digest.upcomingCount > 0 && (
            <span className="text-xs text-stone-500 bg-stone-50 px-2 py-0.5 rounded-full border border-stone-200">
      ...
      <CardContent className="space-y-6">
        {Array.from(groups.entries()).map(([eventId, tasks]) => {
          ...
          return (
            <div key={eventId}>
              {/* Event header row */}
              <Link
                href={first.eventHref}
                className="flex items-center justify-between mb-2 group"
              >
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-stone-800 group-hover:text-brand-600 truncate">
                    {first.eventOccasion ?? 'Private Event'} — {first.clientName}
                  </p>
                  <p className={`text-xs mt-0.5 ${hasOverdue ? 'text-red-500 font-medium' : 'text-stone-400'}`}>
                    {displayDate}
                    {hasOverdue
              ...
              <div className="space-y-1 pl-2 border-l-2 border-stone-100">
                {tasks.slice(0, 5).map(task => (
                  <div
                    key={task.taskId}
                    className={`flex items-start gap-2.5 rounded-md px-2 py-1.5 ${
                      task.isOverdue ? 'bg-red-50 border border-red-100' : ''
                    }`}
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={`text-sm font-medium truncate ${
                          task.isOverdue ? 'text-red-700' : 'text-stone-700'
                        }`}>
                          {task.taskLabel}
                        </span>
                      </div>
                      {task.isOverdue && task.deadline && (
                        <p className="text-xs text-red-500 mt-0.5 pl-5">
                          Was due {formatDeadlineTime(task.deadline) || new Date(task.deadline + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                        </p>
                      )}
                    ...
                    {/* Phase badge */}
                    <span className={`text-xs shrink-0 px-1.5 py-0.5 rounded whitespace-nowrap ${
                      task.isOverdue
                ...
                {tasks.length > 5 && (
                  <Link
                    ...
                    className="text-xs text-brand-600 hover:text-brand-700 pl-2 block pt-1"
                  >
        ...
        {groups.size > 3 && (
          <Link
            href="/calendar"
            className="block text-center text-sm text-brand-600 hover:text-brand-700 pt-2 border-t border-stone-100"
          >
            View all events →
          </Link>
        )}
