...
const STEPS: Array<{
  ...
  { key: 'profile',  label: 'Complete your profile',    detail: 'Business name + display name',   href: '/settings/profile' },
  { key: 'clients',  label: 'Add your first client',     detail: 'Import or enter manually',       href: '/onboarding/clients' },
  { key: 'loyalty',  label: 'Configure loyalty program', detail: 'Set up rewards for repeat clients', href: '/onboarding/loyalty' },
  { key: 'recipes',  label: 'Enter your first recipe',   detail: 'Build your culinary library',    href: '/onboarding/recipes' },
  { key: 'staff',    label: 'Add a team member',         detail: 'Staff roster for scheduling',    href: '/onboarding/staff' },
]

function isDone(progress: OnboardingProgress, key: string): boolean {
  const val = progress[key as keyof OnboardingProgress]
  if (typeof val === 'boolean') return val
  if (typeof val === 'object' && val !== null && 'done' in val) return (val as { done: boolean }).done
  return false
}
              ...
              <li key={step.key}>
                <Link
                  ...
                  className={`flex items-start gap-2.5 rounded-md px-2 py-1.5 transition-colors ${
                      ...
                      : 'hover:bg-brand-100/60 cursor-pointer'
                  }`}
                >
                  {done ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-xs font-medium ${done ? 'text-stone-400 line-through' : 'text-stone-800'}`}>
                      {step.label}
                    </p>
                    {!done && (
                      <p className="text-[11px] text-stone-400 mt-0.5">{step.detail}</p>
                    )}
                  </div>
