      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          <div className="rounded-lg border border-stone-200 bg-stone-50 p-3">
            ...
            <p className="text-xl font-semibold text-stone-900 mt-1">{insights.total_journeys}</p>
          </div>
          <div className="rounded-lg border border-stone-200 bg-stone-50 p-3">
            <p className="text-xs uppercase tracking-wide text-stone-500 flex items-center gap-1">
            ...
            <p className="text-xl font-semibold text-stone-900 mt-1">{insights.total_entries}</p>
          </div>
          <div className="rounded-lg border border-stone-200 bg-stone-50 p-3">
            <p className="text-xs uppercase tracking-wide text-stone-500 flex items-center gap-1">
            ...
            <p className="text-xl font-semibold text-stone-900 mt-1">{insights.total_media}</p>
          </div>
          <div className="rounded-lg border border-stone-200 bg-stone-50 p-3">
            <p className="text-xs uppercase tracking-wide text-stone-500 flex items-center gap-1">
            ...
            <p className="text-xl font-semibold text-stone-900 mt-1">{insights.mapped_entries}</p>
          </div>
          <div className="rounded-lg border border-stone-200 bg-stone-50 p-3">
            <p className="text-xs uppercase tracking-wide text-stone-500 flex items-center gap-1">
            ...
            <p className="text-xl font-semibold text-stone-900 mt-1">{insights.total_recipe_links}</p>
          </div>
          <div className="rounded-lg border border-stone-200 bg-stone-50 p-3">
            <p className="text-xs uppercase tracking-wide text-stone-500 flex items-center gap-1">
            ...
            <p className="text-xl font-semibold text-stone-900 mt-1">{adoptionRate(insights)}%</p>
          </div>
        </div>
        ...
        {latestJourney ? (
          <div className="rounded-lg border border-stone-200 p-3 space-y-2">
            <div className="flex items-center justify-between gap-2">
              <p className="text-sm font-semibold text-stone-900 truncate">{latestJourney.title}</p>
              <Link
            ...
            <p className="text-xs text-stone-500">{formatJourneyDateRange(latestJourney.started_on, latestJourney.ended_on)}</p>
            {destinationLabel(latestJourney) && (
              <p className="text-xs text-stone-600">{destinationLabel(latestJourney)}</p>
            )}
            <div className="flex flex-wrap gap-2 text-xs text-stone-500">
              ...
              <span>{latestJourney.adopted_idea_count}/{latestJourney.idea_count} adopted ideas</span>
            </div>
          </div>
        ) : (
          <div className="rounded-lg border border-dashed border-stone-300 p-4 text-center">
            <p className="text-sm text-stone-700">No Chef Journal started yet.</p>
            <p className="text-xs text-stone-500 mt-1">Create your first journal to track growth over time.</p>
            <Link
              href="/settings/journal"
