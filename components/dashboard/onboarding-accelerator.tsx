...
import { ArrowRight, CheckCircle2, Circle, Clock, FileSpreadsheet, Sparkles, Upload } from 'lucide-react'
  ...
  const steps: Step[] = [
    {
      ...
      detail: 'Drop a CSV from Google Contacts, iPhone, or any spreadsheet — or paste a brain dump.',
    ...
    <Card className="border-brand-200 bg-brand-50/30">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-stone-700">
              Import your contacts and history first — then use ChefFlow for everything going forward.
            </p>
          </div>
          <span className="inline-flex w-fit items-center rounded-full border border-brand-200 bg-white px-3 py-1 text-xs font-semibold text-brand-700">
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-3">
          {steps.map((step) => (
            <div key={step.id} className="flex items-start gap-3 rounded-lg border border-stone-200 bg-white p-3">
              {step.done ? (
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-stone-600">{step.detail}</p>
              </div>
              {!step.done && (
                <Link href={step.href} className="text-xs font-medium text-brand-700 hover:text-brand-800 shrink-0">
                  Start
                </Link>
              )}
            </div>
