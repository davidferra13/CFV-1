...
export function PriceCascadePreview({ preview }: PriceCascadePreviewProps) {
  ...
  const totalDelta = preview.affectedRecipes.reduce(
    (sum, r) => sum + r.deltaCents,
    0
  )

  function handleApply() {
    startTransition(async () => {
      await cascadeIngredientPrice(
        preview.ingredientId,
        preview.newPriceCents,
      )
      setApplied(true)
    })
  }
    ...
    <Card>
      <CardHeader>
        ...
        <div className="mt-3 p-3 rounded-lg bg-stone-50 border border-stone-200">
          ...
          <p className="font-semibold text-stone-900 text-lg">{preview.ingredientName}</p>
          <div className="flex items-center gap-3 mt-2">
            <span className="text-sm text-stone-500">
              {formatMoney(preview.oldPriceCents)}
            </span>
            <ArrowRight className="h-4 w-4 text-stone-400" />
            <span className={`text-sm font-semibold ${priceIncreased ? 'text-red-600' : 'text-emerald-600'}`}>
              {formatMoney(preview.newPriceCents)}
            </span>
            <Badge variant={priceIncreased ? 'error' : 'success'}>
              {priceIncreased ? '+' : ''}{formatMoney(priceDelta)}
            </Badge>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="p-0">
        {preview.affectedRecipes.length === 0 ? (
          ...
          <>
            {/* Table header */}
            <div className="px-6 py-2 bg-stone-50 border-y border-stone-100 grid grid-cols-4 gap-4 text-xs font-medium text-stone-500 uppercase tracking-wide">
              <span>Recipe</span>
      ...
      <CardFooter className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-stone-600">
            Total impact across {preview.affectedRecipes.length} recipe{preview.affectedRecipes.length !== 1 ? 's' : ''}:
          </span>
          <span className={`text-sm font-bold ${totalDelta > 0 ? 'text-red-600' : totalDelta < 0 ? 'text-emerald-600' : 'text-stone-500'}`}>
