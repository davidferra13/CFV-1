    ...
    try {
      ...
      const marketplaceCityExpression = hasColumn('chef_marketplace_profiles', 'service_area_city')
        ...
        : hasColumn('chef_marketplace_profiles', 'city')
          ? 'mp.city'
          : 'null::text'
      const marketplaceStateExpression = hasColumn('chef_marketplace_profiles', 'service_area_state')
...
function sortCandidates(candidates: PreferredRegionAuditCandidate[]) {
  return [...candidates].sort((left, right) => {
    const classificationOrder = compareClassificationPriority(left.classification, right.classification)
...
function formatSignalSummary(candidate: PreferredRegionAuditCandidate) {
  ...
  const cities = candidate.signalCities.length > 0 ? candidate.signalCities.join(', ') : '(no city signal)'
  const areaSummary =
...
function buildEnvironmentBlockerMessage(
  error: unknown,
  options: PreferredRegionAuditCliOptions
) {
