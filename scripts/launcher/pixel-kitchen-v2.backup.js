...
;(function () {
  ...
  function drawOpenClawBrigade(bx, by, online) {
    ...
    const agents = [
      { name: 'main', color: C.crimson, role: 'Conductor', icon: '\u2605' },
      { name: 'build', color: C.lime, role: 'Builder', icon: '\u2713' },
      { name: 'qa', color: C.gold, role: 'QA', icon: '?' },
      { name: 'runner', color: C.salmon, role: 'Runner', icon: '>' },
    ]
    ...
    registerClickRegion(
      ...
      () => {
        tooltipData = {
          ...
          lines: [
            online ? 'Gateway: ONLINE (v2026.3.13)' : 'Gateway: OFFLINE',
            'All agents: ollama/llama3.2 (local GPU)',
            'main: Conductor (work orders, briefings)',
            'build: Builder (code changes)',
            'qa: QA Auditor (code patrol, reviews)',
            'runner: Task Runner (health, uptime)',
          ],
        }
      },
      'OpenClaw Brigade'
  ...
  function drawOpenClawCommandCenter(cx, cy, cw, ch) {
    ...
    // 4 agent rows - matches real OpenClaw brigade (main, build, qa, runner)
    const agents = [
      { name: 'main', role: 'Conductor', model: 'llama3.2', color: C.crimson, hat: 'toque' },
      { name: 'build', role: 'Builder', model: 'llama3.2', color: C.lime, hat: 'bandana' },
      { name: 'qa', role: 'QA Auditor', model: 'llama3.2', color: C.gold, hat: 'bandana' },
      { name: 'runner', role: 'Task Runner', model: 'llama3.2', color: C.salmon, hat: 'bandana' },
    ]
