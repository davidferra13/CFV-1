...
function changedFiles() {
  ...
  return Array.from(new Set([...tracked, ...untracked])).filter(
    (file) =>
...
function consolidatedHubForRoute(route) {
  for (const coverage of CONSOLIDATED_ROUTE_COVERAGE) {
...
function countRefs(routes, corpusFiles) {
  ...
  for (let i = 0; i < routes.length; i++) {
    ...
    const consolidatedHub = consolidatedHubForRoute(r.route)
    if (consolidatedHub && navRefs === 0) {
      for (const [filePath, content] of fileContents) {
        if (!filePath.startsWith(NAV_DIR)) continue
        if (!content.includes(consolidatedHub)) continue
        const rel = relative(PROJECT_ROOT, filePath).replace(/\\/g, '/')
        refFiles.push(`${rel} (consolidated via ${consolidatedHub})`)
        navRefs++
        break
      }
    }
...
const output = {
  ...
  weak: weak.map((r) => ({
    ...
    dynamic: r.dynamic,
    refs: r.refs,
    ref_files: r.ref_files,
  })),
  all_routes: results,
}

writeFileSync(
  join(PROJECT_ROOT, 'scripts', 'wiring-audit-results.json'),
