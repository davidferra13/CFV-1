...
export function auditRuntimeInvariants(rootDir = process.cwd()): RuntimeInvariantResult[] {
  return RUNTIME_INVARIANTS.map((spec) => {
    ...
    const forbiddenPatterns = (spec.absentPatterns ?? []).filter((pattern) =>
...
export async function runDbContractAudit(
  options: AuditOptions = {}
): Promise<DbContractAuditReport> {
  ...
  if (migrationDirectory.invalidFiles.length > 0) {
    addFinding(findings, {
      ...
      paths: migrationDirectory.invalidFiles.map((file) =>
        path.posix.join('database/migrations', file)
      ),
      severity: 'error',
    })
  }
