...
import { COMMAND_GROUPS, COMMANDS } from './cheflow/command-metadata.mjs'
...
function validationSelector(args = {}) {
  ...
  if (files.some((file) => /\.(ts|tsx|js|jsx|mjs)$/.test(file))) {
    ...
    for (const file of files.filter((item) => item.startsWith('scripts/cheflow/') && item.endsWith('.mjs'))) {
      add(`node --check ${file}`, 'CLI metadata or support module changed.')
    }
...
function main(argv = process.argv.slice(2)) {
  ...
  switch (command) {
    ...
    case 'closeout': {
      ...
      return result.executed && result.ok === false ? 1 : 0
    }
    case 'db': {
