...
function resolveEventDate(fixture: GoldenCaseFixture): string {
  ...
  if (fixture.summary.event_date_iso && /^\d{4}-\d{2}-\d{2}$/.test(fixture.summary.event_date_iso)) {
...
async function ensureClient(admin: any, tenantId: string, fixture: GoldenCaseFixture): Promise<string> {
  const clientFields = {
    tenant_id: tenantId,
...
async function ensureMenu(
  ...
  if (existing?.id) {
    ...
    await admin.from('events').update({ menu_id: existing.id, updated_at: nowIso() }).eq('id', eventId)
    return existing.id as string
  }
...
async function ensureDish(
  ...
  if (existingError) {
    throw new Error(`Failed to look up golden-case dish ${input.courseName}: ${existingError.message}`)
  }
  ...
  if (existing?.id) {
    ...
    if (error) throw new Error(`Failed to update golden-case dish ${input.courseName}: ${error.message}`)
    return existing.id as string
  }
...
async function maybeTriggerScheduledRefresh(eventDate: string): Promise<void> {
  ...
  const daysUntilEvent =
    (new Date(`${eventDate}T00:00:00.000Z`).getTime() - new Date(`${today}T00:00:00.000Z`).getTime()) /
    (24 * 60 * 60 * 1000)
