#!/usr/bin/env bash
# Migrate from Supabase Docker PostgreSQL to standalone PostgreSQL
# This script:
#   1. Dumps the current Supabase database
#   2. Stops Supabase Docker containers
...
DUMP_FILE="backup-supabase-final-$(date +%Y%m%d%H%M%S).sql"
SUPABASE_CONTAINER="supabase_db_CFv1"
STANDALONE_CONTAINER="chefflow_postgres"

echo "=== Step 1: Dump Supabase PostgreSQL ==="
docker exec -i "$SUPABASE_CONTAINER" pg_dump -U postgres -d postgres --no-owner --no-acl > "$DUMP_FILE"
echo "Dump saved to: $DUMP_FILE ($(du -h "$DUMP_FILE" | cut -f1))"

echo ""
echo "=== Step 2: Stop Supabase Docker ==="
npx supabase stop
...
echo ""
echo "You can now remove supabase/ config if you no longer need it."
echo "To revert: bash scripts/migrate-to-standalone-pg.sh (just re-run supabase start)"
