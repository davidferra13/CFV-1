...
```
    ...
    | 4.6     |   | Groq 70B  |   | Groq   |
    | $3/$15  |   | Llama 3.3 |   | 70B    |
    | complex |   | $0 (free) |   |$0(free)|
    | escalate|   | validator |   | testing|
    +---------+   +-----------+   +--------+
                       |
                  +---------+
                  | RUNNER  |
...
> Full hierarchy documentation: `docs/OPENCLAW-HIERARCHY.md`
...
| **Sonnet** | Claude Sonnet 4.6  | $3/$15 per M tokens  | Feature implementation, complex refactors | The primary code writer. Handles 80%+ of actual implementation. Multi-file changes, new components, server actions, bug fixes. Reports back to Main for review. Does NOT make architectural decisions.      |
| **Build**  | Groq Llama 3.3 70B | $0 (free cloud)      | Build validation only                     | Runs tsc and next build. Reports PASS or FAIL with error details. Does NOT write code, fix errors, or make decisions. Read-only validator.                                                                  |
| **QA**     | Groq Llama 3.3 70B | $0 (free cloud)      | Testing, verification, review             | Runs test suites. Checks regressions. Reviews diffs for bugs. Reports findings. Does NOT fix bugs or write code.                                                                                            |
| **Runner** | Groq Llama 3.1 8B  | $0 (free cloud)      | Simple, mechanical tasks                  | File listings, git status, reading configs, running scripts, file operations. Any agent can delegate to runner. Zero reasoning needed, just execution.                                                      |
...
| Write a new React component                    | Sonnet                           | Standard implementation            |
| Write a server action                          | Sonnet                           | Standard implementation            |
| Write a database migration                     | Main (writes directly)           | Security-sensitive, Main handles   |
| Fix a type error                               | Sonnet                           | Code change required               |
| Fix a complex cross-file bug                   | Sonnet                           | Requires multi-file understanding  |
| Run tsc, next build                            | Build                            | Build validation only              |
| Review Sonnet's code                           | QA first, then Main if QA passes | Two-stage review                   |
...
| Security-sensitive changes (auth, RLS, ledger) | Main (writes directly)           | High-stakes, Main handles          |
| Refactor across 10+ files                      | Sonnet                           | Complex, many interdependencies    |
| Write a simple utility function                | Sonnet                           | Standard implementation            |
| Update PROGRESS.md                             | Runner                           | Mechanical                         |
| Research existing codebase pattern             | QA                               | Read and report, no changes        |
| List files, read configs, git status           | Runner                           | Grunt work, any agent can delegate |
