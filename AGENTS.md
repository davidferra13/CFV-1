...
- `show-me`: Open and inspect the live ChefFlow website when David says "show me", "show me the site", "show me what is built", "open the website", or asks to see the current app. The first action is always opening the website with browser or Playwright tools, then reporting screenshots and visible UI state, without changing code, starting servers, running builds, deploying, or mutating data unless explicitly approved. File: `.claude/skills/show-me/SKILL.md`
