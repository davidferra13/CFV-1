...
const JOY_INQUIRIES = {
  awaitingChef: {
    ...
    confirmed_service_expectations:
      'Relaxed plated dinner with a welcome bite and one interactive course',
  },
  awaitingClient: {
    sourceMessage:
...
const JOY_EVENTS = {
  draft: {
    ...
    client_journey_note:
      'Joy is comparing two date options and wants a confident menu recommendation.',
  ...
  paid: {
    ...
    client_journey_note:
      'Joy paid quickly once the team menu balanced pescatarian and dairy-light options.',
  ...
  confirmed: {
    ...
    kitchen_notes:
      'Roof deck power is available, but wind picks up after sunset; keep hot courses plated indoors.',
    site_notes: 'Use the side entrance by the garden gate and bring one portable induction burner.',
    client_journey_note:
      'Joy approved the overall shape of the night and is now reviewing final menu details.',
    ambiance_notes:
      'Sunset rooftop dinner with bright citrus, Champagne, and a relaxed but elevated progression.',
  },
  completed: {
    ...
    kitchen_notes:
      'Leave one shelf open in the fridge for labeled leftovers and dessert components.',
    site_notes: 'Kitchen reset should include compost separation and glass storage containers.',
    client_journey_note:
      'This dinner cemented Joy as a repeat client with strong referral potential.',
...
const JOY_FUN_QA = {
  ...
  last_meal:
    'Oysters, lemon pasta, grilled fish, olive oil cake, and a very cold glass of Champagne.',
  trash_food: 'McDonald’s fries eaten in the car before anyone notices.',
  sweet_or_savory:
    'Savory wins almost every time, but a warm salted chocolate chip cookie still gets me.',
  prove_yourself:
    'A flawless crudo followed by something comforting with browned butter and herbs.',
  ...
  dream_menu_theme:
    'Seafood night with bright citrus, shaved fennel, and a big grilled centerpiece.',
...
async function ensureAuthUser(
  ...
  const { error: updateError } = await runAdminAuthOperation(
    `update auth user ${input.email}`,
    () =>
...
async function upsertPrimaryClient(
  ...
  const fields = {
    ...
    parking_instructions:
      'Street parking is easiest on Worcester Square after 6 PM. Text when you are 10 minutes out.',
    access_instructions:
      'Use the side entrance by the garden gate. Buzz 2R if the front door is locked.',
    kitchen_size: 'Medium galley kitchen',
    kitchen_constraints:
      'Single oven runs hot by about 15 degrees and counter space is tight near the stove.',
    house_rules:
    ...
    farewell_style:
      'Likes a quick kitchen recap and a short next-morning text with leftover notes.',
    ...
    kitchen_burner_notes:
      'Front-right burner is strongest; avoid crowding the back-left simmer burner.',
    ...
    leftovers_preference:
      'Portion leftovers into glass containers and leave one host plate for next-day lunch.',
    has_dishwasher: true,
    ...
    water_quality_notes:
      'Filtered tap water is fine for cooking; sparkling water is stocked in the pantry.',
    available_place_settings: 12,
    formality_level: 'semi_formal',
    communication_style_notes:
      'Prefers concise texts during the day and one thorough email for menu revisions.',
...
async function upsertMessage(
  ...
  query = payload.subject == null ? query.is('subject', null) : query.eq('subject', payload.subject)
  query =
...
async function upsertClientSatisfactionSurvey(
  ...
  if (error || !inserted) {
    throw new Error(`[e2e-seed] Failed to insert client satisfaction survey: ${error?.message}`)
  }
...
export async function seedE2EData(): Promise<SeedResult> {
  ...
  await safeUpdateById(
    ...
    proposedEventId,
    {
      ...
      service_style: JOY_EVENTS.proposed.service_style,
      access_instructions:
        'Side entrance preferred. Dessert plates are in the lower dining credenza.',
      dietary_restrictions: ['light dairy'],
      allergies: ['sesame'],
      special_requests: JOY_EVENTS.proposed.special_requests,
      quoted_price_cents: JOY_EVENTS.proposed.quoted_price_cents,
  ...
  await safeUpdateById(
    ...
    confirmedEventId,
    {
      ...
      service_style: JOY_EVENTS.confirmed.service_style,
      access_instructions:
        'Enter through the garden gate and stage hot courses inside before moving upstairs.',
      dietary_restrictions: ['pescatarian', 'light dairy'],
      allergies: ['sesame'],
      special_requests: JOY_EVENTS.confirmed.special_requests,
      quoted_price_cents: JOY_EVENTS.confirmed.quoted_price_cents,
...
The client must first accept the proposal pricing and event details before signing this agreement.`
  ...
  await upsertMessage(admin, chefId, primaryClientId, {
    ...
    body: 'I recommend a bright coastal menu with one vegetarian plate, a small interactive crudo moment, and a printed menu card at each place setting.',
  ...
  await upsertMessage(admin, chefId, primaryClientId, {
    ...
    body: 'Attached is the revised proposal at $1,850 with plated dessert service and a lighter seafood main. I included two dessert formats so you can choose the pacing you want.',
    sent_at: isoFromNow(-3, 15, 10),
  ...
  await upsertMessage(admin, chefId, primaryClientId, {
    ...
    to_user_id: clientAuth.id,
    subject: null,
    body: 'Menu preview is live in your portal. The crudo finish and olive oil cake are both in there - let me know if you want the semifreddo swap.',
  ...
  await upsertMessage(admin, chefId, primaryClientId, {
    ...
    to_user_id: chefAuth.id,
    subject: null,
    body: 'Approved overall. Keep dessert plated individually and please leave one host portion for the next day.',
  ...
  await upsertMessage(admin, chefId, primaryClientId, {
    ...
    to_user_id: clientAuth.id,
    subject: null,
    body: 'Everything is reset, leftovers are labeled on the top fridge shelf, and the olive oil cake is boxed separately for tomorrow.',
  ...
  await upsertServedDishHistory(admin, chefId, primaryClientId, {
    ...
    client_reaction: 'loved',
    notes:
      'The tableside finish landed exactly the way Joy likes: quick, polished, and interactive.',
