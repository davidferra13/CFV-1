...
const BASE = process.env.BASE_URL || 'https://beta.cheflowhq.com'
console.log(`Using server: ${BASE}`)

// Go to /dashboard first so redirect target is /dashboard (not /sign-in which loops)
console.log('Navigating to dashboard (will redirect to sign-in)...')
...
console.log('Submitted credentials, waiting for workspace to open...')

// Wait for redirect to dashboard
for (let i = 0; i < 30; i++) {
  await page.waitForTimeout(2000)
  const url = page.url()
  if (url.includes('/dashboard') && !url.includes('auth') && !url.includes('signin')) {
    console.log(`Authenticated after ${(i + 1) * 2}s! URL: ${url}`)
    break
  }
  if (i === 29) console.log('Timed out waiting for redirect, continuing anyway...')
}
...
const pages = [
  ...
  { name: '027-events-new-wizard', url: '/events/new/wizard' },
...
for (const p of pages) {
  try {
    ...
    fail++
    console.log(`FAIL: ${p.name} - ${err.message.slice(0, 100)}`)
    try { await page.screenshot({ path: `${DIR}/${p.name}-error.png` }) } catch {}
  }
}

await browser.close()
console.log(`\nDone! ${ok} succeeded, ${fail} failed out of ${pages.length} pages.`)
