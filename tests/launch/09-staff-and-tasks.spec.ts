...
async function authenticateChef(page: Page) {
  ...
  expect(response.ok(), `E2E auth failed: ${response.status()} ${await response.text()}`).toBeTruthy()
}

async function openTaskForm(page: Page, selectedDate: string) {
...
test.describe('Task Board', () => {
  ...
  test('task create flow persists after refresh', async ({ page }) => {
    test.setTimeout(240_000)

    const selectedDate = '2030-01-15'
    const title = `Launch task ${Date.now()}`

    await authenticateChef(page)
    await openTaskForm(page, selectedDate)
  ...
  test('task create failure stays honest', async ({ page }) => {
    test.setTimeout(240_000)

    const selectedDate = '2030-01-16'
    const title = `Launch task failure ${Date.now()}`
