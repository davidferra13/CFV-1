...
async function gotoEventDetail(page: Parameters<Parameters<typeof test>[1]>[0]['page'], path: string) {
  try {
...
test.describe('Event Workflow Contract', () => {
  test('completed event exposes debrief, follow-up, and closure surfaces', async ({
    ...
    await expect(
      trustLoop.getByRole('heading', { name: /capture satisfaction/i })
    ).toBeVisible()
    await expect(
      trustLoop.getByRole('heading', { name: /request a public review/i })
    ).toBeVisible()
    ...
    await expect(debriefLink).toHaveAttribute('href', `/events/${seedIds.eventIds.completed}/debrief`)
