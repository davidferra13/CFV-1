...
test.describe('Inquiries', () => {
  ...
  test('TEST awaiting-chef inquiry appears', async ({ page }) => {
    ...
    await expect(page.getByText(/TEST E2E inquiry awaiting chef/i).first()).toBeVisible({
      timeout: 10_000,
    })
  })
  ...
  test('TEST awaiting-client inquiry appears', async ({ page }) => {
    await page.goto(ROUTES.inquiries)
    await page.waitForLoadState('networkidle')
    await expect(page.getByText(/TEST E2E inquiry awaiting client/i).first()).toBeVisible({
      timeout: 10_000,
    })
  })
  ...
  test('clicking inquiry navigates to detail', async ({ page, seedIds }) => {
    ...
    const link = page
      .getByRole('link')
      .filter({ hasText: /TEST E2E inquiry awaiting chef/i })
      .first()
    await link.click()
  ...
  test('inquiry list is filterable by status', async ({ page }) => {
    ...
    const tabOrFilter = page
      .getByRole('tab')
      .or(page.getByRole('button').filter({ hasText: /all|awaiting/i }))
