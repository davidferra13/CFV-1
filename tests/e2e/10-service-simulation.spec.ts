...
async function createNoMenuEvent(admin: ReturnType<typeof createAdminClient>, seedIds: any) {
  ...
  if (error || !data)
    throw new Error(`[service-simulation] Failed to create no-menu event: ${error?.message}`)
...
async function createRichEvent(
  ...
  if (componentError) {
    throw new Error(
      `[service-simulation] Failed to create rich components: ${componentError.message}`
    )
  }
  ...
  if (prepBlockError) {
    throw new Error(
      `[service-simulation] Failed to create rich prep block: ${prepBlockError.message}`
    )
  }
  ...
  if (options?.includeTravelLeg !== false) {
    ...
    if (travelError) {
      throw new Error(
        `[service-simulation] Failed to create rich travel leg: ${travelError.message}`
      )
    }
  }
...
test.describe('Service Simulation', () => {
  ...
  test('soft-gates unsimulated and stale transitions, then clears stale state on re-simulate', async ({
    ...
    if (updateError) {
      throw new Error(
        `[service-simulation] Failed to update event for stale test: ${updateError.message}`
      )
    }
