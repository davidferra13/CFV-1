...
test('recordPayment blocks when POS role matrix is enabled and actor lacks cashier access', async () => {
    ...
    await assert.rejects(
      async () =>
        actions.recordPayment({
          ...
          amountCents: 500,
          paymentMethod: 'cash',
          idempotencyKey: 'payment-role-gate',
        }),
      /Cashier role required/i
    )

    const paymentInsert = tracker.queries.find(
...
test('updatePaymentStatus blocks when POS role matrix is enabled and actor lacks lead access', async () => {
  const tracker: Tracker = { queries: [] }
  const previousRoleMatrix = process.env.POS_ENFORCE_ROLE_MATRIX
  process.env.POS_ENFORCE_ROLE_MATRIX = 'true'
  ...
  try {
    await assert.rejects(
      async () => actions.updatePaymentStatus('payment-1', 'settled'),
      /Lead role required/i
    )

    const paymentUpdate = tracker.queries.find(
      (query) => query.table === 'commerce_payments' && query.action === 'update'
    )
    assert.equal(paymentUpdate, undefined)
  } finally {
    if (previousRoleMatrix == null) {
      delete process.env.POS_ENFORCE_ROLE_MATRIX
    } else {
