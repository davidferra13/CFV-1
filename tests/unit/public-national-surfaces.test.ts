...
function flattenMetadataText(value: unknown): string {
  ...
  return Object.values(value as Record<string, unknown>).map(flattenMetadataText).join(' ')
}
...
test('nearby OG snapshots stay neutral on landing surfaces', () => {
  ...
  assert.doesNotMatch(
    [staticSnapshot.title, staticSnapshot.subtitle, staticSnapshot.detail, ...staticSnapshot.entries].join(' '),
    CITY_EXAMPLE_PATTERN
  )
  assert.doesNotMatch(
    [dynamicSnapshot.title, dynamicSnapshot.subtitle, dynamicSnapshot.detail, ...dynamicSnapshot.entries].join(' '),
    CITY_EXAMPLE_PATTERN
  )
})
