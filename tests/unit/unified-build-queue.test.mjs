...
test('unified queue normalizes sources, dedupes, and groups module batches', () => {
  ...
  assert.ok(result.candidates.every((candidate) => candidate.assignment))
  assert.ok(result.candidates.every((candidate) => candidate.module?.id))
  assert.ok(result.candidates.some((candidate) => candidate.submodule?.id === 'quote-safety'))
  ...
  assert.ok(result.moduleBatches.every((batch) => batch.submodule?.id))
  assert.ok(result.summary.byAssignment.proposed > 0)
})
