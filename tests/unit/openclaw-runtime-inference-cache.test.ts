  ...
  async (t) => {
    await withRuntimeDb(t, async (mod) => {
      ...
      const seeded = mod
        ...
        .find((slice: { sliceKey: string }) => slice.sliceKey === 'explicit-inference-cache-audit-plane')
...
test(
  ...
  async (t) => {
    await withRuntimeDb(t, async (mod) => {
      ...
      const currentPriceCount = db.prepare('SELECT COUNT(*) as count FROM current_prices').get() as {
        count: number
      }
...
test(
  ...
  async (t) => {
    await withRuntimeDb(t, async (mod) => {
      ...
      const expiredRows = mod.listPriceInferenceCacheEntries(db, { state: 'expired' }, { now: '2026-04-21T03:00:00.000Z' })
      const invalidatedRows = mod.listPriceInferenceCacheEntries(db, { state: 'invalidated' }, { now: '2026-04-21T03:00:00.000Z' })

      assert.equal(activeRows.length, 0, 'default inference reads should exclude expired and invalidated rows')
      ...
      const remaining = mod.listPriceInferenceCacheEntries(db, { state: 'all' }, { now: '2026-04-21T03:00:00.000Z' })
