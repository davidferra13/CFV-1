...
function getPhase(context: ServiceSimulationContext, key: ServiceSimulationPhaseKey) {
...
describe('service simulation engine', () => {
  ...
  it('treats placeholder location values as unknown instead of venue truth', () => {
    ...
    assert.equal(
      coreFacts.missingItems.some((item) => item.id === 'core-location'),
      true
    )
  ...
  it('builds hard and soft transition gates from deterministic simulation state', () => {
    ...
    const softGate = getServiceSimulationTransitionGate(
      buildPanelState(softContext, 'unsimulated'),
      'confirmed'
    )

    assert.equal(softGate.status, 'soft')
    assert.equal(
      softGate.reasons.some((reason) => reason.code === 'unsimulated'),
      true
    )

    const hardContext = makeConfirmedContext()
    hardContext.travel.serviceLegCount = 0
    const hardGate = getServiceSimulationTransitionGate(
      buildPanelState(hardContext, 'current'),
      'in_progress'
    )

    assert.equal(hardGate.status, 'hard')
    assert.equal(
      hardGate.reasons.some((reason) => reason.code === 'critical'),
      true
    )
  })
})
