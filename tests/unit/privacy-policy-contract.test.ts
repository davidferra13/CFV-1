...
const privacyPageSource = readFileSync(join(ROOT, 'app', '(public)', 'privacy', 'page.tsx'), 'utf-8')
...
test('privacy baseline stays explicit about no sale and no ad-tech reuse', () => {
  ...
  assert.ok(
    NO_DATA_SALE_BUILD_GUARDRAILS.some((rule) => /ad networks|remarketing|data brokers/i.test(rule)),
...
test('core processor inventory covers the runtime processors used by ChefFlow', () => {
  ...
  assert.deepStrictEqual(
    Array.from(coreIds).sort(),
    ['cloudflare', 'google-maps', 'posthog', 'resend', 'sentry', 'stripe']
  )
})
