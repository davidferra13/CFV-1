...
test('buildOpenClawWarnings highlights competing truths and failed steps', () => {
  ...
  assert.equal(warnings.some((warning) => warning.includes('downstream OpenClaw data still appears fresh')), true)
  assert.equal(warnings.some((warning) => warning.includes('local OpenClaw mirror')), true)
  assert.equal(warnings.some((warning) => warning.includes('Pull catalog from Pi')), true)
})
