...
describe('chef decision engine', () => {
  ...
  it('marks execution not ready when safety, ingredient coverage, and equipment all fail', () => {
    ...
    assert.equal(
      result.riskFlags.some((flag) => flag.code === 'dietary_conflict' && flag.severity === 'critical'),
      true
    )
    assert.equal(
      result.riskFlags.some((flag) => flag.code === 'equipment_gap' && flag.severity === 'critical'),
      true
    )
