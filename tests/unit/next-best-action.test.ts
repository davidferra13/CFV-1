...
describe('next best action', () => {
  ...
  it('uses signal precedence and carries explainability into the chosen action', () => {
    ...
    assert.equal(action?.description, 'The client viewed the quote, but no later client response is recorded.')
  ...
  it('prefers a graph-backed prepared quote revision over signal-only follow-up work', () => {
    ...
    assert.equal(action?.href, '/quotes/new?source=quote_revision&quote_name=Birthday+Dinner+Quote+Revised')
