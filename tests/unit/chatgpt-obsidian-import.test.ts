...
test('imports an unpacked ChatGPT export folder into Obsidian markdown notes', async () => {
  ...
  const noteFiles = fs.readdirSync(namespaceDir).filter((entry) => entry.endsWith('.md') && entry !== '_INDEX.md')
  ...
  assert.doesNotMatch(noteText, /Alternative branch answer that should stay out of the visible transcript/)
