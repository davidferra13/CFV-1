    ...
    await withRuntimeDb(t, async (mod) => {
      ...
      const seeded = slices.find(
        (slice: { sliceKey: string }) =>
          slice.sliceKey === 'durable-runtime-task-incident-dedupe-spine'
      )
      ...
      assert.ok(
        seeded.kpis.some(
          (kpi: { metricName: string }) => kpi.metricName === 'active_task_duplication_rate'
        )
      )
    ...
    await withRuntimeDb(t, async (mod) => {
      ...
      const first = mod.upsertSourceIncident(
        db,
        {
          ...
          evidence: [{ truthLabel: 'observed', httpStatus: 429 }, { retryAfterSeconds: 60 }],
        },
      ...
      const resolved = mod.resolveSourceIncident(db, openIncidents[0].incidentId, {
      ...
      assert.equal(resolved.evidence[resolved.evidence.length - 1]?.truthLabel, 'derived')
