...
test('client route surfaces and Remy stay wired to the shared work graph', () => {
  ...
  const contractSigningClient = read('app/(client)/my-events/[id]/contract/contract-signing-client.tsx')
  ...
  assert.match(dashboardActions, /actionRequired:\s*\{\s*proposalCount: workGraph\.summary\.proposalCount/s)
