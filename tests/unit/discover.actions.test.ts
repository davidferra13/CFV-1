...
class DirectoryListingsBuilder {
  ...
  async limit(count: number) {
    ...
    if (this.selectColumns === 'id, name, slug, state, status, lead_score') {
      ...
      return { data: hasNameFilter ? this.scenario.exactMatches ?? [] : this.scenario.looseMatches ?? [] }
    }
  ...
  async single() {
    if (this.mode === 'insert' && this.selectColumns === 'id, slug') {
      return { data: this.scenario.insertedRecord ?? null, error: this.scenario.insertError ?? null }
    }
...
function loadDiscoverActions({
  ...
  require.cache[GEO_PATH] = {
    ...
    loaded: true,
    exports: {
      resolvePublicLocationQuery: async () => ({ data: null, error: 'Geocoding service unavailable' }),
    },
  } as NodeJS.Module
