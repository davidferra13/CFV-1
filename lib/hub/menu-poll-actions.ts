...
async function resolveTenantActorUserId(input: { db: any; tenantId: string }): Promise<string | null> {
...
async function loadDishIndexOptions(input: {
  ...
  const { data: dishes, error } = await db
    .from('dish_index')
    .select(
      'id, name, course, description, dietary_tags, allergen_flags, linked_recipe_id'
    )
    .in('id', dishIndexIds)
    .eq('tenant_id', tenantId)
...
async function resolveManagerForEvent(input: {
  db: any
  eventId: string
  profileToken: string
}) {
...
export async function createDinnerCircleMenuPollIteration(
  ...
  for (const course of validated.courses) {
    ...
    if (pollError || !poll) {
      throw new Error(`Failed to create Dinner Circle poll: ${pollError?.message ?? 'unknown error'}`)
    }
...
export async function lockDinnerCircleMenuSelections(
  ...
  for (const course of state.courses) {
    ...
    const optionId =
      chosen?.optionId ??
      buildDefaultLockSelections(course.pollType, course.options)

    if (!optionId) {
      throw new Error(`No canonical dish option is available for ${course.courseName ?? 'this course'}`)
    }

    const option = course.options.find((candidate) => candidate.id === optionId)
    if (!option || option.optionType !== 'standard' || !option.dishIndexId) {
      throw new Error(`Finalized selection for ${course.courseName ?? 'this course'} must map to a canonical dish`)
    }
