...
export type ChefLocationRelationshipType =
  (typeof CHEF_LOCATION_RELATIONSHIP_OPTIONS)[number]
...
export function normalizeRelationshipType(
  ...
  return isAllowedValue(normalized, CHEF_LOCATION_RELATIONSHIP_OPTIONS)
    ? normalized
    : 'preferred'
}
...
export function evaluatePublicLocationExperienceReadiness(
  ...
  const hasLocation =
    hasNonEmptyText(input.address) ||
    (hasNonEmptyText(input.city) && hasNonEmptyText(input.state))
  const hasImages =
    Array.isArray(input.images) &&
    input.images.some((image) => hasNonEmptyText(image.image_url))
