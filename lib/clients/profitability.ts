...
export async function getClientProfitabilityHistory(clientId: string): Promise<ClientProfitabilityHistory> {
  ...
  const { data: events } = await supabase
    .from('events')
    .select('id, occasion, event_date, time_shopping_minutes, time_prep_minutes, time_travel_minutes, time_service_minutes, time_reset_minutes')
  ...
  const avgHourlyRateCents = withTime.length > 0
    ? Math.round(
        withTime.reduce((s, e) => {
  ...
  if (enriched.length >= 4) {
    ...
    const secondHalfAvg = enriched.slice(mid).reduce((s, e) => s + e.marginPercent, 0) / (enriched.length - mid)
