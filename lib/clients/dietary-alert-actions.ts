...
export async function getDietaryAlerts(
  unacknowledgedOnly = false
): Promise<DietaryAlert[]> {
...
export async function getDietaryTrends(): Promise<{
  ...
  const commonRestrictions: DietaryTrend[] = Array.from(
    restrictionMap.entries()
  )
    .map(([value, stats]) => ({
      value,
      count: stats.count,
...
export async function getClientDietaryTimeline(
  clientId: string
): Promise<DietaryAlert[]> {
  const user = await requireChef()
  const supabase = createServerClient()
...
export async function getAlertStats(): Promise<AlertStats> {
  ...
  return {
    ...
    unacknowledged: rows.filter(
      (r: Record<string, unknown>) => !r.acknowledged
    ).length,
    critical: rows.filter(
      (r: Record<string, unknown>) => r.severity === 'critical'
    ).length,
    warning: rows.filter(
      (r: Record<string, unknown>) => r.severity === 'warning'
    ).length,
    info: rows.filter(
      (r: Record<string, unknown>) => r.severity === 'info'
    ).length,
  }
}
