...
import { getClientHealthScores, type ClientHealthScore, type ClientHealthTier } from './health-score'
...
export function getClientInteractionSignalShortLabel(signalType: ClientInteractionSignalType): string {
...
function buildQuoteSignals(
  ...
  const quoteViewedWithoutResponse =
    quoteViewedCandidates.sort((left, right) => parseTimestamp(right.occurredAt) - parseTimestamp(left.occurredAt))[0] ??
    null
...
export async function getClientInteractionSignalMap(
  ...
  const ledgers = await Promise.all(
    uniqueClientIds.map(async (clientId) => [
      clientId,
      await getClientInteractionLedger(clientId),
    ] as const)
  )
