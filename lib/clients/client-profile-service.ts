...
function inferPreferenceCategory(
  rawType: string | null | undefined
): ProfilePreference['category'] {
...
function mapServiceDepth(raw: string | null | undefined): ClientServiceDepth | null {
  ...
  if (
    value.includes('tasting') ||
    value.includes('multi course') ||
    value.includes('multicourse')
  ) {
...
function buildAffectiveContext(
  ...
  const scoreByState = new Map<
    ClientEmotionalState,
    { count: number; refs: ProfileEvidenceRef[] }
  >()
...
function buildServiceDepth(
  ...
  const best =
    ...
    (['semi_formal_shared', { score: 0.3, refs: [] }] as const)
...
function buildHouseholdAlignment(
  ...
  const primaryDislikes = uniqueStrings(bundle.client.dislikes ?? [])
  ...
  for (const linked of bundle.linkedClients) {
    members.push({
      ...
      role: 'linked_client',
      relationshipLabel:
        bundle.householdMembers.find((member) => member.client_id === linked.id)?.relationship ??
        null,
      likes: uniqueStrings([
        ...(linked.favorite_cuisines ?? []),
        ...(linked.favorite_dishes ?? []),
      ]),
  ...
  for (const managed of bundle.managedSubjects) {
    ...
    const dislikesFromPayload = uniqueStrings(
      ((payload.dislikes as string[] | undefined) ?? []) as string[]
    )
  ...
  const consensusLikes = uniqueStrings(
    members.flatMap((member) =>
      member.likes.filter(
        (like) =>
          members.filter((candidate) => candidate.likes.some((value) => overlapsValue(value, like)))
            .length > 1
      )
    )
  )
  ...
  const tensions = uniqueStrings(
    members.flatMap((member) =>
      member.likes.filter((like) =>
        members.some(
          (other) =>
            other.subjectKey !== member.subjectKey &&
            other.vetoes.some((veto) => overlapsValue(veto, like))
        )
      )
    )
  )
  ...
  const strategy =
    consensusVetoes.length > 0
      ? 'block_on_any_hard_veto'
      : members.length > 1
        ? 'maximize_overlap'
        : 'single_profile'
      ...
      queries: [
        {
          ...
          status: 'pending',
          question:
            'Can the menu use non-chili heat, or should all spicy preparations be excluded?',
        ...
        queries: [
          {
            ...
            reasoning:
              'Different members of the same household currently point in opposite directions.',
            options: [
...
function buildCoverageSummary(vector: Omit<CulinaryProfileVector, 'coverage'>) {
  ...
  const signalDensity = clamp01(
    (vector.statedLikes.length + vector.hardVetoes.length + vector.historicalConsumption.length) /
      18
  )
  const overallConfidence = clamp01(
    completenessScore * 0.35 + recencyScore * 0.25 + consistencyScore * 0.2 + signalDensity * 0.2
  )
...
function collectVectorEvidenceRefs(
  vector: Omit<CulinaryProfileVector, 'coverage'> | CulinaryProfileVector
) {
...
export function buildCulinaryProfileVector(
  bundle: ClientProfileSourceBundle
...
async function persistConflicts(
  ...
  if (queryResult?.error && !isMissingRelationError(queryResult.error)) {
    console.error(
      '[client-profile-service] Failed to persist arbitration queries:',
      queryResult.error
    )
  }
}
...
async function persistVector(
  ...
  if (previousUpdate?.error && !isMissingRelationError(previousUpdate.error)) {
    console.error(
      '[client-profile-service] Failed to clear previous vectors:',
      previousUpdate.error
    )
  }
  ...
  if (evidenceRows.length > 0) {
    ...
    if (evidenceResult?.error && !isMissingRelationError(evidenceResult.error)) {
      console.error(
        '[client-profile-service] Failed to persist evidence ledger:',
...
export function recommendCandidateMealsAgainstVector(
  ...
  for (const candidate of parsedRequest.candidates) {
    ...
    switch (currentEmotion) {
      ...
      case 'relaxed': {
        const delta =
          0.07 * candidate.comfortScore + 0.05 * (1 - Math.abs(candidate.complexityScore - 0.5))
        score += delta
        justifications.push(
          ConfidenceJustificationSchema.parse({
    ...
    if (
      vector.householdAlignment?.tensions.some((tension) =>
        texts.some((text) => overlapsValue(text, tension))
      )
    ) {
      score -= 0.16
      justifications.push(
...
async function persistRecommendation(
  ...
  if (insertResult?.error) {
    if (!isMissingRelationError(insertResult.error)) {
      console.error(
        '[client-profile-service] Failed to persist recommendation:',
        insertResult.error
      )
    }
    return result
