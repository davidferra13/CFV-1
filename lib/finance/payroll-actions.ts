...
export async function updateEmployee(
  ...
  await (supabase as any).from('employees').update(patch).eq('id', id).eq('chef_id', user.entityId)
}
...
export async function recordPayroll(input: {
  ...
  await (supabase as any).from('payroll_records').insert({
    chef_id: user.entityId,
...
export async function save941Summary(taxYear: number, quarter: number): Promise<void> {
  ...
  await (supabase as any).from('payroll_941_summaries').upsert(
    {
      chef_id: user.entityId,
      tax_year: taxYear,
      quarter,
...
export async function generateW2Summaries(taxYear: number): Promise<void> {
  ...
  for (const emp of employees || []) {
    ...
    await (supabase as any).from('payroll_w2_summaries').upsert(
      {
        chef_id: user.entityId,
        employee_id: emp.id,
        tax_year: taxYear,
