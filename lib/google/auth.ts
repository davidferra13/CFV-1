...
import { getGoogleGmailControl, upsertGoogleMailboxFromLegacyConnection } from './mailbox-control'
import {
...
export async function disconnectGoogle(service: 'gmail' | 'calendar') {
  ...
  const otherServiceConnected =
    service === 'gmail'
      ...
      : control.mailbox?.gmailConnected || conn.gmail_connected
