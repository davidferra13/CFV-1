...
export type PublicSeasonalMarketPulseIntent = z.infer<
  typeof PublicSeasonalMarketPulseIntentSchema
>
...
function pickLeadSpotlight(
  ...
  for (const preferredName of normalizedPreferredNames) {
    const match = spotlights.find((spotlight) => spotlight.name.trim().toLowerCase() === preferredName)
