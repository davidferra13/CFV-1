 ...
 */
export async function markContainerReturned(containerId: string): Promise<Container | { error: string }> {
  const user = await requirePro('meal-prep')
  const supabase = createServerClient()
 ...
 */
export async function markContainerLost(containerId: string): Promise<Container | { error: string }> {
  const user = await requirePro('meal-prep')
  const supabase = createServerClient()
 ...
 */
export async function getContainers(statusFilter?: ContainerStatus): Promise<Container[] | { error: string }> {
  const user = await requirePro('meal-prep')
  const supabase = createServerClient()
...
export async function getContainerSummary(): Promise<ContainerSummary | { error: string }> {
  ...
  for (const c of containers) {
    ...
    if (c.status === 'with_client' && c.client_id) {
      const existing = clientCounts.get(c.client_id) ?? { name: c.client_name ?? 'Unknown', count: 0 }
      existing.count++
