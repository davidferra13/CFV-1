 ...
 */
export function computeEngagementScore(events: ActivityEvent[], windowDays = 14): EngagementScore {
  if (events.length === 0) {
  ...
  const level: EngagementLevel =
    score >= 60 ? 'hot' : score >= 25 ? 'warm' : score > 0 ? 'cold' : 'none'

  // Most recent event timestamp
  const lastActivityAt =
    recent.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0]
      ?.created_at ?? null
