...
import { getGoogleAccessToken } from '@/lib/gmail/google-auth'
...
export async function initiateGoogleCalendarConnect(): Promise<{ redirectUrl: string }> {
  const { initiateGoogleConnect } = await import('@/lib/gmail/google-auth')
...
export async function disconnectGoogleCalendar() {
  const { disconnectGoogle } = await import('@/lib/gmail/google-auth')
