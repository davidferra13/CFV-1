...
export async function getPostEventSurveyPageData(
  ...
  if (survey.event_id) {
    const { getEventDishFeedbackChoicesByTenant } = await import('@/lib/post-event/learning-actions')
...
export async function submitPostEventSurveyResponse(
  ...
  {
    const { refreshEventOutcomeLearningByTenant } = await import('@/lib/post-event/learning-actions')
