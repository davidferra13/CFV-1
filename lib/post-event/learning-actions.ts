...
async function getMenuDishesForMenu(
  db: any,
  menuId: string | null,
  tenantId: string
): Promise<MenuDishRow[]> {
  ...
  const { data } = await db
    .from('dishes')
    .select('id, name, course_name, recipe_id, dish_index_id, copied_from_dish_index_id')
    .eq('menu_id', menuId)
...
async function resolveDishIndexIds(
  ...
  const canonicalNames = [
    ...new Set(unresolved.map((dish) => canonicalizeDishName(dish.name ?? ''))),
  ]
  const { data } = await db
    .from('dish_index')
...
async function ensureEventOutcomeSeedInternal(
  ...
  if (dishes.length === 0 && plannedSnapshot.length > 0) {
    ...
    const { data: insertedRows } = await db.from('event_outcome_dishes').insert(inserts).select('*')
...
function buildSurveyChoiceRows(
  outcome: EventOutcomeRow,
  dishes: EventOutcomeDishRow[]
...
function buildComparableMenuPattern(
  ...
  return {
    ...
    averageSuccessScore:
      averageSuccessScore === null ? null : Number(averageSuccessScore.toFixed(2)),
  }
}
...
async function buildInsightsForEvent(
  ...
  const { data: comparableOutcomesRaw } = await db
    .from('event_outcomes')
    .select('event_id, planned_dish_count, time_accuracy, prep_accuracy, success_score')
    .eq('tenant_id', event.tenant_id)
    .not('event_id', 'is', null)

  const comparableOutcomes = (comparableOutcomesRaw as Array<Record<string, unknown>> | null) ?? []
  ...
  for (const row of (comparableEventRows as Array<{
    id: string
    guest_count: number | null
  }> | null) ?? []) {
...
export async function refreshEventOutcomeLearningByTenant(
  ...
  const { data: hostSurvey } = await db
    ...
    .select('overall, food_quality, completed_at, dish_feedback')
    .eq('event_id', event.id)
    .eq('tenant_id', event.tenant_id)
    .maybeSingle()
  ...
  const guestFeedbackRows = (guestFeedbackRowsRaw as Array<Record<string, unknown>> | null) ?? []
  ...
  if (coerceNumber(hostSurvey?.overall) !== null)
    guestOverallRatings.push(coerceNumber(hostSurvey?.overall)!)
  if (coerceNumber(hostSurvey?.food_quality) !== null)
    guestFoodRatings.push(coerceNumber(hostSurvey?.food_quality)!)
  if (coerceString(hostSurvey?.completed_at))
    feedbackTimestamps.push(coerceString(hostSurvey?.completed_at)!)
...
export async function getEventOutcomeCapture(
  eventId: string
): Promise<EventOutcomeCaptureData | null> {
...
export async function getEventOutcomeSummary(eventId: string): Promise<EventOutcomeSummary | null> {
  ...
  return {
    ...
    matchedDishCount: capture.dishes.filter((dish) => dish.outcomeStatus === 'planned_served')
      .length,
    ...
    substitutedDishCount: capture.dishes.filter((dish) => dish.outcomeStatus === 'substituted')
      .length,
...
export async function saveChefOutcomeCapture(
  ...
  const canonicalNames = [
    ...new Set(
      input.dishes.map((dish) => canonicalizeDishName(dish.actualName ?? dish.plannedName))
    ),
  ]
