...
async function handleTakeAChefEmail(
  ...
  const canonicalCommunication = await ingestGmailImportedCommunicationEvent({
    ...
    mailbox,
    source: 'takeachef',
    isRawSignalOnly: !['tac_new_inquiry', 'tac_client_message', 'tac_booking_confirmed'].includes(
      parsed.emailType
    ),
  })
  ...
  try {
    switch (parsed.emailType) {
      ...
      case 'tac_payment':
        await handleTacPayment(db, email, parsed, tenantId, result, canonicalCommunication, mailbox)
        break
