...
import type {
  ...
  ViewportSize,
  ViewportSummary,
} from './viewport-discipline-types'
...
export async function getViewportAudits(
  ...
  let query = db.select().from('viewport_audits').where('tenant_id', tenantId)
...
export async function getViewportRules(viewport?: ViewportSize): Promise<ViewportRule[]> {
  const user = await requireChef()
  const tenantId = user.tenantId!

  let query = db.select().from('viewport_rules').where('tenant_id', tenantId)
...
export async function getViewportSummary(): Promise<ViewportSummary> {
  ...
  const audits = await db.select().from('viewport_audits').where('tenant_id', tenantId)

  const rules = await db.select().from('viewport_rules').where('tenant_id', tenantId)
...
export async function getViewportFailures(viewport?: ViewportSize): Promise<ViewportAudit[]> {
  const user = await requireChef()
  const tenantId = user.tenantId!
