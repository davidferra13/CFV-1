 ...
 */
export async function getOrderAheadMenu(statusFilter?: OrderAheadItemStatus): Promise<OrderAheadItem[] | { error: string }> {
  const user = await requirePro('commerce')
  const supabase = createServerClient()
...
export async function updateOrderAheadItem(
  ...
  if (input.maxQuantityPerOrder !== undefined) updateData.max_quantity_per_order = input.maxQuantityPerOrder
 ...
 */
export async function getOrders(statusFilter?: OrderStatus): Promise<OrderAheadOrder[] | { error: string }> {
  const user = await requirePro('commerce')
  const supabase = createServerClient()
