...
export async function reconcileCommunicationDeliveryState(
  ...
  if (event && reducedEventState) {
    const eventPatch = buildPatch(currentEventState!, reducedEventState, [
  ...
  const threadPatch = buildPatch(currentThreadState, projectedThreadState, [
  ...
  return {
    ...
    nextDeliveryStatus:
      reducedEventState?.providerDeliveryStatus ||
      projectedThreadState.latestOutboundDeliveryStatus,
    nextProviderStatus:
