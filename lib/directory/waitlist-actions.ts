...
export async function saveNearbySavedSearchAlert(input: {
  ...
  if (preferenceError) {
    console.error('[directory-waitlist] Failed to refresh nearby email preferences:', preferenceError)
  }
...
export async function sweepDirectoryWaitlist(): Promise<{
  ...
  const waitlistEntries = [...(legacyWaitlistResult.data || []), ...(sourcedWaitlistResult.data || [])]
