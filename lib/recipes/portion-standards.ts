...
const STANDARDS: Partial<Record<string, Partial<Record<CoursePosition, RawStandard>>>> = {
  ...
  protein: {
    amuse:     { oz: 1.5,  note: '1–2 oz cooked bite-size piece per guest',
                 rawFactor: 1.33, rawNote: '~25% cooking loss — buy ~2 oz raw per guest' },
    appetizer: { oz: 2.5,  note: '2–3 oz cooked — plated starter or passed bite',
                 rawFactor: 1.30, rawNote: '~23% cooking loss' },
    first:     { oz: 3.5,  note: '3–4 oz cooked — first-course protein',
                 rawFactor: 1.30, rawNote: '~23% cooking loss' },
    main:      { oz: 6.0,  note: '5–8 oz cooked — 6 oz is the industry standard for main course',
                 rawFactor: 1.33, rawNote: '~25% cooking loss — buy ~8 oz raw per guest' },
    side:      { oz: 2.0,  note: '~2 oz cooked — protein as a supporting element',
                 rawFactor: 1.25, rawNote: '~20% cooking loss' },
    full_meal: { oz: 7.0,  note: '7–8 oz cooked — generous standalone protein entrée',
                 rawFactor: 1.33, rawNote: '~25% cooking loss — buy ~9 oz raw per guest' },
  },
  ...
  vegetable: {
    ...
    main:      { oz: 4.0,  note: '4 oz cooked — featured or side vegetable',
                 rawFactor: 1.15, rawNote: '~12% trim and cooking loss' },
    side:      { oz: 4.0,  note: '4 oz cooked — standard vegetable side',
                 rawFactor: 1.15, rawNote: '~12% trim and cooking loss' },
...
export function getPortionInfo(
  ...
  return {
    ...
    rawTotalLabel: rawTotalOz ? formatOz(rawTotalOz, guestCount, std.oz * (std.rawFactor ?? 1)) + ' raw' : null,
  }
}
...
const SCALING_RULES: Record<string, ScalingRule> = {
  ...
  dairy: {
    exponent: 0.92,
    label: 'near-linear',
    reason: 'Cream and butter slightly concentrate in larger batches — start with ~90–92% and taste.',
  },
  pantry: {
    exponent: 0.85,
    label: 'sub-linear',
    reason: 'Pantry items (stocks, canned goods, vinegars) are more efficient at scale — use 80–88%.',
  },
  spice: {
    exponent: 0.75,
    label: 'highly sub-linear',
    reason: 'Spice flavor intensifies with batch size. At 4× scale use ~3× spice, not 4×. Taste and adjust.',
  },
  oil: {
    exponent: 0.88,
    label: 'sub-linear',
  ...
  fresh_herb: {
    exponent: 0.90,
    label: 'slightly sub-linear',
    reason: 'Visual garnish uses are near-linear; flavoring uses concentrate — split the difference.',
  },
  dry_herb: {
    exponent: 0.75,
    label: 'highly sub-linear',
  ...
  condiment: {
    exponent: 0.80,
    label: 'sub-linear',
    reason: 'Mustard, Worcestershire, fish sauce, hot sauce — punch-above-weight ingredients. Use 75–85%.',
  },
  beverage: {
    exponent: 1.00,
    label: 'linear',
...
function formatOz(totalOz: number, guestCount: number, portionOz: number): string {
  ...
  if (totalOz >= 8) {
    ...
    const cupsStr = Number.isInteger(cups) ? `${cups} cup${cups !== 1 ? 's' : ''}` : `${fmtNum(totalOz)} oz`
