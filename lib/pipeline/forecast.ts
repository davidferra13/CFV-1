...
export async function getPipelineRevenueForecast(): Promise<PipelineRevenueForecast> {
  ...
  const stageMap = new Map<
    string,
    { count: number; totalCents: number; probability: number; label: string }
  >()
  ...
  for (const inquiry of inquiries) {
    const prob = INQUIRY_PROBABILITY[inquiry.status] ?? 0.1
    ...
    const existing = stageMap.get(key) ?? {
      ...
      totalCents: 0,
      probability: prob,
      label: INQUIRY_LABELS[inquiry.status] ?? inquiry.status,
    }
    stageMap.set(key, {
      ...existing,
  ...
  for (const event of events) {
    const prob = EVENT_PROBABILITY[event.status] ?? 0.5
  ...
  const STAGE_ORDER = [
    ...
    'inquiry_awaiting_chef',
    'inquiry_awaiting_client',
    'inquiry_quoted',
    'inquiry_confirmed',
    'event_draft',
    'event_proposed',
  ...
  const stages: ForecastStage[] = STAGE_ORDER.filter((key) => stageMap.has(key)).map((key) => {
