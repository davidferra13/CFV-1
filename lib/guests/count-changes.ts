...
function mapGuestCountChangeRow(row: Record<string, any>): GuestCountChange {
  return {
    ...
    requested_by_role:
      row.requested_by_role === 'chef' ? 'chef' : ('client' as GuestCountChange['requested_by_role']),
...
function buildApprovedEventUpdate(
  ...
  if (
    ...
    update.quoted_price_cents = Math.max(0, (event.quoted_price_cents ?? 0) + pricing.totalDeltaCents)
  }
...
async function syncInquiryGuestCount(db: any, event: GuestCountEventRecord, newCount: number) {
  ...
  await db
    .from('inquiries')
    .update({ confirmed_guest_count: newCount })
...
async function getEventForClientMutation(eventId: string, clientId: string): Promise<GuestCountEventRecord | null> {
...
async function getEventForChefMutation(eventId: string, tenantId: string): Promise<GuestCountEventRecord | null> {
  const db: any = createServerClient()
...
export async function reviewGuestCountChange(
  ...
  if ((event.guest_count ?? 0) !== change.previous_count) {
    return {
      success: false,
      error: 'The event guest count changed after this request was submitted. Ask the client to resubmit.',
    }
  }
