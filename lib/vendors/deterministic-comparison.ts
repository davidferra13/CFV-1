...
export function buildVendorRecommendation(
  ...
  const spreadPercent =
    second.priceCents > 0 ? ((second.priceCents - cheapest.priceCents) / second.priceCents) * 100 : 0
  ...
  return {
    ...
    ).toFixed(2)} per ${cheapest.unit}) and is about ${spreadPercent.toFixed(1)}% below the next option.`,
