...
function cityMatches(candidateCity: string | null | undefined, expectedCity: string | null | undefined) {
...
function scoreDirectoryListingAccountLink(
  ...
  if (
    ...
    (normalizeComparableText(candidate.claimed_by_email) || normalizeComparableText(candidate.email))
  ) {
    return null
  }
...
export async function resolveChefDirectoryListingLink(
  ...
  if (email) {
    const [claimedByEmail, listingEmail] = await Promise.all([
      db
        .from('directory_listings')
        .select('id, name, slug, city, state, status, claimed_by_email, email, website_url, linked_chef_id')
        .neq('status', 'removed')
...
> {
  ...
  const linkedChefId = (listing as { linked_chef_id?: string | null } | null)?.linked_chef_id ?? null
