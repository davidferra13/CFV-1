...
export function buildNearbyBrowseHrefFromFilters(filters: NearbyFallbackFilters) {
  ...
  if (
    normalized.radiusMiles != null &&
    (normalized.locationQuery || hasCoordinateLocation(normalized))
  ) {
...
export function buildNearbyBrowseFallbackActions({
  ...
  if (normalized.query) {
    pushAction({
      ...
      description:
        'Keep the current geography and category filters without requiring a keyword match.',
      variant: 'secondary',
    })
  }
  ...
  if (
    ...
    normalized.businessType &&
    typeCollectionLabel &&
    (normalized.state || normalized.city)
  ) {
    const typeCity = findBestTypeCity(
      stats,
      normalized.businessType,
      normalized.state,
      normalized.city
