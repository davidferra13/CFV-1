...
export function computeEventOutcomeMetrics(input: EventOutcomeMetricInput): EventOutcomeMetrics {
  ...
  const matchedDishCount = input.dishes.filter(
    (dish) => dish.outcomeStatus === 'planned_served'
  ).length
  ...
  const substitutedDishCount = input.dishes.filter(
    (dish) => dish.outcomeStatus === 'substituted'
  ).length
