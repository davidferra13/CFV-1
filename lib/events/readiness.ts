...
function isBlockingForTarget(proof: ServiceSimulationProof, targetStatus: ReadinessTargetStatus): boolean {
...
function computeConfidence(counts: ReadinessResult['counts']): number {
  return Math.max(0, Math.min(100, 100 - counts.blockers * 25 - counts.risks * 10 - counts.stale * 6))
}
...
async function fetchOverrideRows(
  ...
  if (
    error &&
    !error.message?.includes('event_readiness_gates')
  ) {
    throw new Error(`Failed to load readiness overrides: ${error.message}`)
  }

  return new Map(
    ((data ?? []) as OverrideRow[]).map((row) => [row.gate as ReadinessGate, row])
  )
}
...
async function evaluateReadinessInternal(params: {
  ...
  const risks = gates.filter(
    (gate) =>
      ...
      (gate.status === 'unverified' && !gate.isHardBlock)
  ).length
