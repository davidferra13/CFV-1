...
import { isContractClientSignable, isMenuClientReviewPending } from '@/lib/action-graph/bookings'
    ...
    },
    {
      ...
      completedAt:
        menuApprovalStatus === 'approved'
          ...
          : null,
      isSkipped: false,
      actionHref: isMenuClientReviewPending(menuApprovalStatus)
...
function hasReachedJourneyStep(step: JourneyStepDefinition, input: JourneyStepInput): boolean {
  if (step.completedAt) return true

  switch (step.key) {
    case 'inquiry_received':
      return Boolean(
        input.inquiryCreatedAt || input.quoteSentAt || input.quoteStatus || input.eventStatus
      )
    case 'proposal_sent':
      return Boolean(
        input.quoteSentAt ||
