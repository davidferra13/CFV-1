// Churn Prevention Triggers - Internal (no auth required)
// Core logic extracted so it can be called from cron jobs, CIL analyzers,
// and the churn-cadence-bridge without requiring auth context.
// Types are defined here; churn-prevention-triggers.ts re-exports them.
...
/**
 * Compute churn prevention triggers for a specific tenant.
 * Does not require auth context. Safe for cron, CIL scanner, bridge calls.
 */
export async function getChurnPreventionTriggersForTenant(
  tenantId: string
): Promise<ChurnPreventionResult | null> {
