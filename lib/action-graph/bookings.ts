...
export function compareActionGraphActions(
  left: ActionGraphAction,
  right: ActionGraphAction
): number {
...
export function isMenuClientReviewPending(menuApprovalStatus: string | null | undefined): boolean {
...
export function buildCanonicalBookingActions(event: BookingActionEvent): BookingAction[] {
    ...
    actions.push(
      createBookingAction({
        ...
        chef: {
          ...
          description:
            'Payment is blocked until the service agreement is ready for client signature.',
      ...
      createBookingAction({
        ...
        evidence: [
          ...
          approvalNeedsRefresh
            ? 'events.menu_modified_after_approval=true'
  ...
  if (
    isActiveBookingStatus(event.status) &&
    isMenuClientReviewPending(event.menu_approval_status)
  ) {
    actions.push(
      createBookingAction({
        event,
...
function getInquiryLabel(inquiry: Pick<InquiryActionRecord, 'confirmed_occasion'>): string {
