...
const stableCache: typeof cache =
  typeof cache === 'function'
    ? cache
    : ((fn: typeof cache extends (callback: infer T) => unknown ? T : never) => fn) as typeof cache
...
export const getFounderProfile = stableCache(async (): Promise<FounderProfile> => {
  ...
  try {
    ...
    const city = typeof data.city === 'string' && data.city.trim() ? data.city.trim() : fallback.city
    const state =
