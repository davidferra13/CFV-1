...
export async function reviewQuarantinedPrice(
  ...
  try {
    ...
    if (writebackRequested && reviewContext && reviewContext.tenantId && reviewedPriceCents !== null) {
      ...
      await nonBlocking(
        ...
        async () => {
          await refreshIngredientCostsForTenant(reviewContext.tenantId!, [reviewContext.ingredientId])
        }
      )
    } else {
      await pgClient`
