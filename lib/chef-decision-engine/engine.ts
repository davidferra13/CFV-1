...
const ALLERGEN_EQUIVALENTS: Record<string, string[]> = {
  dairy: ['milk', 'cream', 'butter', 'cheese', 'yogurt', 'whey', 'casein', 'ghee', 'lactose'],
...
function assessDishAgainstConstraint(
  ...
  const evidence = uniqueStrings(
    searchableValues.filter((value) =>
      terms.some(
        (term) => overlapsValue(term, value) || searchableText.includes(normalizeKey(term))
      )
    )
  )
  ...
  if (evidence.length > 0) {
    const caution =
      ...
      !terms.some(
        (term) =>
          allergenFlagSet.has(term) || ingredientNames.some((name) => overlapsValue(term, name))
      )
...
function evaluateCourseOptions(
  ...
  return course.options.map((option, index) => {
    ...
    const rawCostScore = lowerIsBetterScore(
      dish.costMetrics.costPerPortionCents,
      comparableCosts,
      65
    )
    const costScore = clamp(rawCostScore - (dish.costMetrics.hasCompleteCostData ? 0 : 12), 0, 100)
    ...
    const totalScore = round1(factorScores.reduce((sum, factor) => sum + factor.contribution, 0))
...
function buildAccommodationCount(option: EvaluatedOption, effectiveGuestCount: number): number {
...
function pickAccommodationOption(
  ...
  strategy: 'single_path' | 'main_plus_accommodation' | 'split_menu'
} {
...
function buildCourseRecommendation(
  ...
  if (course.options.length === 0) {
    ...
    const flag: ChefDecisionRiskFlag = {
      ...
      message,
      suggestedResolution:
        'Attach candidate dishes or pass the Dinner Circle selection output into the engine.',
    }
  ...
  if (!locked) {
    ...
    if (course.totalVotes <= 0 && topVoteShare == null) {
      ...
      flags.push({
        ...
        suggestedResolution:
          'Use the recommendation trace to break the tie or lock the preferred dish manually.',
      })
    } else if (
      topVoteShare != null &&
      ...
      flags.push({
        ...
        suggestedResolution:
          'Confirm the recommendation or lock a dish before execution documents are generated.',
      })
  ...
  if (!locked && course.totalVotes <= 0) {
    warnings.push(
      'No stable poll outcome was available, so the system fell back to operational scoring.'
    )
  }

  const factorScores = selected?.factorScores ?? []
  const strongestFactor = [...factorScores].sort(
...
function collectUniqueAllocatedDishes(
  allocations: PlannedDishAllocation[]
): ChefDecisionDishInput[] {
...
function buildPrepPlan(allocations: PlannedDishAllocation[]): ChefDecisionPrepPlan {
  ...
  if (plan.branchCount > 0) {
    plan.steps.push(
      'Label and separate accommodation paths before packing to avoid service confusion.'
    )
  }
...
function buildEquipmentGapFlags(
  ...
  const available = new Set(
    uniqueStrings([...context.event.confirmedEquipment, ...context.event.equipmentAvailable]).map(
      normalizeKey
    )
  )

  const missing = prepPlan.uniqueEquipment.filter(
    (equipment) => !available.has(normalizeKey(equipment))
  )
...
function buildComplexityFlags(
  ...
  if (prepPlan.totalPrepMinutes >= thresholds.notReadyPrepMinutes) {
    flags.push({
      ...
      suggestedResolution:
        'Simplify one or more courses or increase make-ahead coverage before locking.',
    })
  ...
  if (prepPlan.componentCount >= thresholds.notReadyComponentCount) {
    flags.push({
      ...
      suggestedResolution:
        'Consolidate garnishes, sauces, or side components before locking the menu.',
    })
  ...
  if (uniqueIngredientCount >= thresholds.notReadyUniqueIngredientCount) {
    flags.push({
      ...
      suggestedResolution:
        'Reduce ingredient variety or consolidate dishes around shared mise en place.',
    })
...
function buildReadiness(
  ...
  return {
    ...
    warnings,
    metrics: {
      selectedCourseCount: recommendations.filter((course) => Boolean(course.selectedDishId))
        .length,
...
function buildSummary(
  readiness: ChefDecisionExecutionReadiness,
  recommendations: ChefDecisionCourseRecommendation[]
): string {
...
export function buildChefDecisionEngine(context: ChefDecisionContext): ChefDecisionEngineResult {
  ...
  const effectiveGuestCount = Math.max(context.event.guestCount || 0, activeGuests.length, 1)
