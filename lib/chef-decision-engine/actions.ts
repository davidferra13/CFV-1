...
function deriveCourseKey(courseNumber: number | null, courseName: string | null): string {
  const safeName = cleanText(courseName) ?? (courseNumber != null ? `Course ${courseNumber}` : 'Course')
...
function mergeOptionSignals(
  ...
  return {
    ...
    selectedGuestNames: uniqueStrings([...existing.selectedGuestNames, ...incoming.selectedGuestNames]),
    source,
  ...
  for (const rawCourse of input?.courses ?? []) {
    const courseNumber =
      typeof rawCourse.courseNumber === 'number' && Number.isFinite(rawCourse.courseNumber)
        ...
        : null
    const courseName = cleanText(rawCourse.courseName) ?? (courseNumber != null ? `Course ${courseNumber}` : 'Course')
...
function mergeSelectionSignals(
  ...
  const map = new Map(base.map((course) => [course.courseKey, { ...course, options: [...course.options] }]))

  for (const course of incoming) {
    const existing =
      map.get(course.courseKey) ??
      {
...
async function loadHubSelectionSignals(
  ...
  for (const option of optionRows) {
    ...
    if (readMetadataBoolean(metadata, ['locked', 'isLocked', 'is_locked', 'finalSelection', 'final_selection'])) {
...
function normalizeIngredientRelation(
  value: RawRecipeIngredientRow['ingredients']
): {
  name: string | null
  allergen_flags: string[] | null
  dietary_tags: string[] | null
  ...
  for (const dish of input.dishes) {
    ...
    result.set(dish.id, {
      ...
      operationalMetrics: {
        ...
        makeAheadComponentCount: components.filter((component) => Boolean(component.is_make_ahead)).length,
      ...
      costMetrics: {
        ...
        ingredientCount: recipeCosts.reduce((sum, cost) => sum + Number(cost.ingredient_count ?? 0), 0),
...
function buildGuests(input: {
  ...
  const dietaryItemsByGuest = new Map<string, Array<{
    subject: string | null
    item_type: string | null
    label: string | null
    severity: string | null
  ...
  if (primaryConstraints.length > 0) {
    guests.push({
      ...
      attending: true,
      constraints: uniqueStrings(primaryConstraints.map((constraint) => `${constraint.type}:${constraint.severity}:${constraint.label}`)).map((entry) => {
  ...
  for (const guest of input.eventGuests) {
    ...
    const constraints = [
      ...
      ...structured
        ...
        .map((item) => ({
          ...
          type: normalizeKey(item.item_type) === 'allergy' ? ('allergy' as const) : ('dietary' as const),
          severity:
    ...
    guests.push({
      ...
      attending,
      constraints: uniqueStrings(constraints.map((constraint) => `${constraint.type}:${constraint.severity}:${constraint.label}`)).map((entry) => {
        const [type, severity, ...label] = entry.split(':')
        return {
          type: type as 'dietary' | 'allergy',
          severity: severity as 'preference' | 'intolerance' | 'anaphylaxis',
    ...
    if (plusOneConstraints.length > 0 || cleanText(guest.plus_one_name)) {
      guests.push({
        ...
        attending,
        constraints: uniqueStrings(plusOneConstraints.map((constraint) => `${constraint.type}:${constraint.severity}:${constraint.label}`)).map((entry) => {
          const [type, severity, ...label] = entry.split(':')
          return {
            type: type as 'dietary' | 'allergy',
            severity: severity as 'preference' | 'intolerance' | 'anaphylaxis',
    ...
    .map((courseKey) => {
      ...
      const optionIds =
        signal && signal.options.length > 0
          ? uniqueStrings([
              ...signal.options.map((option) => option.dishId),
              signal.lockedDishId,
            ])
          : fallbackDishIds
      ...
      return {
        courseKey,
        courseNumber: signal?.courseNumber ?? meta?.courseNumber ?? options[0]?.dish.courseNumber ?? null,
        courseName:
          signal?.courseName ??
          meta?.courseName ??
          options[0]?.dish.courseName ??
          'Course',
...
async function buildChefDecisionContextForEvent(
  ...
  const menuPreferenceMenuId = cleanText((menuPreferencesResponse.data as { selected_menu_id?: string | null } | null)?.selected_menu_id)
  const lockedMenuId = cleanText((event.menu_id as string | null) ?? null) ?? menuPreferenceMenuId ?? null
  ...
  const menuIds = uniqueStrings([...menus.map((menu) => menu.id), lockedMenuId, menuPreferenceMenuId])
  ...
  const confirmedEquipment = uniqueStrings([
    ...
    ...assignments.flatMap((assignment) => [
      ...
      assignment.equipment_item_id ? ownedById.get(assignment.equipment_item_id)?.name ?? null : null,
    ...
    createPlanningEvidenceSource({
      ...
      coveragePercent:
        courseInputs.length > 0
          ? Math.round(
              (courseInputs.filter((course) => course.totalVotes > 0 || course.lockedDishId).length /
                courseInputs.length) *
                100
            )
          : 100,
...
function buildChefDecisionDataQuality(input: {
  ...
  const selectionCoveragePercent =
    input.context.courses.length > 0
      ? Math.round(
          (input.context.courses.filter((course) => course.totalVotes > 0 || course.lockedDishId).length /
  ...
  return summarizePlanningDataQuality([
    ...
    },
    {
      ...
      status: selectionCoveragePercent >= 100 ? 'pass' : selectionCoveragePercent >= 50 ? 'warn' : 'fail',
      message:
...
function buildChefDecisionSummaryPayload(result: ChefDecisionArtifact | ChefDecisionBuildResult['result']) {
  return {
...
export async function getChefDecisionForEvent(
  ...
  if (shouldAllowCache) {
    ...
    if (cached && isStoredChefDecisionPayload(cached.artifact.payload)) {
      return materializeChefDecisionResponse(
        cached,
