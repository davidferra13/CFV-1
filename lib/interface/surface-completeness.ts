...
import {
  ...
  hasPublicRouteSeoExpectation,
} from '@/lib/site/public-route-seo'
...
type BuildSurfaceManifestModule = {
  ...
  getBuildSurfaceIncludePaths?: (surfaceNameOrManifest: string | BuildSurfaceManifestEntry) => string[]
...
const BUILD_SURFACE_MODE_SHELL_PATHS = ['build-surfaces/web-beta/app/_components/release-portal-shell.tsx']
...
function classifyDerivedOutputMethod(
  source: string
): DerivedOutputInventoryDerivationMethod {
...
function buildDerivedOutputInventory(rootDir = process.cwd()): DerivedOutputInventory {
  ...
  const entries = relativePaths
    .map((relativeFilePath) => {
      ...
      const usesAi = DERIVED_OUTPUT_AI_PATTERN.test(source) || DERIVED_OUTPUT_HYBRID_PATTERN.test(source)
...
function runSurfaceModeDeclarationCheck(): SurfaceCompletenessCheckResult {
  ...
  for (const relativePath of BUILD_SURFACE_MODE_SHELL_PATHS) {
    ...
    if (!source.includes('data-cf-portal={portal}') || !source.includes('data-cf-surface={surfaceMode}')) {
...
function runDerivedOutputProvenanceInventoryCheck(): SurfaceCompletenessCheckResult {
  ...
  const missingModelMetadataPaths = inventory.entries
    .filter(
      (entry) =>
        entry.derivationMethod !== 'deterministic' && !entry.hasModelMetadataSignal
    )
    .map((entry) => entry.relativeFilePath)
...
function runPublicSeoContractCheck(): SurfaceCompletenessCheckResult {
  ...
  const staleExpectationRoutes = PUBLIC_ROUTE_SEO_CHECKS.map((expectation) => expectation.path).filter(
    (route) => !discoveredRoutes.has(route)
  )
  const missingExpectationRoutes = publicEntries
    .filter((entry) => hasPublicMetadataContract(entry) && !hasPublicRouteSeoExpectation(entry.route))
  ...
  if (staleExpectationRoutes.length > 0) {
    findings.push({
      ...
      message: 'Public SEO contract contains routes that were not discovered in the static public surface',
...
function getBuildSurfaceIncludePaths(
  ...
  if (buildSurfaceModule?.getBuildSurfaceIncludePaths) {
    return buildSurfaceModule.getBuildSurfaceIncludePaths(manifest).map((entry) =>
      entry.replace(/\\/g, '/')
    )
  }

  return normalizeBuildSurfaceIncludeEntries(manifest, buildSurfaceModule).map((entry) => entry.path)
}
...
async function importScriptModule<T>(relativeScriptPath: string, rootDir = process.cwd()): Promise<T> {
...
export async function buildSystemContractGraph(
  ...
  for (const pageEntry of pageEntries) {
    ...
    const policyAligned = pageEntry.isDynamic
      ? null
      : !(staticRoute != null && (routePolicyGapsByRole.get(pageEntry.role)?.has(staticRoute) ?? false))
    ...
    const siteAuditMetadata = siteAuditRoutes.length > 0
      ? {
      ...
      : siteAuditSkipped.length > 0
        ? {
            ...
            skipKinds: [...new Set(siteAuditSkipped.map((item) => item.resolver?.kind ?? 'unknown'))],
  ...
  const summary: SystemContractGraphSummary = {
    ...
    pageFacingWriteContractWarningActions: serverActionMutationInventory.contractWarningFunctionCount,
