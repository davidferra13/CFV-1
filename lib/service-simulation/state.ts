...
async function buildEventServiceSimulationContextForScope(
  ...
  if (
    ...
    !includesMissingTable(snapshotRowsResult.error, 'event_document_snapshots')
  ) {
  ...
  if (
    prepBlocksResult.error &&
    !includesMissingTable(prepBlocksResult.error, 'event_prep_blocks')
  ) {
  ...
  if (
    travelLegsResult.error &&
    !includesMissingTable(travelLegsResult.error, 'event_travel_legs')
  ) {
  ...
  const activeGuestRows = guestRows.filter(
    (guest) => (guest.rsvp_status ?? 'attending') !== 'declined'
  )
  ...
  const unresolvedGuestCount = Math.max(
    (eventGuestCount || guestRecordCount) - coveredGuestCount,
    0
  )
...
export async function buildEventServiceSimulationContext(eventId: string): Promise<{
  tenantId: string
  simulation: ServiceSimulationResult
  snapshot: ReturnType<typeof buildServiceSimulationSnapshot>
  hash: string
...
export function buildServiceSimulationPanelState(
  ...
  if (latestRun.engine_version !== SERVICE_SIMULATION_ENGINE_VERSION) {
    staleReasons.push({
      ...
      detail:
        'The saved run used an older engine version and should be refreshed under current logic.',
    })
  }
...
export async function ensureCurrentEventServiceSimulation(
  ...
  const user = await requireChef()
  return ensureCurrentEventServiceSimulationForScope(eventId, { tenantId: user.tenantId! }, options)
}
...
export async function ensureCurrentEventServiceSimulationForTenant(
  ...
  return ensureCurrentEventServiceSimulationForScope(eventId, { tenantId, admin: true }, options)
}
