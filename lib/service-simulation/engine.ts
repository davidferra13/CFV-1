...
function buildReadinessProofs(
  ...
  const packingStatus: ServiceSimulationProofStatus = !packingVerifiedAt
    ...
    : (menuChangedAt && toMs(menuChangedAt) > toMs(packingVerifiedAt)) ||
        (eventUpdatedAt &&
          toMs(eventUpdatedAt) > toMs(packingVerifiedAt) &&
          ctx.event.guestCount !== null)
      ? 'stale'
      : 'verified'
  ...
  const servicePlanStatus: ServiceSimulationProofStatus =
  ...
  return [
    ...
    makeProof({
      ...
      staleReason:
        prepStatus === 'stale'
          ? 'The menu changed after the current prep blocks were scheduled.'
          : null,
      blocking: true,
      severity: 'critical',
      sourceOfTruth:
    ...
    makeProof({
      ...
      blocking: true,
      severity: 'critical',
      sourceOfTruth: logisticsVerifiedAt
...
function buildRollup(
  ...
  const optionalImprovement = actionItems.filter((item) => item.severity === 'optional_improvement')
...
function buildTravelPhase(
  ...
  if (!ctx.event.eventDate || !hasLocation(ctx)) {
    return buildPhase({
      ...
      status: 'waiting',
      summary:
        'Travel planning is waiting on the event date and location before it can be rehearsed honestly.',
      missingItems: [
        issue(
          'travel-core-facts',
...
function buildCloseOutPhase(
  ...
  if (!ctx.closeOut.aarFiled) {
    missingItems.push(
      issue(
        'closeout-aar',
        'File the after-action review',
        'The event review has not been recorded yet.',
        routes.aar
      )
    )
  }
  ...
  if (!ctx.closeOut.resetComplete) {
    missingItems.push(
      issue(
        'closeout-reset',
        'Complete the reset checklist',
        'Kitchen reset and pack-back are not yet marked complete.',
        routes.reset
      )
    )
  }
  ...
  if (!ctx.closeOut.followUpSent) {
    missingItems.push(
      issue(
        ...
        'The post-service follow-up is still open on this event.',
        routes.wrap
      )
    )
  }
  ...
  if (!ctx.closeOut.financiallyClosed) {
    missingItems.push(
      issue(
        ...
        'Final expenses and event close-out are not yet reconciled.',
        routes.financial
      )
    )
  }
...
export function buildServiceSimulation(ctx: ServiceSimulationContext): ServiceSimulationResult {
  ...
  const severityBands = {
    ...
    optionalImprovement: actionItems.filter((item) => item.severity === 'optional_improvement'),
  }
