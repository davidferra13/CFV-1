...
export function getServiceSimulationTransitionGate(
  ...
  if (panelState.status === 'unsimulated') {
    reasons.push({
      ...
      detail: 'Current truth can be checked now, but no chef-acknowledged service simulation has been saved.',
    })
  }
  ...
  const status: ServiceSimulationTransitionGate['status'] =
    hardBlock ? 'hard' : reasons.length > 0 ? 'soft' : 'clear'
  const summary =
    status === 'clear'
