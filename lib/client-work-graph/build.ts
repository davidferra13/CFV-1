...
function buildQuoteWorkItems(
  ...
  return quotes
    ...
    .map((quote) => buildSharedGraphWorkItem(getClientQuoteAction(quote), quote.id, 'quote', 'quote_review'))
...
function buildInquiryWorkItems(inquiries: ClientGraphInquiry[]): ClientWorkItem[] {
  return inquiries
    .map((inquiry) =>
      buildSharedGraphWorkItem(getClientInquiryAction(inquiry), inquiry.id, 'inquiry', 'inquiry_reply')
    )
    .filter((item): item is ClientWorkItem => Boolean(item))
}
...
function buildEventStubWorkItems(eventStubs: ClientEventStubSummary[]): ClientWorkItem[] {
  const activeStubs = eventStubs.filter((stub) => stub.status !== 'cancelled' && stub.status !== 'adopted')
...
export function buildClientWorkGraph(input: ClientWorkGraphInput): ClientWorkGraph {
  const eventItems = input.events.map(buildEventWorkItem).filter((item): item is ClientWorkItem => Boolean(item))
  ...
  const summary = {
    ...
    profileCount: items.filter((item) => ['profile_completion', 'meal_request', 'signal_notifications'].includes(item.kind)).length,
    notificationCount: items.filter((item) => item.kind === 'notification_follow_up').length,
    planningCount: items.filter((item) => ['stub_planning', 'stub_seeking_chef'].includes(item.kind)).length,
  }
...
function defaultPageSuggestions(message: string): ClientNavSuggestion[] {
  ...
  if (/(guest count|change guest|booking change|change request|change my booking)/.test(normalized)) {
    return [{ label: 'Open Events', href: '/my-events' }]
  }
...
function matchesQuery(item: ClientWorkItem, message: string): boolean {
  ...
  if (/(menu|dietary|allerg|meal|food)/.test(normalized)) {
    return ['event_menu', 'event_checklist', 'profile_completion', 'meal_request'].includes(item.kind)
  }
...
function uniqueSuggestions(items: Array<{ label: string; href: string }>, limit: number): ClientNavSuggestion[] {
