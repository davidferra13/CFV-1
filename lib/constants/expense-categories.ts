...
export const EXPENSE_CATEGORIES: Record<ExpenseCategory, CategoryMeta> = {
  ...
  vehicle: { label: 'Vehicle', color: 'bg-brand-100 text-brand-800' },
  // Operations
  equipment: { label: 'Equipment', color: 'bg-stone-700 text-stone-200' },
  supplies: { label: 'Supplies', color: 'bg-brand-100 text-brand-800' },
  venue_rental: { label: 'Venue/Rental', color: 'bg-rose-100 text-rose-800' },
  labor: { label: 'Labor', color: 'bg-orange-100 text-orange-800' },
  uniforms: { label: 'Uniforms', color: 'bg-brand-100 text-brand-800' },
  // Business Overhead
...
export const EXPENSE_CATEGORY_GROUPS: ExpenseCategoryGroup[] = [
  ...
  {
    label: 'Other',
    categories: [{ value: 'other', ...EXPENSE_CATEGORIES.other }],
  },
]

// Flat options for simple selects (e.g., import hub)
export const EXPENSE_CATEGORY_OPTIONS = EXPENSE_CATEGORY_VALUES.map((value) => ({
  value,
