...
export function RecipeShareEmail({
  ...
  return (
    <BaseLayout preview={`${sharerName} shared a recipe with you: ${recipeName}`}>
      <Text style={heading}>A chef shared a recipe with you</Text>
      <Text style={paragraph}>Hi {chefName},</Text>
      <Text style={paragraph}>
        <strong>{sharerName}</strong> has shared one of their recipes with you via ChefFlow.
        If you accept, an independent editable copy will be added to your recipe library.
      </Text>
      ...
      <Text style={muted}>
        Accepting creates a fully independent copy in your recipe library — you can edit it
        freely without affecting the original. You can also decline if it&apos;s not a fit.
      </Text>
    </BaseLayout>
  )
}
