...
export async function remyCreateTask(inputs: {
  ...
  try {
    ...
    // chef_todos table currently has: id, chef_id, text, completed, completed_at, sort_order, created_at, created_by
    // Extended fields (due_date, priority, notes, event_id, client_id) pending migration.
    // Pass title as string to use the simple API path.
    const todoText = [
      inputs.title,
      inputs.dueDate ? `(due: ${inputs.dueDate})` : '',
      inputs.priority && inputs.priority !== 'medium' ? `[${inputs.priority}]` : '',
...
// ─── Command Registry ──────────────────────────────────────────────────────
// MOVED to lib/ai/remy-write-registry.ts (plain object exports forbidden in 'use server' files)
