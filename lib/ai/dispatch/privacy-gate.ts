...
export function scanAiPrivacyRisk(input: AiDispatchRequest): AiPrivacyScan {
  ...
  const combinedContent = [input.systemPrompt ?? '', input.userContent ?? '', safeStringify(input.metadata)]
    .filter(Boolean)
    .join('\n')
  ...
  if ([...matchedSignals].some((signal) => signal.startsWith('task:') || signal.startsWith('content:'))) {
    level = 'restricted'
  } else if (
