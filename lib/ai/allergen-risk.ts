...
export async function getEventAllergenRisk(eventId: string): Promise<AllergenRiskResult> {
  ...
  const [eventResult, guestsResult, menuResult]: [
    ...
    { data: { name: string | null; dietary_restrictions: unknown; allergies: unknown; notes: string | null }[] | null },
    { data: { name: string; description: string | null; allergen_tags: unknown }[] | null },
  ] = await Promise.all([
    db.from('events').select('occasion, dietary_restrictions, allergies').eq('id', eventId).eq('tenant_id', user.tenantId!).single(),
    db.from('event_guests').select('name, dietary_restrictions, allergies, notes').eq('event_id', eventId),
    db.from('event_menu_components').select('name, description, allergen_tags').eq('event_id', eventId),
  ])
  ...
  if (menuItems.length === 0) {
    return { rows: [], safetyFlags: ['No menu items assigned yet — add dishes to run allergen analysis.'], confidence: 'low' }
  }
...
`
  ...
  try {
    return await parseWithOllama(systemPrompt, userContent, AllergenRiskResultSchema)
  } catch (err) {
    if (err instanceof OllamaOfflineError) throw err
    console.error('[allergen-risk] AI analysis failed:', err)
    return {
      rows: [],
      safetyFlags: ['AI analysis unavailable — verify allergens manually before service.'],
      confidence: 'low',
    }
  }
}
