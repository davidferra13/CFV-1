...
export function mapCaptureToOperatingLoopItem(
  capture: CaptureEntryLikeSource
): OperatingLoopItem {
  ...
  const confidence =
    capture.priority === 'urgent' ? 0.95
    : capture.priority === 'high' ? 0.8
    : capture.priority === 'normal' ? 0.6
    : 0.4
  ...
  return baseItem({
    ...
    evidenceLabel: 'user_entered',
    confidence,
    title: capture.content.length > 80
      ? capture.content.slice(0, 77) + '...'
      : capture.content,
