...
export async function copyToClipboard(value: string, label = 'Text'): Promise<CopyResult> {
  ...
  try {
    ...
    recordHandoffUsage('copy', 'copy', label)
    return { success: true }
  } catch {
...
export function recordHandoffUsage(kind: HandoffKind, action: HandoffAction, surface?: string): void {
