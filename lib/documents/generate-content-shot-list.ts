...
export function renderContentShotList(pdf: PDFLayout, data: ContentShotListData) {
  ...
  pdf.title('CONTENT ASSET CAPTURE SHEET', FONT.sectionHeader.size + 1)
  ...
  pdf.sectionHeader('PRE-EVENT - GROCERY & PREP', FONT.bodyText.size)
  pdf.checkbox(
    'Grocery haul flat lay - overhead, all ingredients spread out, vertical 9:16',
    FONT.metadata.size
  )
  pdf.checkbox(
  ...
  pdf.checkbox(
    'Knife roll / tools laid out - overhead or 45° before cooking begins',
    FONT.metadata.size
  )
  pdf.checkbox(
    'Mise en place spread - everything prepped, portioned, and arranged',
    FONT.metadata.size,
    'overhead'
  )
  ...
  pdf.sectionHeader('ACTIVE COOKING', FONT.bodyText.size)
  pdf.checkbox(
  ...
  pdf.checkbox('Sauce or reduction - overhead close-up, glossy spoon, 5–8 sec', FONT.metadata.size)
  pdf.checkbox(
    'Hands seasoning or finishing touch - pinch of salt, torn herb, 3–5 sec slow-mo',
    FONT.metadata.size
  )
  pdf.checkbox(
  ...
  pdf.checkbox(
    'Texture detail / close-up cross-section - demonstrates technique at macro level',
    FONT.metadata.size
  )
  ...
  pdf.sectionHeader('SERVICE & WRAP', FONT.bodyText.size)
  pdf.checkbox(
    'Empty plate after course - overhead, social proof without showing client faces',
    FONT.metadata.size
  )
  pdf.checkbox(
  ...
  pdf.checkbox(
    'Clean kitchen at wrap - wide shot, proves you left it better than you found it',
    FONT.metadata.size
  )
  ...
  pdf.sectionHeader(
    'BRAND CONSISTENCY - repeat the same decisions at every event',
    FONT.bodyText.size
  )
  pdf.checkbox(
  ...
  pdf.sectionHeader(
    'SOLO SETUP - mount phone before service starts, never during it',
    FONT.bodyText.size
  )
  pdf.text(
