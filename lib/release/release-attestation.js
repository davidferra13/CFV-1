...
export function resolveReleaseAttestationPath({
  profile = resolveReleaseProfile(),
  env = process.env,
} = {}) {
...
export function evaluateReleaseAttestation(
  attestation,
  { expectedProfile, currentSnapshot = null } = {}
) {
  const advisoryCount = Number(
    attestation?.summary?.advisoryCount ?? attestation?.advisories?.length ?? 0
  )
  const blockerCount = Number(
    attestation?.summary?.blockerCount ?? attestation?.blockers?.length ?? 0
  )
  ...
  const attestedSnapshot =
    attestation.snapshot && typeof attestation.snapshot === 'object' ? attestation.snapshot : null
  const attestedHead =
