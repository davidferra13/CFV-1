...
async function fetchUnansweredInquiries(db: any, tenantId: string): Promise<AttentionItem[]> {
  ...
  return data.map((i: any) => {
    const daysSince = Math.floor((Date.now() - new Date(i.created_at).getTime()) / 86400000)
...
async function fetchUnsignedContracts(db: any, tenantId: string): Promise<AttentionItem[]> {
  const { data } = await db
    ...
    .select(
      'id, status, sent_at, created_at, event_id, events(occasion, event_date), clients(full_name)'
    )
    .eq('chef_id', tenantId)
