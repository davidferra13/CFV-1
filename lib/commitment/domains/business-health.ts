...
export type BusinessHealthContext = {
  dashboardViewedDaysAgo: number | null
  lastRateReviewDaysAgo: number | null
  certificationsExpired: boolean
...
const RULE_DESCRIPTIONS: Record<string, string> = {
  ...
  quarterly_rate_review: 'Quarterly rate review overdue',
  certification_currency: 'One or more certifications have expired',
  savings_reserve_percent: 'Savings reserve is below your committed percentage',
}
...
export async function evaluateBusinessHealthCommitments(
  ...
  for (const row of rows) {
    ...
    if (rule.type === 'weekly_financial_review') {
      violated = context.dashboardViewedDaysAgo === null || context.dashboardViewedDaysAgo > 7
    } else if (rule.type === 'quarterly_rate_review') {
      violated = context.lastRateReviewDaysAgo === null || context.lastRateReviewDaysAgo > 90
    } else if (rule.type === 'certification_currency') {
      violated = context.certificationsExpired
    } else if (rule.type === 'savings_reserve_percent') {
      if (context.savingsReservePercent != null) {
        violated = context.savingsReservePercent < (rule.percent ?? 0)
      }
    }
...
export async function getBusinessHealthSuggestions(
  ...
  // Check if chef has any active business health commitments already
  ...
  // If no business health commitments exist, suggest a weekly review
  if (!existing || existing.length === 0) {
    suggestions.push({
      ...
      rationale:
        'Regular financial review keeps you aware of margins, outstanding invoices, and revenue trends. A weekly check takes 5 minutes and prevents surprises.',
      evidence: null,
      status: 'pending',
      respondedAt: null,
