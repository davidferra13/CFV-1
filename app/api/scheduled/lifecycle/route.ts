        ...
        for (const quote of expiredQuotes) {
          try {
            ...
            try {
              ...
              console.error(
                `[lifecycle] Quote expiry notification failed for ${quote.id}:`,
                notifErr
              )
              try {
...
${chefName}`
    ...
    await recordCronHeartbeat(
      'lifecycle',
      { ...results, ...reviewResults } as Record<string, unknown>,
      Date.now() - startedAt
    )
    ...
    await recordCronError('lifecycle', message, Date.now() - startedAt)
