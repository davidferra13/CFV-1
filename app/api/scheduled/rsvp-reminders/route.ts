...
async function handleRSVPReminders(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const result = await runMonitoredCronJob('rsvp-reminders', async () => {
      ...
      for (const rawShare of shareRows || []) {
        ...
        if (!share.token || !share.rsvp_deadline_at) continue

        const deadline = new Date(share.rsvp_deadline_at)
        const msUntilDeadline = deadline.getTime() - now.getTime()
        const dueCadences = getReminderOffsetKeys(share.reminder_schedule || []).filter(
          (cadence) => {
    ...
    console.error('[rsvp-reminders] Cron failed:', error)
