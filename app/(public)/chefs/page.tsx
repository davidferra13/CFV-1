  ...
  return (
    <div className="flex items-center gap-2.5 rounded-lg bg-stone-100 dark:bg-stone-800 px-3 py-2">
      {partner.cover_image_url ? (
        ...
        <div className="flex h-8 w-8 items-center justify-center rounded-md bg-stone-200 dark:bg-stone-700 flex-shrink-0">
          <svg
            className="h-4 w-4 text-stone-500 dark:text-stone-300"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
      ...
      <div className="min-w-0 flex-1">
        ...
        {cityState && (
          <p className="text-[11px] text-stone-500 dark:text-stone-300 truncate">{cityState}</p>
        )}
      </div>
      <span className="flex-shrink-0 rounded-full bg-stone-200/70 dark:bg-stone-700/70 px-2 py-0.5 text-[10px] font-medium text-stone-600 dark:text-stone-300">
...
function ChefTile({ chef }: { chef: DirectoryChef }) {
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-2xl bg-white dark:bg-stone-900 shadow-[0_2px_20px_rgb(0,0,0,0.06)] ring-1 ring-stone-200 dark:ring-stone-700 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_40px_rgb(0,0,0,0.12)] dark:hover:shadow-[0_8px_40px_rgb(0,0,0,0.25)] hover:ring-brand-600">
      <div className="relative aspect-[4/3] overflow-hidden bg-gradient-to-br from-brand-100 to-brand-50">
        ...
        {chef.is_founder && (
          <div className="absolute top-4 right-4 rounded-full bg-white/90 dark:bg-stone-900/90 backdrop-blur-sm px-3 py-1 text-xs font-semibold text-brand-600 dark:text-brand-400 shadow-sm">
            Featured
          </div>
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        {chef.bio && (
          <p className="text-sm leading-relaxed text-stone-600 dark:text-stone-300 line-clamp-2">
            {chef.bio}
          </p>
        )}
        ...
        {hasPartners && (
          <div className="mt-4">
            <p className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-stone-500 dark:text-stone-300">
              Where I Cook
            </p>
            <div className="space-y-1.5">
              ...
              {extraCount > 0 && (
                <p className="text-center text-[11px] text-stone-500 dark:text-stone-300">
        ...
        <div className="mt-5 flex gap-3">
          ...
          <TrackedLink
            ...
            analyticsProps={{ chef_slug: chef.slug }}
            className="rounded-xl border border-stone-200 dark:border-stone-700 px-4 py-3 text-center text-sm font-medium text-stone-600 dark:text-stone-300 transition-colors hover:bg-stone-50 dark:hover:bg-stone-800 hover:text-stone-900 dark:hover:text-stone-100 hover:border-stone-300 dark:hover:border-stone-600"
          >
            Profile
          </TrackedLink>
        </div>
  ...
  return (
    <div className="min-h-screen bg-stone-50 dark:bg-stone-800">
      ...
      <section className="mx-auto -mt-8 max-w-6xl px-4 pb-4 sm:px-6 lg:px-8">
        <div className="rounded-2xl border border-stone-200 dark:border-stone-700 bg-white/95 dark:bg-stone-900/95 p-4 shadow-lg backdrop-blur-sm sm:p-6">
      ...
      <section className="mx-auto max-w-6xl px-4 pb-20 pt-12 sm:px-6 lg:px-8">
          ...
          {activeFilters.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {activeFilters.map((filterValue) => (
                <span
                  key={filterValue}
                  className="rounded-full border border-stone-300 dark:border-stone-600 bg-white dark:bg-stone-900 px-3 py-1 text-xs text-stone-600 dark:text-stone-300"
                >
                  {filterValue}
                </span>
              ))}
        ...
        {chefs.length === 0 ? (
          <div className="text-center py-24">
            <h2 className="text-xl font-semibold text-stone-600 dark:text-stone-300">
            ...
            <div className="mt-6 flex items-center justify-center gap-4">
              ...
              <Link
                href="/contact"
                className="rounded-xl border border-stone-300 dark:border-stone-600 px-5 py-2.5 text-sm font-medium text-stone-600 dark:text-stone-300 transition-colors hover:border-stone-400 dark:hover:border-stone-500 hover:text-stone-900 dark:hover:text-stone-100"
              >
                Contact ChefFlow
              </Link>
            </div>
