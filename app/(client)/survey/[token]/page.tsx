    ...
    return (
      <div className="min-h-screen bg-stone-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-sm border border-stone-200 p-8 max-w-md w-full text-center">
          ...
          <p className="text-stone-600">Your feedback has already been submitted. We appreciate your time!</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-stone-50 py-12 px-4">
      <div className="max-w-lg mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-stone-900">{chefName}</h1>
          <p className="text-stone-600 mt-1">
            {occasion ? `${occasion} · ` : ''}{eventDate ? new Date(eventDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : ''}
          </p>
