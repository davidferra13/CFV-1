...
export default async function ClientRecurringPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-stone-100">Recurring Service</h1>
      ...
      <div className="space-y-3">
        <h2 className="text-base font-semibold text-stone-100">Service Arrangement</h2>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-stone-400 capitalize">
                      {s.frequency} · ${(s.rate_cents / 100).toFixed(0)}/session
                    </p>
                    {s.typical_guest_count && (
                      <p className="text-xs text-stone-400">
                        {s.typical_guest_count} guests typical
                      </p>
                    )}
                    <p className="text-xs text-stone-400">
              ...
              {history.map((entry: any) => (
                <div key={entry.id} className="flex items-center justify-between py-2 text-sm">
                  ...
                  {entry.client_reaction && (
                    <span
                      className={`text-xs font-medium ${REACTION_COLORS[entry.client_reaction] ?? ''}`}
                    >
