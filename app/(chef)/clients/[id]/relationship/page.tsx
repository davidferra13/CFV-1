...
export default async function ClientRelationshipPage({ params }: { params: { id: string } }) {
  ...
  const predictedNextBooking = snapshot.relationshipHealth?.rebookingPrediction.predictedNextBookingDays
          ...
          {snapshot.signals.secondaryLearned.length > 0 ? (
            <div className="border-t border-stone-800 pt-5">
              ...
              <p className="mt-1 text-sm text-stone-400">
                These patterns were learned from history, but a profile-owned fact already exists for
                the same concept.
              </p>
              <SignalList
              ...
              {lastFeedback ? (
                <div className="rounded-xl border border-stone-800 bg-stone-950/50 p-4">
                  ...
                  {lastFeedback.whatTheyLoved ? (
                    <p className="mt-2 text-sm text-stone-300">Loved: {lastFeedback.whatTheyLoved}</p>
                  ) : null}
              ...
              {lastVenueNotes ? (
                <div className="rounded-xl border border-stone-800 bg-stone-950/50 p-4">
                  ...
                  {lastVenueNotes.location ? (
                    <p className="mt-2 text-sm font-medium text-stone-100">{lastVenueNotes.location}</p>
                  ) : null}
...
function SignalList({
  ...
  return (
    <div className={`mt-3 grid gap-3 ${compact ? 'md:grid-cols-2' : 'md:grid-cols-2 xl:grid-cols-3'}`}>
      {signals.map((signal) => (
        <div
          key={signal.id}
