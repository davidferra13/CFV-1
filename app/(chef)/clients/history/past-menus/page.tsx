...
export const metadata: Metadata = { title: 'Past Menus - ChefFlow' }
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-900">Past Menus</h1>
          <span className="bg-stone-100 text-stone-600 text-sm px-2 py-0.5 rounded-full">
            {eventMenus.length}
          </span>
        </div>
        <p className="text-stone-500 mt-1">Approved and archived event-specific menus you have served</p>
      </div>

      {eventMenus.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          <Card className="p-4">
            <p className="text-2xl font-bold text-stone-900">{eventMenus.length}</p>
      ...
      {eventMenus.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium mb-1">No past menus yet</p>
          <p className="text-stone-400 text-sm">Approved event menus will appear here once events are completed</p>
          <Link href="/culinary/menus" className="text-sm text-brand-600 hover:underline mt-2 block">
            Browse menu library
          </Link>
        </Card>
      ) : (
              ...
              {eventMenus.map(menu => (
                <TableRow key={menu.id}>
                  <TableCell className="font-medium">
                    <Link href={`/culinary/menus/${menu.id}`} className="text-brand-600 hover:text-brand-800 hover:underline">
                      {menu.name}
                    </Link>
                    {menu.description && (
                  ...
                  <TableCell className="text-stone-600 text-sm">{menu.cuisine_type || '—'}</TableCell>
                  <TableCell>
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${menu.status === 'locked' ? 'bg-green-100 text-green-700' : 'bg-stone-200 text-stone-500'}`}>
