    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-900">Client Notes</h1>
          <span className="bg-stone-100 text-stone-600 text-sm px-2 py-0.5 rounded-full">
            {clientsWithNotes.length} / {clients.length}
          </span>
        </div>
        <p className="text-stone-500 mt-1">Private vibe notes and context attached to each client profile</p>
      </div>
      ...
      {clientsWithNotes.length > 0 && (
        <div>
          <h2 className="text-sm font-semibold text-stone-700 mb-3">Clients with Notes</h2>
          <div className="space-y-3">
            {clientsWithNotes.map(client => (
              <Card key={client.id} className="p-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <Link href={`/clients/${client.id}`} className="font-medium text-brand-600 hover:text-brand-800 hover:underline">
                      {client.full_name}
                    </Link>
                    {client.email && (
                  ...
                  <Link href={`/clients/${client.id}`} className="text-xs text-brand-600 hover:underline shrink-0">
                    Edit
                  </Link>
                </div>
              </Card>
          ...
          <div className="flex flex-wrap gap-2">
            {clientsWithoutNotes.map(client => (
              <Link
                ...
                className="bg-stone-100 hover:bg-stone-200 text-stone-600 text-sm px-3 py-1.5 rounded-full transition-colors"
              >
                {client.full_name}
              </Link>
            ))}
      ...
      {clients.length === 0 && (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium mb-1">No clients yet</p>
