    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-900">Upcoming Touchpoints</h1>
          <span className="bg-stone-100 text-stone-600 text-sm px-2 py-0.5 rounded-full">
            {upcoming.length} in next 60 days
          </span>
        </div>
        <p className="text-stone-500 mt-1">Scheduled events in the next 60 days — planned client touchpoints</p>
      </div>
      ...
      {upcoming.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          ...
          <Card className="p-4">
            <p className="text-2xl font-bold text-stone-600">{next60.length}</p>
      ...
      {upcoming.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium mb-1">No upcoming events in the next 60 days</p>
          <p className="text-stone-400 text-sm">Confirmed and accepted events will appear here as your schedule fills up</p>
          <Link href="/events/upcoming" className="text-sm text-brand-600 hover:underline mt-2 block">
          ...
          <Table>
            ...
            <TableBody>
              {upcoming.map(event => {
                ...
                const urgencyClass = daysAway <= 7 ? 'text-red-600 font-semibold' : daysAway <= 14 ? 'text-amber-700' : 'text-stone-600'
                return (
