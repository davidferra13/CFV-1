...
function urgencyLabel(days: number): { label: string; style: string } {
  if (days > 180) return { label: 'Overdue', style: 'bg-red-100 text-red-700' }
  if (days > 90) return { label: 'At Risk', style: 'bg-amber-100 text-amber-700' }
  return { label: 'Check In', style: 'bg-sky-100 text-sky-700' }
}
...
export default async function FollowUpsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href="/clients/communication" className="text-sm text-stone-500 hover:text-stone-700">
          ← Communication
        </Link>
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-900">Follow-Ups</h1>
          <span className="bg-stone-100 text-stone-600 text-sm px-2 py-0.5 rounded-full">
            {followUpCandidates.length}
          </span>
        </div>
        <p className="text-stone-500 mt-1">Past clients worth reaching out to — sorted by time since last event</p>
      </div>
      ...
      {followUpCandidates.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          ...
          <Card className="p-4">
            <p className="text-2xl font-bold text-sky-700">{followUpCandidates.length - overdue - atRisk}</p>
      ...
      {followUpCandidates.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium mb-1">No follow-ups needed right now</p>
          <p className="text-stone-400 text-sm">Clients with completed events will appear here after 30 days of inactivity</p>
        </Card>
      ) : (
        <Card>
          <Table>
              ...
              {followUpCandidates.map(client => {
                ...
                return (
                  <TableRow key={client.id}>
                    ...
                    <TableCell className="text-stone-600 text-sm">{days} days</TableCell>
                    ...
                    <TableCell>
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${urgency.style}`}>
                        {urgency.label}
                      </span>
                    </TableCell>
                  </TableRow>
