...
export default async function ClientInsightsPage() {
  ...
  const [clients, engagementStats] = await Promise.all([
    getClientsWithStats(),
    getEngagementStats().catch(() => ({ activeToday: 0, activeThisWeek: 0, totalEventsThisWeek: 0 })),
  ])

  const sortedBySpend = [...clients].sort((a, b) => (b.totalSpentCents ?? 0) - (a.totalSpentCents ?? 0))
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold text-stone-900 mt-1">Client Insights</h1>
        <p className="text-stone-500 mt-1">Intelligence on your clientele — who&apos;s valuable, who&apos;s frequent, who needs attention</p>
      </div>

      <div className="grid grid-cols-2 gap-4">
        <Card className="p-4">
          <p className="text-xs text-stone-400 uppercase tracking-wide mb-1">Top Client by Revenue</p>
          {topClient ? (
            <>
              <Link href={`/clients/${topClient.id}`} className="text-lg font-bold text-stone-900 hover:text-brand-600">
                {topClient.full_name}
              </Link>
              <p className="text-sm text-green-700 mt-0.5">{formatCurrency(topClient.totalSpentCents ?? 0)} lifetime</p>
            </>
          ) : (
        ...
        <Card className="p-4">
          <p className="text-xs text-stone-400 uppercase tracking-wide mb-1">Most Frequent Client</p>
          {mostFrequent && (mostFrequent.totalEvents ?? 0) > 0 ? (
            <>
              <Link href={`/clients/${mostFrequent.id}`} className="text-lg font-bold text-stone-900 hover:text-brand-600">
        ...
        <Card className="p-4">
          <p className="text-xs text-stone-400 uppercase tracking-wide mb-1">Average Lifetime Spend</p>
          <p className="text-2xl font-bold text-stone-900">{formatCurrency(Math.round(avgSpend))}</p>
        ...
        <Link href="/clients/presence" className="col-span-2">
          <Card className="p-4 hover:bg-stone-50 transition-colors">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-stone-400 uppercase tracking-wide mb-1">Active on Portal Today</p>
                <p className="text-2xl font-bold text-stone-900">{engagementStats.activeToday}</p>
