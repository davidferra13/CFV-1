    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-900">Allergies</h1>
          <span className="bg-red-100 text-red-700 text-sm px-2 py-0.5 rounded-full">
            {clientsWithAllergies.length} clients
          </span>
        </div>
        <p className="text-stone-500 mt-1">Food allergy records across your entire clientele — critical for event planning</p>
      </div>

      {topAllergies.length > 0 && (
        <Card className="p-4">
          <h2 className="text-sm font-semibold text-stone-700 mb-3">Most Common Allergies</h2>
          <div className="flex flex-wrap gap-2">
            {topAllergies.map(([allergy, count]) => (
              <span key={allergy} className="bg-red-100 text-red-800 text-sm px-3 py-1 rounded-full">
      ...
      {clientsWithAllergies.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium mb-1">No allergies on file</p>
          <p className="text-stone-400 text-sm">Add allergy information to client profiles to see it here</p>
        </Card>
      ) : (
        <Card>
          <Table>
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {(client.allergies as string[]).map(a => (
                        <span key={a} className="bg-red-100 text-red-800 text-xs font-medium px-2 py-0.5 rounded-full">
                          {a}
                        </span>
                      ))}
                    </div>
