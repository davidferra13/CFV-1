    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-900">Dietary Restrictions</h1>
          <span className="bg-stone-100 text-stone-600 text-sm px-2 py-0.5 rounded-full">
            {clientsWithRestrictions.length} clients
          </span>
        </div>
        <p className="text-stone-500 mt-1">Vegetarian, vegan, kosher, halal, and other dietary requirements on file</p>
      </div>

      {topRestrictions.length > 0 && (
        <Card className="p-4">
          <h2 className="text-sm font-semibold text-stone-700 mb-3">Most Common</h2>
          <div className="flex flex-wrap gap-2">
            {topRestrictions.map(([restriction, count]) => (
              <span key={restriction} className="bg-amber-100 text-amber-800 text-sm px-3 py-1 rounded-full">
      ...
      {clientsWithRestrictions.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium mb-1">No dietary restrictions on file</p>
          <p className="text-stone-400 text-sm">Add restrictions to client profiles to see them here</p>
        </Card>
      ) : (
        <Card>
          <Table>
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {(client.dietary_restrictions as string[]).map(r => (
                        <span key={r} className="bg-amber-100 text-amber-800 text-xs px-2 py-0.5 rounded-full">
                          {r}
                        </span>
                      ))}
                    </div>
