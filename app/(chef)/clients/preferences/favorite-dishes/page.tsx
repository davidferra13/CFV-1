    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-900">Favorite Dishes</h1>
          <span className="bg-stone-100 text-stone-600 text-sm px-2 py-0.5 rounded-full">
            {clientsWithFavorites.length} clients
          </span>
        </div>
        <p className="text-stone-500 mt-1">Dishes and flavors each client loves — helpful for menu planning</p>
      </div>

      {popularDishes.length > 0 && (
        <Card className="p-4">
          <h2 className="text-sm font-semibold text-stone-700 mb-3">Most Requested Dishes</h2>
          <div className="flex flex-wrap gap-2">
            {popularDishes.map(([dish, count]) => (
              <span key={dish} className="bg-green-100 text-green-800 text-sm px-3 py-1 rounded-full">
                {dish} {count > 1 && <span className="text-emerald-600 font-semibold">×{count}</span>}
              </span>
            ))}
          </div>
        </Card>
      ...
      {clientsWithFavorites.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium mb-1">No favorite dishes on file</p>
          <p className="text-stone-400 text-sm">Add taste preferences to client profiles to see them here</p>
        </Card>
      ) : (
        <Card>
          <Table>
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {(client.favorite_dishes as string[]).map(d => (
                        <span key={d} className="bg-green-100 text-green-800 text-xs px-2 py-0.5 rounded-full">
                          {d}
                        </span>
                      ))}
                    </div>
