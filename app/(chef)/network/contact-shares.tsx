...
export function ContactShares({ connections, shares: initialShares }: ContactSharesProps) {
  ...
  const [helpType, setHelpType] = useState<'date_help' | 'inquiry_help' | 'full_handover'>('date_help')
  ...
  const received = useMemo(
    () => shares.filter((share) => share.direction === 'received'),
    [shares]
  )
  const sent = useMemo(
    () => shares.filter((share) => share.direction === 'sent'),
    [shares]
  )
  ...
  return (
    <div className="space-y-4">
      <form onSubmit={handleShareSubmit} className="rounded-lg border border-stone-200 p-4 bg-white space-y-3">
        <p className="text-sm font-semibold text-stone-900">Share A Contact</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1.5">Request Type</label>
            <select
              value={helpType}
              onChange={(event) => setHelpType(event.target.value as 'date_help' | 'inquiry_help' | 'full_handover')}
          ...
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1.5">Contact Name</label>
            <input
              type="text"
              value={contactName}
          ...
          {helpType === 'inquiry_help' && (
            <>
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1.5">Inquiry ID</label>
                <input
                  type="text"
                  value={inquiryId}
              ...
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1.5">Inquiry Summary</label>
                <input
                  type="text"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <section className="rounded-lg border border-stone-200 p-4 bg-white">
          <h4 className="text-sm font-semibold text-stone-900">Incoming Shares</h4>
          {received.length === 0 ? (
        ...
        <section className="rounded-lg border border-stone-200 p-4 bg-white">
          <h4 className="text-sm font-semibold text-stone-900">Outgoing Shares</h4>
          {sent.length === 0 ? (
  ...
  return (
    <article className="rounded-lg border border-stone-200 p-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-medium text-stone-900">{share.contact_name}</p>
      ...
      <p className="mt-2 text-sm text-stone-700 whitespace-pre-wrap">{share.details}</p>

      {share.response_note && (
        <p className="mt-2 text-xs text-stone-500">
          Response note: {share.response_note}
        </p>
      )}
      ...
      {!readonly && share.status === 'open' && (
        <div className="mt-3 space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" variant="ghost" onClick={() => onRespond(share.id, 'passed')} disabled={isPending}>
              I Can&apos;t Take It
            </Button>
          </div>
        </div>
...
function StatusBadge({ status }: { status: 'open' | 'accepted' | 'passed' }) {
  if (status === 'accepted') {
    return <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700">Accepted</span>
  }
  if (status === 'passed') {
    return <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-amber-50 text-amber-700">Passed</span>
  }
  return <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-blue-50 text-blue-700">Open</span>
}
