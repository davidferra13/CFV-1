  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
        <StatCard
          label="Inquiries"
          value={insights.inquiries_total}
          sub={`${insights.inquiries_shared} shared / ${insights.inquiries_unshared} unshared`}
        />
        <StatCard
          label="Dinners/Events"
          value={insights.dinners_total}
          sub={`${insights.dinner_dates_shared} dates shared / ${insights.dinner_dates_unshared} unshared`}
        />
        <StatCard
          label="Clients"
          value={insights.clients_total}
          sub={`${insights.clients_shared} shared / ${insights.clients_unshared} unshared`}
        />
        <StatCard
          label="Direct Shares"
          value={insights.shares_sent_total + insights.shares_received_total}
          sub={`${insights.shares_open} open, ${insights.shares_accepted} accepted, ${insights.shares_passed} passed`}
        />
      </div>
          ...
          {insights.unshared_inquiry_ids.length === 0 ? (
            ...
            <ul className="mt-3 space-y-1.5">
              {insights.unshared_inquiry_ids.map((inquiryId) => (
                <li
                  key={inquiryId}
                  className="text-xs text-stone-400 font-mono bg-stone-800 rounded px-2 py-1"
                >
                  {inquiryId}
                </li>
              ))}
          ...
          {insights.unshared_client_names.length === 0 ? (
            ...
            <ul className="mt-3 space-y-1.5">
              {insights.unshared_client_names.map((name) => (
                <li key={name} className="text-sm text-stone-300 bg-stone-800 rounded px-2 py-1">
                  {name}
                </li>
              ))}
            </ul>
