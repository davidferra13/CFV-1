...
export default async function EventsGeneratedPage() {
  ...
  const activePartners = leaderboard.filter(p => p.event_count > 0).sort((a, b) => b.event_count - a.event_count)
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-900">Events Generated</h1>
      ...
      {activePartners.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium mb-1">No partner-generated events yet</p>
          <p className="text-stone-400 text-sm">Events will appear here once partners are linked to inquiries that become events</p>
        </Card>
      ) : (
        <Card>
          <Table>
                ...
                <TableRow key={partner.id}>
                  ...
                  <TableCell>
                    <Link href={`/partners/${partner.id}`}>
                      <button type="button" className="text-xs text-brand-600 hover:underline">View</button>
                    </Link>
                  </TableCell>
                </TableRow>
              ))}
