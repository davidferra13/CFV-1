          ...
          <div className="mt-5 space-y-4">
            {locationChangeRequests.map((request) => {
              ...
              const changedFields = location
                ? getPartnerLocationProposalChangedFields(location, request.requested_payload as any)
                : []

              return (
                <div key={request.id} className="rounded-2xl border border-stone-800 bg-stone-950/70 p-5">
                  ...
                  {request.review_note && (
                    <p className="mt-3 text-xs text-stone-500">Review note: {request.review_note}</p>
                  )}
