...
export function RewardActions({ reward }: { reward: LoyaltyReward }) {
  ...
  function handleSave() {
    ...
    const input: UpdateRewardInput = {
      ...
      reward_value_cents:
        rewardType === 'discount_fixed' ? Math.round(parseFloat(valueDollars) * 100) : null,
      reward_percent: rewardType === 'discount_percent' ? parseInt(valuePercent) : null,
    }
  ...
  if (mode === 'idle') {
    return (
      <div className="flex items-center gap-2">
        <Button variant="ghost" size="sm" onClick={() => setMode('editing')} disabled={isPending}>
          Edit
        </Button>
        <Button
          variant="ghost"
  ...
  // Inline edit form
  return (
    <div className="mt-4 p-4 rounded-xl border border-brand-700 bg-brand-950/30 space-y-4">
      {error && <p className="text-sm text-red-600 bg-red-950 px-3 py-2 rounded-lg">{error}</p>}

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="sm:col-span-2">
          <label className="block text-xs font-medium text-stone-400 mb-1">Reward Name</label>
          <Input
            value={name}
        ...
        <div>
          <label className="block text-xs font-medium text-stone-400 mb-1">Points Required</label>
          <Input
            type="number"
            min="1"
            value={points}
      ...
      <div>
        <label className="block text-xs font-medium text-stone-400 mb-2">Reward Type</label>
