...
export function CreateRaffleForm({ chefId, monthStart, monthEnd }: Props) {
  ...
  const defaultData = useMemo(() => ({ prize: '', prizeTopScorer: '', prizeMostDedicated: '' }), [])
  const currentData = useMemo(
    () => ({ prize, prizeTopScorer, prizeMostDedicated }),
    [prize, prizeTopScorer, prizeMostDedicated]
  )
  ...
  function applyDraftData(data: Record<string, unknown>) {
    if (typeof data.prize === 'string') setPrize(data.prize)
    if (typeof data.prizeTopScorer === 'string') setPrizeTopScorer(data.prizeTopScorer)
    if (typeof data.prizeMostDedicated === 'string') setPrizeMostDedicated(data.prizeMostDedicated)
  }
