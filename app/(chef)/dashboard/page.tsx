...
import { TieredRail, TieredRailSkeleton } from '@/components/rail/tiered-rail'
...
import { AutomationFeedCard } from './_sections/automation-feed'
...
async function AutoGeneratedDraftsSection() {
...
async function HoursLogSection() {
...
async function RecipeCaptureSection() {
...
// ─── Today's Schedule Section (ambient) ───────────────────
async function TodaysScheduleSection() {
...
// ─── Staff Today Section (ambient) ────────────────────────
async function StaffTodaySection() {
  const data = await safe('staffToday', getStaffToday, null)
  if (!data) return null
  return <StaffTodayWidget data={data} />
}

// ─── Revenue Goal Section (ambient) ───────────────────────
async function RevenueGoalSection() {
...
export default async function ChefDashboard() {
  ...
  return (
    <div className="space-y-10">
      ...
      {/* ============================================ */}
      {/* RAIL SYSTEM (4-tier: Critical, Action,       */}
      ...
      {/* Daily Plan progress (shared state with /daily) */}
      {dailyPlanStats && dailyPlanStats.totalItems > 0 && (
        <DailyPlanBanner stats={dailyPlanStats} />
      )}

      {/* Onboarding banner, profile warning, checklist */}
      <OnboardingBanner />
      ...
      {/* ============================================ */}
      {/* AMBIENT LAYER (formula-scored order)          */}
      {/* 1. Hero (above), 2. Today's Schedule,        */}
      {/* 3. Alert Grid, 4. Staff Today,               */}
      {/* 5. Prep Pressure, 6. This Week,              */}
      {/* 7. Quick Notes, 8. Revenue Goal,             */}
      {/* 9. ChefTips, 10. Business Health             */}
      {/* ============================================ */}
      ...
      {/* 6. This Week (schedule, saturation, pipeline, financial, calendar, etc.) */}
      ...
      {/* 10. Business Health (collapsed by default) */}
...
async function PortfolioSummarySection() {
...
// ─── Tiered Rail Section (unified 4-tier) ─────────────
async function TieredRailSection({ queuePromise }: { queuePromise: Promise<PriorityQueue> }) {
  const user = await requireChef()
