  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
        ...
        rows={data.rows.map((r) => [r.category, r.period, formatCents(r.totalCents), String(r.count)])}
      />
    </div>
  )
}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <SummaryCard
          ...
          value={formatPercent(data.eventToCompletedRate)}
        />
        <SummaryCard
...
function SummaryCard({
  ...
  return (
    <div className="rounded-lg border border-stone-700 bg-stone-800 p-3">
      ...
      <p className={`text-lg font-semibold mt-1 ${highlight ? 'text-emerald-400' : 'text-stone-100'}`}>
        {value}
      </p>
    </div>
  )
