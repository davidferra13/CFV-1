...
export default async function ExpensesPage({
  ...
  const grouped = new Map<string, { occasion: string; eventDate: string; clientName: string; expenses: typeof expenses }>()
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4">
          ...
          <p className="text-2xl font-bold text-stone-900 mt-1">{formatCurrency(totalThisMonth)}</p>
        ...
        <Card className="p-4">
          ...
          <p className="text-2xl font-bold text-stone-900 mt-1">{formatCurrency(monthlySummary.totalProfitCents)}</p>
        ...
        <Card className="p-4">
          ...
          <p className="text-2xl font-bold text-stone-900 mt-1">{monthlySummary.averageFoodCostPercent}%</p>
            ...
            {Object.entries(categoryTotals)
              ...
              .map(([cat, total]) => (
                <div key={cat} className="flex items-center gap-2">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${getCategoryColor(cat)}`}>
                    {getCategoryLabel(cat)}
                  </span>
                  <span className="text-sm font-medium text-stone-700">{formatCurrency(total)}</span>
                </div>
              ))}
          </div>
        </Card>
      ...
      <div className="space-y-2">
        <div className="flex flex-wrap gap-2 items-center">
          <Link href="/expenses">
            <Button variant={!searchParams.category ? 'primary' : 'secondary'} size="sm">All</Button>
          </Link>
        </div>
      ...
      {expenses.length === 0 ? (
        <Card className="p-8 text-center">
          ...
          <Link href="/expenses/new" className="text-brand-600 hover:underline text-sm mt-2 inline-block">
                  ...
                  <TableRow key={expense.id}>
                    ...
                    <TableCell>
                      <div>
                        ...
                        {expense.vendor_name && (
                          <span className="text-stone-500 text-xs ml-1">at {expense.vendor_name}</span>
                        )}
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="font-medium">{formatCurrency(expense.amount_cents)}</TableCell>
                    <TableCell>
                      {expense.is_business ? (
                        <span className="text-xs text-stone-600">Business</span>
                      ) : (
