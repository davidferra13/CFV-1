...
export default function AarError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    reportClientBoundaryError(error, { boundary: 'aar', digest: error.digest })
    console.error('[Aar Error]', error)
  }, [error])
  ...
  return (
    <div className="flex items-center justify-center min-h-[400px] px-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <CardTitle className="text-xl">Could not load aar</CardTitle>
          <p className="text-sm text-muted-foreground mt-1">
