...
export default async function AnalyticsHubPage() {
  ...
  const [
    ...
    getClientRetentionStats().catch(() => ({
      activeClients: 0,
      repeatClients: 0,
      repeatBookingRate: 0,
      retentionRate: 0,
    })),
    getClientChurnStats().catch(() => ({
      totalAtRisk: 0,
      dormantCount: 0,
      churnRate: 0,
      avgDaysSinceLastEvent: 0,
    })),
    getRevenueConcentration().catch(() => ({
      top5Clients: [],
      top5SharePercent: 0,
      herfindahlIndex: 0,
    })),
    getClientAcquisitionStats(start, end).catch(() => ({
      newClientsThisPeriod: 0,
      totalMarketingSpendCents: 0,
      cacCents: 0,
      cacRatio: 0,
    })),
    getReferralConversionStats().catch(() => ({
      referredInquiries: 0,
    ...
    getGhostRateStats().catch(() => ({
      totalInquiries: 0,
      ghosted: 0,
      ghostRate: 0,
      avgDaysToGhost: 0,
    })),
    getLeadTimeStats().catch(() => ({
      avgLeadTimeDays: 0,
      avgSalesCycleDays: 0,
    ...
    getAvgInquiryResponseTime().catch(() => ({
    ...
    getRevenuePerUnitStats(start, end).catch(() => ({
    ...
    getTrueLaborCostStats(start, end).catch(() => ({
      ownerHoursCents: 0,
      staffCostCents: 0,
      totalLaborCents: 0,
    ...
    getComplianceStats(start, end).catch(() => ({
      onTimeStartRate: 0,
    ...
    getCulinaryOperationsStats().catch(() => ({
