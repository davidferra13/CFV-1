...
import { computeDashboardKPIs, solveRevenueClosure, yearRange } from '@/lib/analytics/revenue-engine'
...
export default async function FinanceGoalsPage() {
  ...
  const annualTargetCents =
    snapshot.annual?.targetCents ?? snapshot.monthly.targetCents * 12
  const annualBookedCents =
    snapshot.annual?.realizedCents ?? kpis.totalRevenue.value
  const annualProgressPercent =
    annualTargetCents > 0
      ? Math.round((annualBookedCents / annualTargetCents) * 100)
      : 0
  ...
  return (
    <div className="space-y-8 max-w-4xl">
      ...
      <div className="flex items-center gap-2 text-sm text-stone-500">
        <Link href="/finance" className="hover:text-stone-700">Finance</Link>
        <span>/</span>
        <span className="text-stone-900">Revenue Goals</span>
      </div>

      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-stone-900">Revenue Goals</h1>
          <p className="text-stone-500 mt-1">
            Track your annual target, see year-to-date progress, and get strategies to close any gap.
          </p>
        </div>
        <Link
          href="/goals"
          className="inline-flex items-center gap-2 rounded-md border border-stone-300 bg-white px-4 py-2 text-sm font-medium text-stone-700 shadow-sm hover:bg-stone-50 transition-colors whitespace-nowrap"
        >
          Manage Goals
        </Link>
      </div>
      ...
      <Card>
        <CardContent className="pt-6">
          <h2 className="text-lg font-semibold text-stone-900 mb-4">Set Annual Revenue Target</h2>
      ...
      <Card>
        <CardContent className="pt-6 space-y-4">
          <h2 className="text-lg font-semibold text-stone-900">Annual Progress</h2>
          ...
          <div className="flex items-end justify-between">
            ...
            <div className="text-right">
              <p className="text-2xl font-bold text-stone-700">{annualProgressPercent}%</p>
          ...
          <div className="flex gap-6 text-sm text-stone-600 pt-1">
            <span>
              <span className="font-medium">{formatCurrency(Math.max(0, annualTargetCents - annualBookedCents))}</span>{' '}
              remaining gap
            </span>
            <span>
        ...
        <CardContent className="pt-6 space-y-3">
          ...
          {snapshot.dinnersNeededThisMonth > 0 && (
            <p className="text-sm text-stone-600">
              ...
              {snapshot.dinnersNeededThisMonth === 1 ? 'booking' : 'bookings'} this month to hit target.
            </p>
          )}
        </CardContent>
      </Card>
      ...
      {/* YTD KPIs */}
      <div>
        <h2 className="text-lg font-semibold text-stone-900 mb-3">Year-to-Date KPIs</h2>
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
          <Card className="p-4">
            <p className="text-2xl font-bold text-stone-900">{formatCurrency(kpis.totalRevenue.value)}</p>
            <p className="text-xs text-stone-500 mt-1">Revenue collected</p>
          </Card>
          <Card className="p-4">
            <p className="text-2xl font-bold text-stone-900">{formatCurrency(kpis.totalBookedValue.value)}</p>
            <p className="text-xs text-stone-500 mt-1">Total booked value</p>
          </Card>
          <Card className="p-4">
            <p className="text-2xl font-bold text-stone-900">{formatCurrency(kpis.averageEventValue.value)}</p>
            <p className="text-xs text-stone-500 mt-1">Avg event value</p>
          </Card>
          <Card className="p-4">
            <p className="text-2xl font-bold text-stone-900">{kpis.eventsCompleted.value}</p>
            <p className="text-xs text-stone-500 mt-1">Events completed</p>
          </Card>
          <Card className="p-4">
            <p className="text-2xl font-bold text-stone-900">{kpis.inquiriesCount.value}</p>
            <p className="text-xs text-stone-500 mt-1">Inquiries received</p>
          </Card>
          <Card className="p-4">
            <p className="text-2xl font-bold text-stone-900">{kpis.conversionRate.value}%</p>
      ...
      {closureResult.gap > 0 && (
        <div>
          <h2 className="text-lg font-semibold text-stone-900 mb-1">Close the Gap</h2>
          <p className="text-sm text-stone-500 mb-3">
            You need {formatCurrency(closureResult.gap)} more to hit your annual target.
            Here are ways to get there:
          </p>
          <div className="space-y-3">
            {closureResult.strategies.map((s, i) => (
              <Card key={i} className={`p-4 ${s.achievable ? 'border-green-200 bg-green-50' : 'border-stone-200'}`}>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-medium text-stone-900">{s.strategy}</p>
                    <p className="text-sm text-stone-500 mt-0.5">
      ...
      {snapshot.recommendations.length > 0 && (
        <div>
          <h2 className="text-lg font-semibold text-stone-900 mb-3">Recommendations</h2>
            ...
            {snapshot.recommendations.slice(0, 5).map((rec) => (
              <Card key={rec.id} className="p-4">
                <div className="flex items-start gap-3">
                  ...
                  />
                  <div>
                    <p className="font-medium text-stone-900 text-sm">{rec.title}</p>
