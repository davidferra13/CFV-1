...
export default async function RentalsEquipmentExpensesPage() {
  await requireChef()
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-100">Rentals &amp; Equipment</h1>
          <span className="bg-stone-700 text-stone-300 text-sm px-2 py-0.5 rounded-full">
            {expenses.length}
          </span>
        </div>
        <p className="text-stone-500 mt-1">Equipment, supplies, venue rentals, and uniforms</p>
      </div>
      ...
      {expenses.length === 0 ? (
        <div className="text-center py-16 text-stone-400">
          <Wrench className="mx-auto h-12 w-12 mb-4 text-stone-500" />
          <p className="text-lg font-medium text-stone-300">No rentals or equipment expenses yet</p>
          <p className="text-sm mt-1 max-w-md mx-auto">
            Track equipment purchases, supply runs, venue rentals, and uniform costs here.
          </p>
        </div>
      ) : (
        <Card>
