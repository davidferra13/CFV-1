    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-100">Adjustments</h1>
          <span className="bg-stone-800 text-stone-400 text-sm px-2 py-0.5 rounded-full">
            {allAdjustments.length}
          </span>
        </div>
        <p className="text-stone-500 mt-1">
          Credits, add-ons, and manual adjustments to event totals
        </p>
      </div>

      <div className="grid grid-cols-2 gap-4">
        <Card className="p-4">
          <p className="text-2xl font-bold text-sky-200">{formatCurrency(totalCredits)}</p>
          <p className="text-sm text-stone-500 mt-1">Credits issued ({credits.length})</p>
        </Card>
        <Card className="p-4">
          <p className="text-2xl font-bold text-blue-200">{formatCurrency(totalAddOns)}</p>
      ...
      {allAdjustments.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-stone-400 font-medium">No adjustments recorded</p>
          <p className="text-stone-400 text-sm mt-1">
            Credits and add-ons applied to events will appear here
          </p>
        </Card>
      ) : (
        <Card>
            ...
            <TableBody>
              {allAdjustments.map((entry) => (
                <TableRow key={entry.id}>
                  ...
                  <TableCell>
                    <span
                      className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${entry.entry_type === 'credit' ? 'bg-sky-900 text-sky-200' : 'bg-blue-900 text-blue-200'}`}
                    >
                  ...
                  <TableCell className="text-sm font-semibold text-stone-100">
