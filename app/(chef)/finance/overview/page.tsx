...
const VIEWS = [
  { href: '/finance/overview/revenue-summary', label: 'Revenue Summary', icon: '💵', description: 'Completed events and revenue per client' },
  { href: '/finance/overview/outstanding-payments', label: 'Outstanding Payments', icon: '⏳', description: 'Events with unpaid balances' },
  { href: '/finance/overview/cash-flow', label: 'Cash Flow', icon: '📉', description: 'Revenue vs. expenses by month' },
]
...
export default async function FinanceOverviewPage() {
  ...
  const now = new Date()
  const outstandingEvents = events.filter(e =>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold text-stone-900 mt-1">Overview</h1>
        <p className="text-stone-500 mt-1">High-level financial health at a glance</p>
      </div>

      <div className="grid grid-cols-2 gap-4">
        <Card className="p-4">
          <p className="text-xs text-stone-400 uppercase tracking-wide mb-1">Total Revenue Collected</p>
          <p className="text-3xl font-bold text-stone-900">{formatCurrency(summary.totalRevenueCents)}</p>
          <p className="text-sm text-stone-500 mt-1">Net: {formatCurrency(summary.netRevenueCents)} after refunds</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs text-stone-400 uppercase tracking-wide mb-1">Total Business Expenses</p>
        ...
        <Card className="p-4">
          <p className="text-xs text-stone-400 uppercase tracking-wide mb-1">Completed Event Revenue</p>
        ...
        <Card className="p-4">
          <p className="text-xs text-stone-400 uppercase tracking-wide mb-1">Outstanding Balances</p>
        ...
        {VIEWS.map(v => (
          <Link key={v.href} href={v.href}>
            <Card className="p-5 hover:shadow-md transition-shadow cursor-pointer h-full">
              ...
              <h2 className="font-semibold text-stone-900">{v.label}</h2>
