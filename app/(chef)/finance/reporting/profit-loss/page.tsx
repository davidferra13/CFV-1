...
export default async function ProfitLossPage({
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <Link href="/finance/reporting" className="text-sm text-stone-500 hover:text-stone-700">
          ← Reporting
        </Link>
        <div className="flex items-center justify-between mt-1">
          <div>
            <h1 className="text-3xl font-bold text-stone-900">Profit &amp; Loss Statement</h1>
            <p className="text-stone-500 mt-1">
              Full revenue, expenses, and net profit for {validYear}
            </p>
          </div>
          <ProfitLossClientControls
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <Card className="p-5">
          ...
          <p className={`text-2xl font-bold ${pl.netProfitCents >= 0 ? 'text-stone-900' : 'text-red-600'}`}>
        ...
        <Card className="p-5">
          ...
          <p className={`text-2xl font-bold ${pl.profitMarginPercent >= 0 ? 'text-stone-900' : 'text-red-600'}`}>
            {pl.profitMarginPercent}%
          </p>
          {pl.totalTipsCents > 0 && (
            <p className="text-xs text-stone-400 mt-1">
              Tips: {formatCurrency(pl.totalTipsCents)}
            </p>
          )}
        </Card>
      </div>
          ...
          <TableBody>
            {monthlyData.map(row => (
              <TableRow key={row.key}>
                <TableCell className="text-stone-700 font-medium text-sm">{row.month}</TableCell>
                <TableCell className="text-stone-900 font-semibold text-sm">
                  {row.revenueCents > 0 ? formatCurrency(row.revenueCents) : <span className="text-stone-300">—</span>}
                </TableCell>
                <TableCell>
                  <div className="w-full bg-stone-100 rounded-full h-2.5">
                    <div
                      className="bg-green-500 h-2.5 rounded-full"
                      style={{ width: `${Math.round((row.revenueCents / maxMonthlyRevenue) * 100)}%` }}
                    />
                  </div>
                </TableCell>
              </TableRow>
      ...
      <Card className="p-5">
        ...
        {expenseRows.length === 0 ? (
          <p className="text-stone-400 text-sm text-center py-6">No expenses recorded for {validYear}</p>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              ...
              <TableRow>
                <TableCell className="font-bold text-stone-900 text-sm">Total</TableCell>
        ...
        <div className="space-y-2 max-w-sm">
          ...
          {pl.totalRefundsCents > 0 && (
            <div className="flex justify-between">
              ...
              <span className="text-sm font-semibold text-red-500">&minus;{formatCurrency(pl.totalRefundsCents)}</span>
            </div>
          )}
          <div className="flex justify-between">
            <span className="text-sm font-semibold text-stone-700">Net revenue</span>
            <span className="text-sm font-bold text-stone-900">{formatCurrency(pl.netRevenueCents)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-sm text-stone-600">Total expenses</span>
            <span className="text-sm font-semibold text-red-500">&minus;{formatCurrency(pl.totalExpensesCents)}</span>
          </div>
          <div className="flex justify-between pt-2 border-t border-stone-200">
            <span className="text-sm font-bold text-stone-900">Net profit</span>
            <span className={`text-sm font-bold ${pl.netProfitCents >= 0 ? 'text-green-700' : 'text-red-600'}`}>
              {formatCurrency(pl.netProfitCents)}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-sm text-stone-600">Profit margin</span>
            <span className={`text-sm font-semibold ${pl.profitMarginPercent >= 0 ? 'text-stone-700' : 'text-red-600'}`}>
              {pl.profitMarginPercent}%
            </span>
          </div>
        </div>
