  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold text-stone-900 mt-1">Tax Summary {currentYear}</h1>
        <p className="text-stone-500 mt-1">Business income and deductible expense summary — consult your CPA for tax advice</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card className="p-4">
          <p className="text-2xl font-bold text-green-700">{formatCurrency(summary.totalRevenueCents)}</p>
        ...
        <Card className="p-4">
          <p className={`text-2xl font-bold ${estimatedProfit >= 0 ? 'text-stone-900' : 'text-red-600'}`}>
      ...
      <div>
        <h2 className="text-lg font-semibold text-stone-900 mb-3">Business Expense Breakdown ({currentYear})</h2>

        {sortedCategories.length === 0 ? (
          <Card className="p-8 text-center">
            <p className="text-stone-600 font-medium">No business expenses recorded for {currentYear}</p>
          </Card>
        ) : (
          <Card>
            <Table>
              ...
              <TableBody>
                {sortedCategories.map(([cat, total]) => (
                  <TableRow key={cat}>
                    <TableCell>
                      <span className={`text-xs px-2 py-0.5 rounded-full ${EXPENSE_CATEGORIES[cat as keyof typeof EXPENSE_CATEGORIES]?.color ?? 'bg-stone-100 text-stone-600'}`}>
                    ...
                    <TableCell className="text-stone-900 font-semibold text-sm">{formatCurrency(total)}</TableCell>
                  </TableRow>
                ))}
                <TableRow className="border-t-2 border-stone-200">
                  <TableCell className="font-bold text-stone-900">Total</TableCell>
                  <TableCell className="text-stone-600 text-sm">{allExpenses.length}</TableCell>
                  <TableCell className="font-bold text-red-600">{formatCurrency(totalBusinessExpenses)}</TableCell>
                </TableRow>
              </TableBody>
            </Table>
          </Card>
