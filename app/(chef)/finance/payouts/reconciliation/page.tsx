...
export default async function ReconciliationPage() {
  ...
  const [events, paymentEntries, depositEntries, installmentEntries, finalEntries] = await Promise.all([
    getEvents(),
  ...
  for (const entry of [...paymentEntries, ...depositEntries, ...installmentEntries, ...finalEntries]) {
    if (entry.event_id) {
      paymentsByEvent.set(entry.event_id, (paymentsByEvent.get(entry.event_id) ?? 0) + entry.amount_cents)
    }
  }

  // Paid-state events with their invoice value vs recorded payments
  const paidEvents = events.filter(e => ['paid', 'confirmed', 'in_progress', 'completed'].includes(e.status))
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold text-stone-900 mt-1">Reconciliation</h1>
        <p className="text-stone-500 mt-1">Compare event invoice values against recorded ledger payments</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Card className="p-4 bg-stone-50 border-stone-200">
          <p className="text-2xl font-bold text-stone-600">{unrecorded.length}</p>
      ...
      {unrecorded.length > 0 && (
        <>
          <h2 className="text-lg font-semibold text-stone-900">No Payment Recorded</h2>
          <Card>
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {unrecorded.map(event => (
                  <TableRow key={event.id}>
                    ...
                    <TableCell>
                      <span className="text-xs px-2 py-0.5 rounded-full bg-stone-100 text-stone-600 capitalize">{event.status.replace(/_/g, ' ')}</span>
                    </TableCell>
                    <TableCell className="text-stone-900 font-semibold text-sm">{formatCurrency(event.quoted_price_cents ?? 0)}</TableCell>
                    <TableCell className="text-stone-500 text-sm">—</TableCell>
                    <TableCell className="text-red-600 font-semibold text-sm">{formatCurrency(event.quoted_price_cents ?? 0)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
      ...
      {partial.length > 0 && (
        <>
          <h2 className="text-lg font-semibold text-stone-900">Partially Recorded</h2>
          <Card>
            <Table>
              <TableHeader>
                <TableRow>
                ...
                {partial.map(event => {
                  ...
                  return (
                    <TableRow key={event.id}>
                      ...
                      <TableCell className="text-stone-900 font-semibold text-sm">{formatCurrency(invoiced)}</TableCell>
                      <TableCell className="text-green-700 font-semibold text-sm">{formatCurrency(recorded)}</TableCell>
                      <TableCell className="text-amber-700 font-semibold text-sm">{formatCurrency(invoiced - recorded)}</TableCell>
                      <TableCell>
      ...
      {reconciled.length > 0 && partial.length === 0 && unrecorded.length === 0 && (
        <Card className="p-12 text-center">
          <p className="text-emerald-600 font-medium text-lg">All paid events are fully reconciled</p>
          <p className="text-stone-400 text-sm mt-1">Ledger entries match invoice values for all {reconciled.length} paid events</p>
        </Card>
      )}
    </div>
  )
