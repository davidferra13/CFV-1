    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-900">Failed &amp; Pending Payments</h1>
          <span className={`text-sm px-2 py-0.5 rounded-full ${pastDue.length > 0 ? 'bg-amber-100 text-amber-700' : 'bg-stone-100 text-stone-600'}`}>
            {stalled.length}
          </span>
        </div>
        <p className="text-stone-500 mt-1">Events in accepted state awaiting payment — past-due events may indicate stalled or failed payments</p>
      </div>

      <Card className="p-4 bg-amber-50 border-amber-200">
        <p className="text-sm text-amber-800 font-medium">Note on failed payments</p>
        <p className="text-sm text-amber-700 mt-1">
          ChefFlow records successful payment ledger entries only. For detailed Stripe failed charge data,
          check your <strong>Stripe Dashboard → Payments → Failed</strong>. Events listed here are in
          &quot;accepted&quot; state with no recorded payment yet.
        </p>
      </Card>
      ...
      {pastDue.length > 0 && (
        <>
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-semibold text-stone-900">Past-Due</h2>
            <span className="bg-red-100 text-red-600 text-xs px-2 py-0.5 rounded-full">{pastDue.length}</span>
            <span className="text-sm text-stone-500">— event date passed, payment not recorded</span>
          </div>
                ...
                {pastDue.map(event => {
                  ...
                  return (
                    <TableRow key={event.id}>
                      ...
                      <TableCell className="text-red-600 font-semibold text-sm">{daysPast}d</TableCell>
      ...
      {upcoming.length > 0 && (
        <>
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-semibold text-stone-900">Upcoming — Awaiting Payment</h2>
            <span className="bg-amber-100 text-amber-700 text-xs px-2 py-0.5 rounded-full">{upcoming.length}</span>
          </div>
          <Card>
            <Table>
              <TableHeader>
      ...
      {stalled.length === 0 && (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium">No stalled payments</p>
