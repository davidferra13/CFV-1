    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-900">Installments</h1>
          <span className="bg-blue-100 text-blue-700 text-sm px-2 py-0.5 rounded-full">{allEntries.length}</span>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card className="p-4">
          <p className="text-2xl font-bold text-green-700">{formatCurrency(totalInstallments + totalFinal)}</p>
      ...
      {allEntries.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium">No installment payments recorded</p>
          <p className="text-stone-400 text-sm mt-1">Installment and final payment entries will appear here</p>
        </Card>
      ) : (
        <Card>
          <Table>
            ...
            <TableBody>
              {allEntries.map(entry => (
                <TableRow key={entry.id}>
                  ...
                  <TableCell>
                    <span className={`text-xs px-2 py-0.5 rounded-full capitalize ${entry.entry_type === 'final_payment' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'}`}>
