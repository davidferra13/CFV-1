...
export default async function OverdueInvoicesPage() {
  ...
  const criticalCount = overdue.filter(e => differenceInDays(now, new Date(e.event_date)) > 30).length
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-900">Overdue Invoices</h1>
          <span className={`text-sm px-2 py-0.5 rounded-full ${overdue.length > 0 ? 'bg-red-100 text-red-600' : 'bg-stone-100 text-stone-600'}`}>
            {overdue.length}
          </span>
        </div>
      ...
      {overdue.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium">No overdue invoices</p>
