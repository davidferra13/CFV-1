...
const STAGES = [
  { href: '/finance/invoices/draft', label: 'Draft', description: 'Events not yet sent to client', style: 'bg-stone-100 text-stone-600' },
  { href: '/finance/invoices/sent', label: 'Sent', description: 'Awaiting client acceptance', style: 'bg-amber-100 text-amber-700' },
  { href: '/finance/invoices/paid', label: 'Paid', description: 'Accepted and payment underway', style: 'bg-green-100 text-green-700' },
  { href: '/finance/invoices/overdue', label: 'Overdue', description: 'Past event date, unresolved', style: 'bg-red-100 text-red-600' },
  { href: '/finance/invoices/refunded', label: 'Refunded', description: 'Events with refund entries', style: 'bg-purple-100 text-purple-700' },
  { href: '/finance/invoices/cancelled', label: 'Cancelled', description: 'Cancelled events', style: 'bg-stone-200 text-stone-500' },
]
...
export default async function InvoicesPage() {
  ...
  const counts = {
    ...
    paid: events.filter(e => ['paid', 'confirmed', 'in_progress', 'completed'].includes(e.status)).length,
    overdue: events.filter(e => !['completed', 'cancelled'].includes(e.status) && new Date(e.event_date) < now).length,
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold text-stone-900 mt-1">Invoices</h1>
        <p className="text-stone-500 mt-1">Event invoices organized by status — invoices are per-event in ChefFlow</p>
      </div>
            ...
            <Link key={stage.href} href={stage.href}>
              <Card className="p-4 hover:shadow-md transition-shadow cursor-pointer">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${stage.style}`}>{count}</span>
                </div>
