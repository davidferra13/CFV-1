    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-900">Draft Invoices</h1>
          <span className="bg-stone-100 text-stone-600 text-sm px-2 py-0.5 rounded-full">{drafts.length}</span>
        </div>
      ...
      {drafts.length > 0 && (
        <div className="grid grid-cols-2 gap-4">
          <Card className="p-4">
            <p className="text-2xl font-bold text-stone-900">{drafts.length}</p>
            <p className="text-sm text-stone-500 mt-1">Draft invoices</p>
          </Card>
          <Card className="p-4">
            <p className="text-2xl font-bold text-stone-700">{formatCurrency(totalValue)}</p>
      ...
      {drafts.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium">No draft invoices</p>
          <p className="text-stone-400 text-sm mt-1">Events in draft or proposed state will appear here</p>
        </Card>
      ) : (
        <Card>
          <Table>
