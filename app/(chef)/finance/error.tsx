...
export default function FinanceError({
  ...
  useEffect(() => {
    reportClientBoundaryError(error, { boundary: 'finance', digest: error.digest })
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
      ...
      <p className="text-sm text-muted-foreground">Could not load finance data.</p>
      {error.digest && (
        <CopyableErrorId
          digest={error.digest}
      ...
      <ErrorReportButton error={error} boundary="finance" />
    </div>
  )
}
