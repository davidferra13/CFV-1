          ...
          <CardContent className="pt-4 pb-4">
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-sm text-stone-600">{c.relationship}</p>
      ...
      {!showForm ? (
        ...
        <Card>
          <CardContent className="pt-4">
            <h3 className="text-base font-semibold text-stone-900 mb-3">Add Emergency Contact</h3>
            <form onSubmit={handleAdd} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-stone-600 mb-1">Name *</label>
                  <Input value={form.name} onChange={(e) => update('name', e.target.value)} placeholder="Maria Santos" required />
                </div>
                <div>
                  <label className="block text-xs font-medium text-stone-600 mb-1">Relationship *</label>
                  <Input value={form.relationship} onChange={(e) => update('relationship', e.target.value)} placeholder="Sous chef, business partner…" required />
                </div>
                <div>
                ...
                <div className="col-span-2">
                  ...
                  <Input value={form.notes} onChange={(e) => update('notes', e.target.value)} placeholder="Available most weekends, prefers text…" />
                </div>
              </div>
