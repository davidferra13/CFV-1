...
// Chef can see if Stripe, Gmail, notifications, and DOP tasks are healthy.
...
export default async function SystemHealthPage() {
  ...
  const [stripeStatus, gmailStatus, dopDigest] = await Promise.all([
  ...
  // Stripe
  if (!stripeStatus) {
    checks.push({ label: 'Stripe Payments', status: 'unknown', detail: 'Could not check Stripe connection' })
  ...
  // Gmail / Google
  if (!gmailStatus) {
    checks.push({ label: 'Gmail Integration', status: 'unknown', detail: 'Could not check Gmail connection' })
  ...
  } else if (gmailStatus.errorCount > 0) {
    checks.push({
      label: 'Gmail Integration',
      status: 'warning',
      detail: `${gmailStatus.errorCount} sync error${gmailStatus.errorCount === 1 ? '' : 's'} — check your Google connection`,
      actionHref: '/settings',
      actionLabel: 'Review →',
    })
  } else {
    const lastSync = gmailStatus.lastSync
      ? `Last synced ${formatDistanceToNow(new Date(gmailStatus.lastSync), { addSuffix: true })}`
      : 'No sync recorded yet'
    checks.push({
      label: 'Gmail Integration',
      status: 'ok',
      detail: `${gmailStatus.email ?? 'Connected'} · ${lastSync}`,
    })
  }

  // DOP Tasks
  if (!dopDigest) {
    checks.push({ label: 'DOP Task Engine', status: 'unknown', detail: 'Could not load task digest' })
    ...
    checks.push({
      label: 'DOP Tasks',
      status: 'ok',
      detail: dopDigest.totalIncomplete === 0
        ? 'All tasks complete'
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-stone-500 mt-1">Connection and service status for your ChefFlow account</p>
      </div>

      {/* Overall status banner */}
      <Card className={`p-4 border ${STATUS_COLORS[overallStatus]}`}>
        <div className="flex items-center gap-3">
          <span className={`h-3 w-3 rounded-full shrink-0 ${STATUS_DOT[overallStatus]}`} aria-hidden="true" />
