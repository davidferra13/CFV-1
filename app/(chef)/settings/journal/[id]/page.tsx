...
export default async function JournalDetailPage({
  ...
  const requestedTab = typeof searchParams?.tab === 'string'
    ...
    : ''
  const initialTab = requestedTab === 'scrapbook'
    ? 'media'
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        <div className="flex items-center gap-2 text-sm">
          ...
          <Link href="/settings/journal" className="text-brand-600 hover:text-brand-700">
            Chef Journal
          </Link>
          <span className="text-stone-400">/</span>
          <span className="text-stone-500 truncate max-w-[220px]">{journey.title}</span>
        </div>
        <h1 className="text-3xl font-bold text-stone-900 mt-2">{journey.title}</h1>
        <p className="text-stone-600 mt-1">
          Journal every destination, insight, and idea so your culinary repertoire compounds over time.
        </p>
      </div>
