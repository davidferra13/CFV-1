...
export default function SettingsError({
  ...
  useEffect(() => {
    reportClientBoundaryError(error, { boundary: 'settings', digest: error.digest })
    console.error('[Settings Error]', error)
  }, [error])

  return (
    <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
      <h2 className="text-xl font-semibold">Could not load settings</h2>
      ...
      <ErrorReportButton error={error} boundary="settings" />
    </div>
  )
}
