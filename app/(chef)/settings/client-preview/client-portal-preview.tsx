...
function ClientPortalSidebar({ clientName }: { clientName: string }) {
  return (
    <aside className="w-56 flex-shrink-0 bg-white border-r border-stone-200 flex flex-col p-3">
      <div className="px-3 py-2 mb-2">
        <p className="text-xs font-semibold text-stone-400 uppercase tracking-wide truncate">{clientName}</p>
      </div>
...
function PortalContent({ portalData }: { portalData: PortalData }) {
  ...
  return (
    <div className="p-6 space-y-8 overflow-y-auto" style={{ maxHeight: '580px' }}>
        ...
        <Card className="border-purple-200 bg-purple-50">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${TIER_CLASS[loyaltyStatus.tier]}`}>
                {loyaltyStatus.tier.charAt(0).toUpperCase() + loyaltyStatus.tier.slice(1)} Member
              </span>
              <span className="font-bold text-purple-900">{loyaltyStatus.pointsBalance.toLocaleString()} pts</span>
      ...
      {/* My Events */}
      <section>
        <h2 className="text-2xl font-bold text-stone-900 mb-4">My Events</h2>
                  ...
                  <CardContent className="p-6">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                      ...
                      <div className="sm:ml-4">
                        <span className="inline-flex items-center px-3 py-1.5 rounded-lg text-sm font-medium bg-stone-100 text-stone-400 cursor-not-allowed select-none">
        ...
        {past.length > 0 && (
          <div className="mt-6">
            <h3 className="text-lg font-semibold text-stone-700 mb-3">Past Events</h3>
                ...
                <Card key={event.id}>
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium text-stone-800">{event.occasion || 'Event'}</p>
                        <p className="text-xs text-stone-500 mt-0.5">{format(new Date(event.event_date), 'PPP')}</p>
                      </div>
        ...
        {cancelled.length > 0 && (
          <div className="mt-6">
            <h3 className="text-lg font-semibold text-stone-700 mb-3">Cancelled</h3>
            <div className="space-y-2">
              {cancelled.map(event => (
                <Card key={event.id} className="opacity-60">
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <p className="font-medium text-stone-600">{event.occasion || 'Event'}</p>
      ...
      {pendingQuotes.length > 0 && (
        <section>
          <h2 className="text-2xl font-bold text-stone-900 mb-4">My Quotes</h2>
                  ...
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="text-right flex-shrink-0">
                        {total > 0 && (
                          <p className="text-lg font-bold text-stone-900">{formatCurrency(total)}</p>
                        )}
                    ...
                    <div className="mt-3 flex gap-2">
                      ...
                      <span className="inline-flex items-center px-3 py-1.5 rounded-lg text-sm font-medium bg-stone-50 text-stone-400 cursor-not-allowed select-none">
                        Decline
                      </span>
                    </div>
                  </CardContent>
...
function NoClientsState() {
  return (
    <div className="flex min-h-[460px]">
      ...
      <aside className="w-56 flex-shrink-0 bg-white border-r border-stone-200 flex flex-col p-3">
        <div className="px-3 py-2 mb-2">
          <p className="text-xs font-semibold text-stone-400 uppercase tracking-wide">Client name</p>
        </div>
        <div className="flex items-center gap-3 px-3 py-2 rounded-lg bg-stone-200 text-stone-400 text-sm font-semibold cursor-not-allowed select-none mb-2">
          {icons.plus}
          Book Now
        </div>
        <div className="space-y-0.5">
      ...
      <div className="flex-1 flex items-center justify-center p-8 text-center bg-stone-50">
        <div>
          <div className="text-stone-300 mb-4 flex justify-center">{icons.calendar}</div>
          <p className="font-medium text-stone-700">No clients yet</p>
          <p className="text-sm text-stone-500 mt-2 max-w-xs">
            Once you have clients with events or quotes, you can select one to preview their portal experience here.
          </p>
        </div>
      </div>
    </div>
...
export function ClientPortalPreview({ clients, selectedClientId, portalData, isPending, deviceFrame }: Props) {
  ...
  if (clients.length === 0) {
    return (
      <div className={isMobile ? 'flex justify-center' : undefined}>
        <div
          className={[
            'rounded-xl border-2 border-stone-200 overflow-hidden bg-stone-50',
  ...
  return (
    <div className={isMobile ? 'flex justify-center' : undefined}>
      <div
        className={[
          'rounded-xl border-2 border-stone-200 overflow-hidden bg-white',
          isMobile ? 'w-[390px]' : 'w-full',
        ].join(' ')}
      ...
      >
        {isPending ? (
          ...
          <div className="flex">
            ...
            <div className="flex-1 min-w-0 bg-stone-50">
              {isMobile && (
                ...
                <div className="border-b border-stone-200 bg-white px-4 py-2 flex items-center justify-between">
                  <span className="text-xs font-medium text-stone-700">{clientName}</span>
                  <div className="flex gap-4">
                    {[icons.calendar, icons.file, icons.chat, icons.gift, icons.user].map((icon, i) => (
                      <span key={i} className="text-stone-400 cursor-not-allowed">{icon}</span>
                    ))}
                  </div>
                </div>
              )}
          ...
          <div className="flex items-center justify-center h-64">
            <p className="text-sm text-stone-400">Select a client above to load their portal preview.</p>
          </div>
        )}
      </div>
    </div>
