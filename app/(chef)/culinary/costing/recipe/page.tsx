    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-900">Recipe Cost</h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Card className="p-4 text-center">
          <p className="text-2xl font-bold text-stone-900">{noCost.length}</p>
      ...
      {withCost.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium">No recipe costs calculated yet</p>
            ...
            <TableBody>
              {sorted.map(recipe => (
                <TableRow key={recipe.id}>
                  ...
                  <TableCell>
                    {costBar(recipe.total_cost_cents ?? 0, maxCostCents)}
                  </TableCell>
                  <TableCell>
          ...
          <div className="flex flex-wrap gap-2">
            {noCost.map(r => (
              <Link key={r.id} href={`/culinary/recipes/${r.id}`}>
                <span className="text-xs bg-stone-100 text-stone-600 hover:bg-stone-200 px-2 py-1 rounded-full cursor-pointer">
                  {r.name}
                </span>
              </Link>
            ))}
