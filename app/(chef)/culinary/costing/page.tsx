...
export default async function CostingPage() {
  await requireChef()
  const [recipes, menuCosts] = await Promise.all([
    getRecipes(),
  ...
  const avgRecipeCostCents = costedRecipes.length > 0
    ? Math.round(totalRecipeCostCents / costedRecipes.length)
    : 0

  const costingCoverage = recipes.length > 0
    ? Math.round((costedRecipes.length / recipes.length) * 100)
    : 0
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold text-stone-900 mt-1">Food Costing</h1>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <Card className="p-4">
          <p className="text-2xl font-bold text-stone-900">{formatCurrency(avgRecipeCostCents)}</p>
      ...
      {/* Recipe costs */}
      <div>
        <h2 className="text-lg font-semibold text-stone-900 mb-3">Recipe Costs</h2>
        {costedRecipes.length === 0 ? (
          <Card className="p-8 text-center">
            <p className="text-stone-600 font-medium mb-1">No recipe costs yet</p>
                ...
                {costedRecipes
                  ...
                  .map(recipe => (
                    <TableRow key={recipe.id}>
                      ...
                      <TableCell className="text-stone-900 font-medium text-sm">
                        {formatCurrency(recipe.total_cost_cents ?? 0)}
                      </TableCell>
                      <TableCell className="text-stone-600 text-sm">
                        {recipe.yield_quantity && recipe.total_cost_cents
                          ? formatCurrency(Math.round(recipe.total_cost_cents / recipe.yield_quantity))
                          : '—'}
                      </TableCell>
                      <TableCell>
      ...
      {menuCosts.length > 0 && (
        <div>
          <h2 className="text-lg font-semibold text-stone-900 mb-3">Menu Costs</h2>
          <Card>
            <Table>
              <TableHeader>
                <TableRow>
