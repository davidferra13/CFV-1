...
export async function generateMetadata({ params }: { params: { id: string } }) {
  const menu = await getMenuById(params.id).catch(() => null)
  return {
    title: menu ? `Nutrition - ${menu.name}` : 'Nutrition Analysis',
  }
}

export default async function NutritionDetailPage({ params }: { params: { id: string } }) {
  await requireChef()
  ...
  const [nutrition, showOnProposals] = await Promise.all([
    getMenuNutrition(params.id),
    getNutritionDisplaySetting(params.id),
  ])
