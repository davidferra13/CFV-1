    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-900">Menu Templates</h1>
          <span className="bg-amber-100 text-amber-700 text-sm px-2 py-0.5 rounded-full">{templates.length}</span>
        </div>
      ...
      {templates.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium">No menu templates yet</p>
            ...
            <Link key={menu.id} href={`/culinary/menus/${menu.id}`}>
              <Card className="p-5 hover:shadow-md transition-shadow cursor-pointer h-full">
                <div className="flex items-start justify-between mb-2">
                  <h2 className="font-semibold text-stone-900">{menu.name}</h2>
                  <span className="bg-amber-100 text-amber-700 text-xs px-2 py-0.5 rounded-full ml-2 shrink-0">Template</span>
                </div>
                {menu.description && (
                ...
                <p className="text-xs text-stone-400 mt-2">Created {format(new Date(menu.created_at), 'MMM d, yyyy')}</p>
              </Card>
            </Link>
          ))}
        </div>
