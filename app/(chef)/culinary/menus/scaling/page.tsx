  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold text-stone-900 mt-1">Menu Scaling</h1>
        <p className="text-stone-500 mt-1">Menus with target guest counts — reference for quantity adjustments</p>
      </div>
      ...
      {menusWithGuests.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium">No menus with guest counts set</p>
              ...
              {menusWithGuests
                ...
                .map(menu => (
                  <TableRow key={menu.id}>
                    ...
                    <TableCell className="text-stone-900 font-semibold">{menu.target_guest_count}</TableCell>
      ...
      {menus.filter(m => m.target_guest_count == null).length > 0 && (
        <p className="text-xs text-stone-400">
          {menus.filter(m => m.target_guest_count == null).length} menus without a target guest count —{' '}
          <Link href="/culinary/menus" className="text-brand-600 hover:underline">view all menus</Link>
        </p>
      )}
    </div>
  )
