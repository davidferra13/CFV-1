...
const FERMENT_KEYWORDS = ['ferment', 'pickle', 'kimchi', 'lacto', 'kvass', 'kefir', 'miso', 'koji', 'vinegar', 'brine', 'cured']
    ...
    <div className="space-y-6">
      ...
      <Card className="p-4 bg-stone-50 border-stone-200">
        <p className="text-sm text-stone-600">
          Ferments are matched by name — recipes and components containing &quot;ferment&quot;, &quot;pickle&quot;,
          &quot;kimchi&quot;, &quot;miso&quot;, &quot;lacto&quot;, &quot;cured&quot;, or similar terms appear here.
        </p>
      </Card>
      ...
      {fermentRecipes.length === 0 && fermentComponents.length === 0 ? (
        ...
        <>
          {fermentRecipes.length > 0 && (
            <>
              <h2 className="text-base font-semibold text-stone-700">Recipes ({fermentRecipes.length})</h2>
              <div className="grid grid-cols-2 gap-3">
                {fermentRecipes.map(recipe => (
                  <Card key={recipe.id} className="p-4">
                    <Link href={`/culinary/recipes/${recipe.id}`} className="font-medium text-brand-600 hover:underline">
                      {recipe.name}
                    </Link>
          ...
          {fermentComponents.length > 0 && (
            <>
              <h2 className="text-base font-semibold text-stone-700">Menu Components ({fermentComponents.length})</h2>
              <Card>
                <Table>
                  <TableHeader>
                    <TableRow>
