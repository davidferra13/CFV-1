...
export default async function VendorNotesPage() {
  ...
  const vendors = Array.from(byVendor.entries()).sort((a, b) =>
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-3xl font-bold text-stone-900">Vendor Notes</h1>
          <span className="bg-stone-100 text-stone-600 text-sm px-2 py-0.5 rounded-full">{vendors.length} vendors</span>
        </div>
      ...
      {withVendor.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-stone-600 font-medium">No vendor assignments yet</p>
          <p className="text-stone-400 text-sm mt-1">
            Set a preferred vendor on any ingredient to track sourcing here
          </p>
          <Link href="/culinary/ingredients" className="text-brand-600 hover:underline text-sm mt-3 inline-block">
            Browse ingredients →
          </Link>
        </Card>
      ) : (
          ...
          {vendors.map(([vendor, items]) => (
            <div key={vendor}>
              <div className="flex items-center gap-2 mb-2">
                <h2 className="text-base font-semibold text-stone-800">{vendor}</h2>
                <span className="text-xs text-stone-400">({items.length} ingredient{items.length !== 1 ? 's' : ''})</span>
              </div>
              <Card>
                    ...
                    .map(ing => (
                      <div key={ing.id} className="px-4 py-3 flex items-center gap-4">
                        <div className="flex-1">
                          ...
                          {ing.is_staple && (
                            <span className="ml-2 text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded">Staple</span>
                          )}
                        </div>
      ...
      {ingredients.filter(i => !i.preferred_vendor).length > 0 && (
        <p className="text-xs text-stone-400">
          {ingredients.filter(i => !i.preferred_vendor).length} ingredients without a vendor assignment —{' '}
          <Link href="/culinary/ingredients" className="text-brand-600 hover:underline">view all</Link>
        </p>
      )}
    </div>
  )
