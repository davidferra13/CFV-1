...
export default function GuestLeadsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    reportClientBoundaryError(error, { boundary: 'guest-leads', digest: error.digest })
    console.error('[Guest Leads Error]', error)
  }, [error])
  ...
  return (
    <div className="flex items-center justify-center min-h-[400px] px-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <CardTitle className="text-xl">Could not load guest leads</CardTitle>
          <p className="text-sm text-muted-foreground mt-1">
