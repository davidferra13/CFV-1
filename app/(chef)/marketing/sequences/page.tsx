    ...
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-stone-100">Automated Sequences</h1>
      ...
      {/* How it works */}
      <div className="rounded-lg border border-stone-700 bg-stone-800 p-4 text-sm text-stone-400 space-y-1">
        <p className="font-medium text-stone-100">How sequences work</p>
        <p>
          Each sequence has a trigger (birthday, dormant client, post-event) and one or more email
          steps. When a client matches the trigger, they&apos;re automatically enrolled and the
          steps fire in order. Clients who unsubscribe are skipped.
        </p>
      </div>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-medium text-stone-100">{seq.name}</span>
                      ...
                      <p className="text-xs text-stone-500">
                        {TRIGGER_DESCRIPTIONS[seq.trigger_type]}
                      </p>
                      <div className="flex gap-3 text-xs text-stone-400">
                        ...
                        {enrollmentCount > 0 && (
                          <span>
                            {enrollmentCount} client{enrollmentCount !== 1 ? 's' : ''} enrolled
                          </span>
                        )}
                      </div>
                      {/* Step summary */}
                          ...
                          {steps
                            ...
                            .map((s: any) => (
                              <div key={s.id} className="text-xs text-stone-500 flex gap-2">
                                ...
                                <span className="font-medium text-stone-300 truncate max-w-xs">
                                  {s.subject}
                                </span>
                              </div>
                            ))}
                    ...
                    <SequenceToggleButton sequenceId={seq.id} isActive={seq.is_active} />
                  </div>
                </CardContent>
              </Card>
            )
