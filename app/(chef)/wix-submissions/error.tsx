...
export default function WixSubmissionsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    reportClientBoundaryError(error, { boundary: 'wix-submissions', digest: error.digest })
    console.error('[Wix Submissions Error]', error)
  }, [error])
  ...
  return (
    <div className="flex items-center justify-center min-h-[400px] px-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <CardTitle className="text-xl">Could not load wix submissions</CardTitle>
          <p className="text-sm text-muted-foreground mt-1">
