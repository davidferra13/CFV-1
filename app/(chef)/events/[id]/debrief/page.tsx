    ...
    <div className="max-w-2xl mx-auto space-y-6 pb-16">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-stone-900">Post-Dinner Debrief</h1>
          <p className="text-stone-500 mt-1">
            {eventTitle} &middot; {eventDateFormatted}
            {blanks.client && (
              <> &middot; {blanks.client.name}</>
            )}
          </p>
        </div>

        {blanks.event.debriefCompletedAt && (
          <div className="flex-shrink-0 flex items-center gap-1.5 text-sm text-green-700 bg-green-50 border border-green-200 rounded-full px-3 py-1">
            <span>&#10003;</span>
            <span>Complete</span>
          </div>
        )}
      ...
      {!blanks.event.debriefCompletedAt && (
        <div className="text-sm text-stone-600 bg-stone-50 border border-stone-200 rounded-lg px-4 py-3">
