...
export function EventDetailGuestCountRequests({ changes }: { changes: GuestCountChange[] }) {
          ...
          history.map((change) => (
            <div key={change.id} className="rounded-lg border border-stone-800 bg-stone-950/40 p-3">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                ...
                <p className="text-xs text-stone-400">Price delta: {describePriceDelta(change)}</p>
              </div>
