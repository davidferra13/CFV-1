...
export default async function AdminReferralPartnersPage() {
  ...
  const { data: rawPartners, error } = await supabase
    ...
    .select('id, tenant_id, name, partner_type, status, email, website, is_showcase_visible, created_at')
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 px-4 py-3">
          <p className="text-xs text-slate-500 font-medium uppercase tracking-wide mb-1">Total Partners</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 px-4 py-3">
          <p className="text-xs text-slate-500 font-medium uppercase tracking-wide mb-1">In Showcase</p>
          <p className="text-2xl font-bold text-blue-700">{showcaseCount}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 px-4 py-3">
          <p className="text-xs text-slate-500 font-medium uppercase tracking-wide mb-1">Chef Tenants</p>
      ...
      {Object.keys(typeBreakdown).length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">By Type</h2>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 bg-slate-50">
          <h2 className="text-sm font-semibold text-slate-700">All Referral Partners ({partners.length})</h2>
        </div>
        {partners.length === 0 ? (
          <p className="px-4 py-10 text-center text-sm text-slate-400">No referral partners found across any chef.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500">Partner Name</th>
                  ...
                  <th className="text-center px-4 py-2.5 text-xs font-medium text-slate-500">Showcase</th>
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500">Added</th>
                </tr>
              </thead>
                ...
                {partners.map((partner) => (
                  <tr key={partner.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-2.5">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${STATUS_COLORS[partner.status] ?? 'bg-slate-100 text-slate-600'}`}>
                        {partner.status}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-2.5 text-center">
                      {partner.is_showcase_visible ? (
                        <span title="Visible in showcase"><Globe size={14} className="text-blue-500 mx-auto" /></span>
                      ) : (
