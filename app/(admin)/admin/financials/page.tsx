      ...
      {fin && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white rounded-xl border border-slate-200 px-4 py-4">
            <p className="text-xs text-slate-500 uppercase tracking-wide font-medium">GMV All-Time</p>
            <p className="text-2xl font-bold text-slate-900 mt-1">{formatCents(fin.totalGMV)}</p>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 px-4 py-4">
            <p className="text-xs text-slate-500 uppercase tracking-wide font-medium">GMV This Month</p>
            <p className="text-2xl font-bold text-green-600 mt-1">{formatCents(fin.gmvThisMonth)}</p>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 px-4 py-4">
            <p className="text-xs text-slate-500 uppercase tracking-wide font-medium">Expenses All-Time</p>
            <p className="text-2xl font-bold text-red-500 mt-1">{formatCents(fin.totalExpenses)}</p>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 px-4 py-4">
            <p className="text-xs text-slate-500 uppercase tracking-wide font-medium">Expenses This Month</p>
            <p className="text-2xl font-bold text-red-400 mt-1">{formatCents(fin.expensesThisMonth)}</p>
          </div>
        </div>
      )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-slate-500">Tenant</th>
                ...
                {entries.map((entry) => (
                  <tr key={entry.id} className="hover:bg-slate-50">
                    <td className="px-4 py-2.5">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${ENTRY_TYPE_COLORS[entry.entry_type] ?? 'bg-slate-100 text-slate-600'}`}>
                        {entry.entry_type}
                      </span>
                    </td>
