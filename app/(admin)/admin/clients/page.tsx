      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">
            {clients.length} client{clients.length !== 1 ? 's' : ''} across all tenants · {formatCents(totalLTV)} total LTV
          </p>
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-medium text-slate-500 uppercase tracking-wide">Chef / Tenant</th>
                  <th className="text-right px-4 py-3 text-xs font-medium text-slate-500 uppercase tracking-wide">Events</th>
                  <th className="text-right px-4 py-3 text-xs font-medium text-slate-500 uppercase tracking-wide">LTV</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {clients.map((client) => (
                  <tr key={client.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs text-slate-500">{client.chefBusinessName ?? '—'}</td>
                    <td className="px-4 py-3 text-right text-slate-700">{client.eventCount}</td>
                    <td className="px-4 py-3 text-right font-medium text-slate-900">{formatCents(client.ltvCents)}</td>
