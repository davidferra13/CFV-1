    ...
    <div className="space-y-6">
      ...
      {tableExists && summary.bySource.length > 0 && (
        <div className="bg-stone-900 border border-stone-700 rounded-lg p-4">
          <h2 className="text-sm font-semibold text-stone-300 mb-2">By Source</h2>
          <div className="flex flex-wrap gap-2">
            {summary.bySource.map(({ source, count }) => (
              <span
                key={source}
                className="inline-flex items-center gap-1 bg-slate-100 text-stone-300 text-xs px-2 py-1 rounded-full"
              >
