-- Extend respond_to_quote_atomic to copy pricing baseline/override metadata
-- from accepted quote into the linked event.
-- Also extends the related accepted-event price-lock guard to protect the new columns.
--
-- This replaces the version from 20260330000039 (which copied only
-- quoted_price_cents, deposit_amount_cents, pricing_model).
    ...
    IF v_quote.event_id IS NOT NULL THEN
      ...
      IF FOUND THEN
        IF v_event.status IN ('draft', 'proposed') THEN
          -- Copy final payable amounts AND pricing baseline/override metadata
          UPDATE events
          SET
            ...
            price_per_person_cents          = v_quote.price_per_person_cents,
            pricing_source_kind             = COALESCE(v_quote.pricing_source_kind, 'manual_only'),
            baseline_total_cents            = v_quote.baseline_total_cents,
            baseline_price_per_person_cents = v_quote.baseline_price_per_person_cents,
            override_kind                   = COALESCE(v_quote.override_kind, 'none'),
            override_reason                 = v_quote.override_reason,
            pricing_context                 = v_quote.pricing_context,
