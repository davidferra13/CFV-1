...
3. **Run the Matt Pocock audit on owned code changes** - if this session changed code, use `software-fundamentals` to verify the work attaches to the right module owner, deepen repeated or tangled behavior when it protects the task, and record whether module deepening happened, was unnecessary, or is intentionally deferred
4. **Stage current-task changes** - stage only files owned by this session unless the user explicitly requested all dirty files
5. **Commit** - clear, descriptive commit message
6. **Push the current branch** - `git push origin <current-branch>`
7. **Update CLAUDE.md** - if any new patterns, rules, or decisions were made this session, add them now
8. **Update `docs/session-log.md`** - append departure entry:
   ```
   ...
   ```
9. **Update `docs/build-state.md`** if you ran a build or type check
10. **Report** - tell the developer what was committed and pushed
