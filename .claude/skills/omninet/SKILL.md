...
7. When David declares a module freeze, full freeze, no stray code, or says everything must be moduled before feature work continues, route to `improve-codebase-architecture` with `swarm-governor`, `context-continuity`, and `evidence-integrity` as needed. During module freeze, do not add new product features. Allow only module ownership audits, reachability proof, duplicate pruning with owner proof, recover wiring into canonical routes, load-state hardening, and validation. Treat every orphan as keep, recover, duplicate, uncertain, or prune-candidate before editing it.
8. For ChefFlow release readiness, "what is stopping release," "what must work," or "what should we prove" questions, route to `pricing-reliability` first unless the user explicitly asks about go-to-market. For repeated pricing data audits, proof harness runs, "does ChefFlow work now," "what now" after pricing proof, or data engine reliability checks, route to `pricing-engine-auditor` with `pricing-reliability` as sidecar. Use `pricing-reliability` as the law and `pricing-engine-auditor` as the procedure.
9. Assume ChefFlow has a swarm of agents working concurrently. Treat unfamiliar dirty files, untracked files, deletions, stubs, logs, specs, and generated artifacts as other agents' active work unless you created them in this session.
10. If the task reveals reusable developer behavior, external operator guidance, repeated friction, a missing trigger, or a skill failure, run `skill-garden` in the same turn.
11. If a skill produced bad guidance, run `heal-skill` with `skill-garden` so the fix is both local and durable.
12. At closeout, decide the skill delta internally: `none`, `patch`, `new-skill`, or `heal`. If it is not `none`, make the skill change before final response and commit it.
...
| Module freeze, full freeze, "everything must be moduled," no stray code, or codebase-wide module cleanup                                                                                 | `improve-codebase-architecture` with `swarm-governor`, `context-continuity`, and `evidence-integrity` as needed |
