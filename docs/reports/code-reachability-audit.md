...
Total first-pass production-surface orphan signal after the first cleanup pass and focused AI duplicate cleanup: 488 files.
...
| `lib`        | `ai`            |     9 | High-risk because AI gateway and recipe restrictions apply. Some files may be policy-only or stale. Two duplicate scoring modules were deleted after focused proof. |
...
| `lib/ai/vendor-comparison.ts`                                                | Focused proof found no live imports of `@/lib/ai/vendor-comparison` and no callers of `compareVendors`, `VendorEntry`, or `VendorComparisonResult`. Current vendor comparison is owned by `lib/inventory/vendor-comparison-actions.ts` and rendered through `components/inventory/vendor-comparison-panel.tsx` on vendor and ingredient pages.                                             | Resolved on 2026-05-01 by deleting the duplicate AI vendor comparison module.                                                                         |
| `lib/ai/lead-scoring.ts`                                                     | Focused proof found no live imports of `@/lib/ai/lead-scoring` and no callers of its `scoreInquiry` export. Current inquiry lead scoring is GOLDMINE, owned by `lib/gmail/extract-inquiry-fields.ts`, `lib/inquiries/goldmine-lead-score.ts`, `components/inquiries/lead-score-badge.tsx`, and the inquiry detail page.                                                                    | Resolved on 2026-05-01 by deleting the duplicate AI lead scoring module.                                                                              |
| Public Remy cloud gateway                                                    | AI explorer found `app/api/remy/public/route.ts` and `app/api/remy/landing/route.ts` importing a public-cloud gateway with non-Ollama provider configuration.                                                                                                                                                                                                                              | Resolved on 2026-05-01 by deleting the public-cloud gateway, policy, and audit modules and adding a single-provider public Remy route guard test.     |
