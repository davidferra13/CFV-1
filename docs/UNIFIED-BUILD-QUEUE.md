...
| 27  | Wire brand-voice.ts Into Outbound Emails               | SPEC-READY | #19           | /intensify: brand-voice.ts complete (3 presets, greeting/signoff, forbidden-phrase lint) with zero importers. Wire into cadence-scheduler, auto-response, templates. HIGH yield.                                                                                        |
| 28  | Consolidate Dual Payment Reminder Paths                | SPEC-READY | #21           | /intensify: lib/invoices/reminder-actions.ts (raw pre body) vs lib/email/notifications.ts (proper React template). Two live callers use proper version. Consolidate to one path. HIGH yield.                                                                            |
...
| 8   | Rail Item Lifecycle And Scoring Engine         | SPEC-READY | None       | P0. Shared RailItem type, scoring/decay engine, density caps, item lifecycle states, time-awareness, cross-rail signal flow. Unblocks #2-#6. Spec: `docs/specs/rail-item-lifecycle-and-scoring-engine.md`        |
