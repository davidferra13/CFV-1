...
| 13  | Pre-Event Confidence Cadence                           | PARTIAL    | #12           | P1. BUILT 2026-05-16. CODE-VERIFIED 2026-05-16: cadence scheduler, confidence email template, SMS channel (4 touchpoints), smart-skip logic, chef settings UI. Migrations applied (cadence tables + SMS channel column). Settings page renders but cadence section needs re-test after migration fix. |
...
| 16  | Client Portal Guest Dietary Surfacing                  | PARTIAL    | None          | P1. BUILT 2026-05-16. CODE-VERIFIED 2026-05-16: guest invite card, dietary summary panel, aggregation query, chef-side nudge with reminder action. All components structurally complete with auth gates and tenant scoping.                                                                           |
| 17  | Post-Event Photo Gallery                               | PARTIAL    | #14           | P1. BUILT 2026-05-16. CODE-VERIFIED 2026-05-16: photo CRUD actions, upload prompt, client gallery (lightbox + keyboard nav), chef management page with visibility toggle, drag-drop reorder, tagging. Structurally complete.                                                                          |
| 18  | One-Click Rebook                                       | PARTIAL    | None          | P1. BUILT 2026-05-16. CODE-VERIFIED 2026-05-16: rebook button, prefill logic, seasonal rebook engine (anniversary detection), rebooking prediction scoring, chef-side rebooking bar UI. Structurally complete.                                                                                        |
...
| 33  | Wire CIL-to-Communication Action Bridge Dispatch       | SPEC-READY | #13           | /intensify rank 2: Bridge exists (signal-actions.ts) but terminates at emission. Wire to channel-router for real sends. HIGH yield.                                                                                                                                                                   |
| 34  | Add cadence_schedule as Rail Source                    | SPEC-READY | #13           | /intensify rank 3: Rail sources read scheduled_messages but not cadence_schedule. Single adapter addition. MED yield.                                                                                                                                                                                 |
| 35  | Passive Channel Preference Writer                      | SPEC-READY | #13           | /intensify rank 4: clients.preferred_contact_method exists. 3-line writer tallies reply channels from communication_events. MED yield.                                                                                                                                                                |
...
| 9   | Domain Wiring And Orphan Elimination Pass      | PARTIAL    | #1         | BUILT 2026-05-16: 40+ orphans wired into nav-config.tsx, 27 duplicate pages redirected (5 clusters). Remaining: ~60 status-as-page conversions to URL params. Audit: `docs/chef-portal-navigation-audit.md` Section 10. |
...
| #   | Item                                           | Status     | Depends On | Notes                                                                                                                                                                            |
| --- | ---------------------------------------------- | ---------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| 1   | Wire remy-weather.ts to shared weather utility | SPEC-READY | None       | CORRECTED: `lib/weather/open-meteo.ts` + `lib/weather/weather-actions.ts` already exist. Task is to make `remy-weather.ts` import from them instead of its own Open-Meteo calls. |
...
| 5   | Enrich Weather Alert Language                  | SPEC-READY | #2         | Chef-specific contextual messages. Tiered wind thresholds. Food safety heat language. Event-type-aware context.                                                                  |
...
| 15  | Verify Packing List Weather Prop               | SPEC-READY | None       | NEW (deep-pass). `packing-list-client.tsx` accepts `weather?: EventWeather` with `getWeatherSuggestions()`. Verify parent page passes weather data.                              |
| 16  | Sunset/Sunrise in Event Timeline               | SPEC-READY | None       | Show sunset as operational marker for outdoor events. Golden hour note. Include in event summary PDF.                                                                            |
| 17  | Client Countdown Page Weather                  | SPEC-READY | None       | NEW (deep-pass). `app/(client)/my-events/[id]/countdown/` has no environmental context. Reduces "should I get a tent?" questions.                                                |
| 18  | Autopilot Draft Weather Context                | SPEC-READY | None       | NEW (deep-pass). Append weather forecast to LLM prompt in `lib/autopilot/draft-generator.ts`. AI decides whether to mention it.                                                  |
| 19  | Weather-Conditional Pre-Event Checklist        | SPEC-READY | #4         | CORRECTED target: post-event-checklist is wrong. Needs new pre-event component. Auto-add items based on forecast. Cap at 5. Outdoor only.                                        |
| 20  | Historical Weather Snapshots                   | SPEC-READY | None       | New `event_weather_snapshots` table. Capture on event completion via `runCompletedEventPostProcessing()`. Additive migration.                                                    |
