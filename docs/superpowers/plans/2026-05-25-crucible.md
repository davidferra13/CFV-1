...
**Goal:** Build `/crucible`, an adversarial Opus subagent skill that auto-fires after builds, grades against 5 product-quality lenses, auto-fixes mechanical gaps (once), and persists verdicts for trend tracking.

**Architecture:** Single skill file (`.claude/skills/crucible/SKILL.md`) orchestrates an Opus subagent via the Agent tool. The skill handles complexity triage (skip/lightweight/full), constructs a context-loaded adversarial prompt, dispatches evaluation, parses the verdict, triggers one fix pass if needed, appends the result to `docs/crucible/verdicts.jsonl`, and gates "done" status. No new lib/ or app/ code.

**Tech Stack:** Claude Code skill (markdown), Agent tool (Opus model), git diff, file I/O for verdict persistence.

**Spec:** `docs/superpowers/specs/2026-05-25-crucible-design.md`
...
| ------ | ---------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Create | `.claude/skills/crucible/SKILL.md` | Main skill file: triage, context loading, agent dispatch, verdict parsing, fix pass, persistence, gate |
| Create | `docs/crucible/.gitkeep`           | Verdict persistence directory                                                                          |
| Modify | `CLAUDE.md` (lines ~215-221)       | Add `/crucible` as step 3 in Required closeout                                                         |

---

### Task 1: Create Verdict Persistence Directory

**Files:**

- Create: `docs/crucible/.gitkeep`

- [ ] **Step 1: Create the directory**

```bash
mkdir -p docs/crucible
```
...
```bash
git add docs/crucible/.gitkeep
git commit -m "chore: add crucible verdict persistence directory"
```

---

### Task 2: Create the Crucible Skill File

**Files:**

- Create: `.claude/skills/crucible/SKILL.md`

This is the core deliverable. The skill file contains all logic: triage, context loading, agent dispatch, verdict format, fix pass, persistence, gating, and manual invocation modes.

- [ ] **Step 1: Write the skill file**

Create `.claude/skills/crucible/SKILL.md` with this exact content:
...
2. **The 5 lenses with their evaluation criteria** (from the spec, verbatim)

3. **The grading scale and floor rules:**

> Grading: A(90-100), B(80-89), C(70-79), D(50-69), F(<50)
...
Read `docs/crucible/verdicts.jsonl`. If file doesn't exist or is empty, report "No verdict history yet."
...
Read back `.claude/skills/crucible/SKILL.md` and confirm:

- Frontmatter has `name`, `description`, `user-invocable: true`
- All 4 invocation modes documented
- Complexity gate has all 3 tiers (skip/lightweight/full)
- All 5 lenses referenced with sources
- Grading scale and floor rules present
- Fix pass is capped at one iteration
- Persistence format matches spec
- Trend mode documented
- Gate behavior documented

- [ ] **Step 3: Commit**

```bash
git add .claude/skills/crucible/SKILL.md
git commit -m "feat: add /crucible adversarial post-build evaluator skill"
```

---

### Task 3: Update CLAUDE.md Closeout Chain

**Files:**

- Modify: `CLAUDE.md` (lines ~215-221, the "Required closeout" section)

- [ ] **Step 1: Read the current closeout section**

Read `CLAUDE.md` and find the "Required closeout:" block. It currently reads:
...
- [ ] **Step 2: Add Crucible as the final gate**
...
Read the closeout section back and confirm `/crucible` appears as step 6 with the grade B gate.

- [ ] **Step 4: Commit**

```bash
git add CLAUDE.md
git commit -m "docs: add /crucible to required closeout chain (step 6, grade B gate)"
```
...
The skill should appear in the available skills list. Check that `.claude/skills/crucible/SKILL.md` exists and has valid frontmatter.

- [ ] **Step 2: Test SKIP mode**

Make a trivial docs-only change (e.g., add a blank line to any `.md` file). Invoke `/crucible`. Expected output:

```
CRUCIBLE: SKIP (config/docs/test/asset only)
```

Revert the trivial change after testing.

- [ ] **Step 3: Test trend mode with no history**

Invoke `/crucible --trend`. Expected output:
...
If the smoke test revealed issues in the skill file, fix them and commit:

```bash
git add .claude/skills/crucible/SKILL.md
git commit -m "fix(crucible): [description of smoke test fix]"
```
...
- [ ] `/crucible` skill file exists at `.claude/skills/crucible/SKILL.md`
- [ ] Skill has valid frontmatter with `name: crucible`, `user-invocable: true`
- [ ] Complexity gate triages skip/lightweight/full correctly
- [ ] Full mode dispatches Opus agent with adversarial persona and all 5 lenses
- [ ] Grading scale (A-F) and floor rules implemented
- [ ] Fix pass capped at one iteration, mechanical fixes only
- [ ] Verdicts persist to `docs/crucible/verdicts.jsonl`
- [ ] `--trend` mode reads and reports from verdict history
- [ ] CLAUDE.md closeout chain includes `/crucible` as step 6 with grade B gate
- [ ] Smoke test passes: SKIP mode works, trend mode handles empty history
