...
The new completeness gap map confirms that several core lanes still fail the closure test even though the routes or tables already exist. The highest-confidence remaining examples are release-contract drift and false-complete pipeline analytics. The task/todo contract lane is now closed for its current scope.
...
| Build and release contract truth                          | Targeted release-contract tests now align with the current route env contract, real public launch surfaces, and a web-beta surface that no longer depends on a missing overlay directory. Build identity endpoints now prefer configured build env, `NEXT_DIST_DIR` BUILD_ID, `.next/BUILD_ID`, and git SHA before runtime fallback.                                                                                                                                                                                                 | `docs/research/foundations/2026-04-03-system-completeness-gap-map.md`, `docs/specs/p1-build-and-release-contract-truth.md`, `app/api/health/route.ts`, `app/api/health/readiness/route.ts`, `app/api/build-version/route.ts`, `app/api/sentinel/health/route.ts`, `lib/health/public-health.ts`, `tests/unit/api.health-route.test.ts`, `tests/unit/launch-surface-guards.test.ts`, `tests/unit/web-beta-build-surface.test.ts`, `scripts/build-surface-manifest.mjs`         | `built-build-proof-blocked`      | Re-run `npm run build -- --no-lint` with a longer local build window before promoting this lane to verified.                                                                                                                            | `yes`, for build proof only                                           | Preserve the current canonical build command and current public health/readiness routes, but remove stale assertions that no longer match the runtime and repair the canonical build-artifact contract. | Do not recreate missing beta surfaces or preserve dead overlay architecture just to satisfy a stale test.                                                 | Targeted tests, web-beta typecheck, lint, surface audit, and heap-expanded `npx tsc --noEmit --skipLibCheck` passed on 2026-04-30. `npm run build -- --no-lint` timed out after 15 minutes and left `.next/BUILD_ID` missing. |
...
| Operational task/todo contract truth                      | A real structured `/tasks` system and a lightweight `chef_todos` reminder layer still coexist, but the contract drift between them has now been closed for the current scope: structured AI task reads and writes go through `tasks`, while lightweight reminder completion stays on `chef_todos`.                                                                                                                                                                                                                            | `docs/research/foundations/2026-04-03-system-completeness-gap-map.md`, `docs/specs/p1-task-and-todo-contract-truth.md`, `app/(chef)/tasks/page.tsx`, `lib/tasks/actions.ts`, `lib/todos/actions.ts`, `lib/todos/match.ts`, `lib/ai/agent-actions/operations-actions.ts`, `lib/ai/agent-actions/workflow-actions.ts`, `lib/ai/agent-actions/briefing-actions.ts`, `lib/ai/agent-actions/proactive-actions.ts`, `lib/ai/remy-context.ts`, `lib/ai/remy-intelligence-actions.ts` | `verified-foundation`           | Keep future task/reminder changes on the canonical owners and use the drift-guard tests before reopening this lane.                                                                                                                    | `no`, unless a new drift is proven                                 | Preserve the existing lightweight reminder widget where it is intentional, and keep structured task language and AI task flows on the real task system.                                                 | Do not collapse reminders and structured tasks into one fake universal table without preserving their different jobs.                                     | The repo still has both systems by design; the former gap was contract drift, not total absence.                                                                        |
