...
| Tasks vs todos             | A real structured task system exists on `/tasks`, while `chef_todos` remains the lightweight reminder layer used by dashboard and automations. The task/reminder contract is now explicit across AI readers and reminder completion flows. | Closed for the current lane. Future drift should extend the existing owners instead of reintroducing fake shared schemas or a second task registry.                                                                  | The repo already treats tasks as a core operator workflow and reminders as a separate lightweight concept. That distinction now stays explicit in code.      | `app/(chef)/tasks/page.tsx`, `lib/tasks/actions.ts`, `lib/todos/actions.ts`, `lib/todos/match.ts`, `lib/ai/agent-actions/operations-actions.ts`, `lib/ai/agent-actions/workflow-actions.ts`, `lib/ai/agent-actions/briefing-actions.ts`, `lib/ai/agent-actions/proactive-actions.ts`, `lib/ai/remy-context.ts`, `lib/ai/remy-intelligence-actions.ts`, `app/api/remy/stream/route-prompt-utils.ts`, `tests/unit/task-todo-contract.test.ts` | Task/reminder ownership is no longer ambiguous in Remy, briefings, or structured AI task creation.                                     | Keep future task/reminder work on the canonical owners and use the unit drift guard before reopening this lane.                                       | `verified`                  |
...
2. `docs/specs/p1-pipeline-analytics-truth-and-honesty.md`
