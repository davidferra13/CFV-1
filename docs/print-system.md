...
## Document Attribution ("Generated by")

Every document can show a subtle attribution line: who generated it, what type, and when.

- **PDF documents:** `pdf.generatedBy(name, docType)` — centered, gray, 7pt, 4mm above footer
- **Browser print pages:** `<PrintableDocument generatedBy="Chef Name">` — gray, 9pt/7pt
- **Controlled by chef:** Settings > Print & Documents > Show attribution toggle
- **Server action:** `getDocumentAttribution()` from `lib/print/actions.ts` — returns name or `null` if disabled
- **Full docs:** `docs/print-attribution-and-preferences.md`
...
6. Add attribution: call `getDocumentAttribution()` and pass as `generatedBy` prop
7. No inline `@media print` CSS needed — the shared styles handle everything
