...
3.  **Dynamic Pricing Engine:** The pricing engine must support rules like: *IF (Product = Rare Spice) AND (Source = Direct Farm) AND (Volume > 50kg) THEN (Price = Base Price * 0.90) ELSE (Price = Base Price)*. This level of conditional logic is missing.
...
---
*(Self-Correction/Internal Note: The system needs to move beyond "Product" as the primary inventory unit and treat "Batch/Lot" as the primary, traceable unit.)*
```
