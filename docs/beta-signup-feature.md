...
| `/beta/thank-you` | Public     | Confirmation page - explains what the beta is and expectations |
...
For invited users, the admin table also provides:

- **`/auth/signup?ref=beta&email=<invitee@email>`** (email prefilled + beta attribution)
...
## Signup Handoff Automation

- `/auth/signup` reads `email` and `ref` query params from beta invite links
- Invite links pre-fill the chef email field
- `ref=beta` is preserved through Google OAuth via `/auth/callback?next=...`
- On chef account creation, beta signup records auto-transition to `onboarded` by email match
- If `ref=beta` is present and no beta row exists yet, a new onboarded row is created
- OAuth role-selection chef signup now initializes Stripe customer + trial
- Optional beta trial duration override via `BETA_TRIAL_DAYS` env var (default remains 14)

## Emails

Three automated emails (all non-blocking - failures never block account creation):
...
3. **Account-ready email** when a beta chef creates an account - `lib/email/templates/beta-account-ready.tsx`

- Welcome/admin emails only send for new `/beta` submissions (not re-submissions).
- Account-ready email only sends when the created chef account is linked to beta.
- Admin notification includes total signup count.
...
| File                                         | Purpose                                                                     |
| -------------------------------------------- | --------------------------------------------------------------------------- |
| `lib/beta/actions.ts`                        | Server actions (submit, get, update, delete, export, count, onboarded sync) |
...
| `lib/email/templates/beta-account-ready.tsx` | Account-ready email template                                                |

## Admin Workflow

1. Share `cheflowhq.com/beta` with prospective testers
2. View signups at `/admin/beta` - search, filter by status, export CSV
3. Change status: **Pending** -> **Invited** (when you send access) -> **Onboarded** (when active)
4. Use **Declined** for signups that are not a fit
...
8. Status auto-updates to **Onboarded** when an invitee creates an account
