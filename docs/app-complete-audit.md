...
| Catalog tab  | Data table   | Browse Pi's full 9,000+ ingredient catalog. Stats bar (total/priced/categories), category filter chips, store dropdown (39 stores with search + tier badges), sort options (name/price/stores/updated), priced-only toggle, debounced search, active filter pills with remove buttons, clear-all-filters, paginated results (50/page), expandable rows showing all store prices with confidence badges |
