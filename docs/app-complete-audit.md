...
- **`/radar`** - Chef-facing external culinary intelligence surface. Reads source-backed `chef_radar_matches` scoped to the chef and joins global `culinary_radar_items` and `culinary_radar_sources` for attribution. Header shows critical/high/source counts. Action lanes split safety impact, local sourcing, opportunity inbox, and sustainability brief work streams. Cards show severity, category, source, published date, relevance, why ChefFlow surfaced the item, matched internal entities, recommended next steps, feedback buttons, source link, and real actions to make a task, save a local vendor lead, or add a market calendar note when the category allows it. Mark read, Dismiss, Useful, and Not useful are real server actions scoped by `chef_id`, revalidating Radar, dashboard, briefing, and daily ops paths. Radar controls let the chef pause or resume safety, opportunity, sustainability, craft, business, local, and client-signal categories without removing safety interrupt logic. Empty state explains that source monitors are ready but no relevant matches exist. Error state is explicit and does not claim all-clear when data fails.
- **Culinary Radar ingestion** - `/api/cron/culinary-radar` is a cron-authenticated source monitor registered in cron health. It syncs the approved source registry, ingests FDA, FSIS, CDC, USDA Farmers Market Directory, WCK, Worldchefs, and IFT-backed signals, updates source health, normalizes items, and creates chef-scoped matches from ingredients, upcoming events, vendors, and chef profile terms.
- **Dashboard, briefing, Daily Ops, and Remy connections** - Dashboard renders a Culinary Radar widget linked to `/radar`. Morning Briefing includes a Radar section and injects urgent safety Radar matches into the existing `briefing.alerts` stream, which means Daily Ops urgent banners inherit critical/high safety recalls and outbreaks. Remy has read-only Radar tasks for latest signals, safety, local markets, sustainability, source trust, opportunities, and item explanations, with source-backed task cards instead of raw JSON.
