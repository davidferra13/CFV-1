...
- State-change motion layer: `components/ui/state-motion.tsx` provides brief, data-tied feedback for workflow status changes, progress fills, and live work lists. Current integrations include save state badges, event status badges, event readiness badges, event readiness check rows, determinate progress bars, dashboard list cards, action surface cards, the decision queue, and the return-to-work strip. The global reduced-motion rule in `app/globals.css` applies to these animations.
...
- **Component:** `GettingStartedSection` (`components/dashboard/getting-started-section.tsx`)
- **Condition:** Tenant data presence has fewer than 3 populated feature areas, excluding admin or privileged users
- **Inputs:** `TenantDataPresence` from `getTenantDataPresence()` (`lib/progressive-disclosure/tenant-data-presence.ts`)
- **Actions:** Add first client -> `/clients/new`, create recipe -> `/recipes/new`, create event -> `/events/new`, start inquiry workflow -> `/inquiries/new`
- Auto-hides completed steps and removes the whole section once all starter steps are complete
...
- **Progressive disclosure:** Hidden when tenant data presence reports no circles. Direct `/circles` navigation remains available.
...
- **Component:** `WeeklyBriefingCard` (`components/pricing/weekly-briefing-card.tsx`)
- **Data:** `getWeeklyPriceBriefing()` (`lib/openclaw/weekly-briefing-actions.ts`)
- **Condition:** Always attempted; renders nothing if no data or fetch fails
- **Display:** Full card Mon-Wed, condensed card Thu-Sun
- Full card: headline, basket total (formatted $), week-over-week change %, biggest drops (green, up to 3), biggest spikes (red, up to 3), best store this week, coverage %
...
- **Data:** `getPriceIntelligenceSummary()` (`lib/openclaw/price-intelligence-actions.ts`)
- Single unified StatCard combining: price drops count, freshness breakdown (fresh/aging/stale), cost impact %, stock availability %
- Wrapped in Suspense + WidgetErrorBoundary
...
**Route:** `/events/[id]` (with `?tab=overview|money|prep|tickets|ops|wrap`; `money` is the stable URL key for the Finance tab)
...
- **Component:** `EventOperatingSpineCard` backed by `buildChefEventOperatingSpine()`
- **Mode:** `triage` for active events, `reviewing` for completed or cancelled events
- **Lanes:** Intake, Booking, Menu and dietary, Prep and stock, Finance, Communication
...
- **Collaborate:** RSVP/share/collaborator control surface. Includes a thread-derived coordination panel that extracts timing, headcount, location, dietary, and action signals from the communication thread, then shows scoped collaborator, guest, and viewer views based on share visibility. Each signal carries urgency plus a granular lifecycle policy: persist, auto-expire, or never-store. Critical, sensitive, or clustered high-urgency signals now surface a human intervention recommendation with the reason and next step before briefs are copied or shared. Each scoped view can be copied or shared as a compact instruction brief. Derived signals are runtime-only and the share expiry is surfaced when present.
...
- **Cost Forecast Badge:** (future events with menu only) Shows forecasted menu cost at event date using linear extrapolation from `forecastMenuCost()`. Displays: current cost, forecast cost, change %, confidence level (high/medium/low color-coded), caveat text. Data from `lib/openclaw/cost-forecast-actions.ts`.
...
22. **Household Members** - add/edit/remove residency household members, with relationship, age group, allergies, restrictions, dislikes, favorites, notes, summary counts, and attributed allergy warnings
23. **NDA Panel** - NDA status toggle, coverage, dates, photo permission
24. **Quick Notes** - add/pin/edit/delete notes with category (General/Dietary/Preference/Logistics/Relationship)
25. **Milestone Manager** - add/remove milestones (type, date, label, notes)
26. **Address Manager** - add/remove addresses (label, address, city/state/zip, access instructions, kitchen notes)
27. **Communication History** - sentiment badge + message thread + compose form
28. **AI Client Preference Panel** - AI-analyzed preferences (Ollama)
29. **Chef's Internal Assessment** - referral potential, red flags, acquisition cost, payment behavior, tipping pattern (edit/save)
30. **Unified Relationship Timeline** - chronological events/messages/notes/activity
31. **Event History Table** - status filter buttons + table with "View Details" per row
32. **Client Feedback** - reviews with ratings (conditional)
...
The main client detail also reuses a shared tenant-scoped authenticated-client work snapshot for action-required, balance, profile readiness, pending meal requests, signal-alert state, and the next active RSVP or share lane. The chef page fails closed with an unavailable card if that shared snapshot does not load.
...
| `/clients/[id]/relationship`                  | Dedicated relationship follow-through route. Header copy comes from the shared client action vocabulary. Summary card shows the winning action title, description, urgency/tier, primary signal badge, and reason/source explanation blocks. Direct outreach, client strategy brief, operational recommendation controls, reply intake, outcome capture, strategy diff, workbench timeline, evidence drawers, confirmation request, conflict review, event prep handoff, canonical relationship snapshot, recent relationship history, and key dates live below. |
| `/clients/strategy-readiness`                 | Portfolio client strategy readiness queue with real missing safety, contact, preference, stale profile, known preference, and readiness score data. Filter chips cover all clients, missing safety, stale profile, low readiness, no preferences, and no contact method. Rows link back to each client relationship strategy workspace.                                                                                                                                                                                                                          |
...
- **`/menus`** - **Canonical menu hub.** Grid of menu cards with search + sort. Cards show Template and **Showcase** badges. "Create Menu" → `/menus/new`. Also accessible via dashboard "Create Menu" button, Cmd+K "New Menu" quick action, and Action Bar "Menus" shortcut.
- **`/culinary/menus`** - Redirects to `/menus` (consolidated 2026-03-29). Sub-routes (`/culinary/menus/engineering`, `/culinary/menus/templates`, etc.) remain at their original paths.
...
- `/recipes/[id]/edit` ingredient rows include usable yield percentage inputs, reference-library yield suggestions, and helper text with prep and waste context.
...
- **`/culinary/costing`** - Recipe + menu cost tables. Includes "Freshness" column showing how recently prices were updated per recipe (color-coded: green for recent, amber for stale). **Header:** "Refresh All Prices" button (batch-resolves prices for all active ingredients, shows matched/unmatched/error counts). **Costing Confidence Badges:** green/amber/red coverage indicators on each recipe and menu row (>=90% green, >=60% amber, <60% red). **Ingredient Match Review panel:** collapsible list of unmatched ingredients with pg_trgm suggestions, confirm/dismiss per item, batch confirm for high-confidence matches (>80%). **Other sections:** Event Shopping Planner (aggregate ingredients from upcoming events, optimize across stores with days-ahead selector), Cost Impact panel (ingredients with recent price changes, direction arrows, percentage), Store Scorecard (personalized store rankings with coverage bars, win counts, avg prices).
...
- **`/culinary/costing/sales`** - "On Sale This Week" page. Shows current sale items from OpenClaw Pi grouped by category. Store filter chips (chef's preferred stores pre-selected). Each card: ingredient name, sale price, savings %, store, expiry date. Empty state when no sales data. Error state with retry when Pi offline. Nav entry under Culinary > Costing.
...
- **`/culinary/call-sheet`** - Voice Hub for supplier and business calling. Gated by `supplier_calling`. Tabs cover Call, Call Log, Inbox, My Vendors, and Settings. The Voice Ops Control Tower sits above the tabs and summarizes active sessions, completed and failed calls, answer rate, recording coverage, urgent reviews, opt-outs, professional risk, and the next post-call actions. Each persisted post-call action exposes a "Why this exists" evidence drawer with source webhook, ledger signals, duplicate guard, haptic/interruption reason when recorded, and related call IDs. Actions can be completed, marked for review, snoozed, or skipped with visible feedback. Voice boundaries remain vendor and business focused; automated outbound client calls stay blocked.
- **`/culinary/call-sheet`** - Voice Hub for supplier and business calling. Gated by `supplier_calling`. Tabs cover Call, Call Log, Inbox, My Vendors, and Settings. The Voice Ops Control Tower sits above the tabs and summarizes active sessions, completed and failed calls, answer rate, recording coverage, urgent reviews, opt-outs, professional risk, and the next post-call actions. Each persisted post-call action exposes a "Why this exists" evidence drawer with source webhook, ledger signals, duplicate guard, haptic/interruption reason when recorded, and related call IDs. Actions can be completed, marked for review, snoozed, or skipped with visible feedback. Voice boundaries remain vendor and business focused; automated outbound client calls stay blocked.
...
| `FreshnessDot`          | `components/pricing/freshness-dot.tsx`           | Color-coded dot (green <7d, amber 7-30d, red >30d, gray unknown) with tooltip. Used in admin and chef catalogs.                         |
| `StockBadge`            | `components/pricing/stock-badge.tsx`             | Stock status badge (green all-in-stock, amber limited, red out-of-stock, gray unknown). Used in chef catalog.                           |
| `PriceComparisonBars`   | `components/pricing/price-comparison-bars.tsx`   | Horizontal bar chart comparing store prices for one ingredient. Cheapest bar green. Used in admin catalog expanded rows.                |
| `CategoryCoverageChart` | `components/pricing/category-coverage-chart.tsx` | Horizontal bars showing coverage % per category. Clickable to filter. Used in admin catalog.                                            |
| `BulkPriceChecker`      | `components/pricing/bulk-price-checker.tsx`      | Paste ingredient list, get best prices. Shows totals and single-store comparison. Used in admin catalog.                                |
...
| Route                    | Content                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/hub/g/[groupToken]`    | Group view with 7 tabs: Chat (real-time feed), Meals (weekly meal board with B/L/D grid, inline feedback thumbs, schedule change flags, clone/template tools for chef), Events (linked events), Photos (gallery + upload), Schedule (availability polls), Notes (sticky notes), Members (list + dietary info + household dietary summary). Residency mode option in Settings (meals-first default tab, silent notifications). **Dinner Status section** shows client-visible lifecycle checkpoints (confirmed/pending status, extracted values). Invite actions reuse one attributed join link, and button copy changes for chef, client, or member context. |
| `/hub/join/[groupToken]` | Join page for first-time visitors - group name, member count, role-aware invite badge, inviter-aware headline copy when the link carries attribution, and a quick-join form with allergy capture.                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `/my-hub/g/[groupToken]` | Full group experience wrapped in client layout - reuses HubGroupView (7 tabs: Chat, Meals, Events, Photos, Schedule, Notes, Members). Auto-joins group, sets profile cookie. Meals tab includes weekly board, persistent household dietary banner, per-meal allergy conflict flags, feedback, schedule changes, attendance, comments, and chef shopping-list generation. |
...
| `/my-events/[id]`               | Event detail - booking progress card using the same event spine contract as the chef workspace, journey stepper, event details card, payment summary, **menu section** (smart status: CTA to choose menu, "chef is preparing" live indicator with pulsing dot, approval status badges, revision progress), photo gallery, share/RSVP, feedback form, plus a Dinner Circle card with host-native invite copy and the same text-friendly join link mechanics used everywhere else. |
...
| Supporter proof    | Public credibility band backed by `getPublicSupporterProofReport()`. Shows only featured approved testimonials and showcase-visible active partners. If no approved public proof exists, renders honest early-stage copy instead of names, logos, quotes, or fake counts. If proof data is unavailable, shows an unavailable proof state rather than zeroes.                                                                                                                                                                     |
...
| Submit                           | Posts to `/api/book`. Shows spinner during submission, then redirects to `/book/status/[bookingToken]` when the tracked booking record is created                   |
| Success state                    | Fallback only if no booking token is returned. Normal flow uses the public booking status page                                                                      |
| Trust footer                     | 4 checkmarks: Free to submit, No obligation, Chefs contact you directly, Zero commission                                                                            |

**Status page:** `/book/status/[bookingToken]` is public and token-scoped. It shows the submitted event details, booking readiness across details, dietary, proposal, and payment, matched-chef count, status progression, 48-hour no-response actions, final no-match state, proposal links when available, and Dinner Circle planning links after a chef response creates one.

**API:** `POST /api/book` - rate limited (5/10min per IP), Turnstile CAPTCHA server-side verification, email validation, honeypot. Matches chefs via `matchChefsForBooking()` (Haversine distance, service type, guest count). Creates an `open_bookings` parent record for tracking, preserves guest-count range data, parses budget labels/free-text to cents, records referral partner plus UTM attribution, creates inquiry records under matched chefs (up to 10), stamps `unknown_fields.submission_source = open_booking`, links inquiries through `open_booking_inquiries`, and sends chef notification emails plus a client confirmation with the status link. Event creation remains deferred until commercial commitment.

**Cron:** `/api/cron/booking-escalation` detects chef responses, creates the Dinner Circle for the responding consumer-chef pair, stores proposal/status links, sends the 48-hour no-response email, and sends the 7-day no-match email before closing stale requests.
