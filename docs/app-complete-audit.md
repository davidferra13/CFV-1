...
- **Collaborate:** RSVP/share/collaborator control surface. Includes a thread-derived coordination panel that extracts timing, headcount, location, dietary, and action signals from the communication thread, then shows scoped collaborator, guest, and viewer views based on share visibility. Each signal carries urgency plus a granular lifecycle policy: persist, auto-expire, or never-store. Each scoped view can be copied or shared as a compact instruction brief. Derived signals are runtime-only and the share expiry is surfaced when present.
