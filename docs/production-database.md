...
Current dev/beta database: Supabase project `luefkpakzvxcsqroxyhz`

---

## 1. Production Project Setup

> Status: Not yet created. Production will use a separate Supabase project from dev/beta.

### Steps (to be completed before launch)

- [ ] Create new Supabase project for production (Pro plan recommended for PITR)
...
- [ ] Link CLI to production: `supabase link --project-ref <prod-project-id>`
...
### 2a. Automated Backups (Supabase)

Supabase provides automatic daily backups on all paid plans:
...
| Trigger                       | Action                                  | Who                                 |
| ----------------------------- | --------------------------------------- | ----------------------------------- |
| Before any `supabase db push` | Manual SQL dump                         | Developer (or agent, with approval) |
| Daily (automated)             | Supabase snapshot                       | Supabase (automatic)                |
...
1. Never run `supabase db push` without explicit developer approval
2. Always dump before pushing: `supabase db dump --linked > backup-$(date +%Y%m%d).sql`
