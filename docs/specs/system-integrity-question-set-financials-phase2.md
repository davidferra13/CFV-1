...
| M4  | RESOLVED (partial)        | Contract now uses `event.deposit_amount_cents` dynamically (M1 fix). Full installment schedule rendering is a feature enhancement, not a data integrity gap.                                                                                |
...
| S1  | RESOLVED                  | Stripe webhook (`app/api/webhooks/stripe/route.ts:740-754`) creates `payment_received` notification in real-time via canonical pipeline. Manual ledger entries (cash) don't trigger notifications, acceptable since chef is already aware.  |
