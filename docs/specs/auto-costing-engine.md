...
1. Create `propagatePriceChange(ingredientId: string)` in `lib/pricing/cost-refresh.ts`:
   - Find all `recipe_ingredients` rows referencing this ingredient
   - Call `computeRecipeIngredientCost()` for each (uses conversion engine + yield)
   - Collect unique recipe IDs
   - Call `refreshRecipeTotalCost()` for each affected recipe (deduped)
...
2. Wire `propagatePriceChange()` into existing price update flows:
   - `logIngredientPrice()` in `lib/ingredients/pricing.ts` - after `updateIngredientPriceFields()`
   - `cascadeIngredientPrice()` in `lib/inventory/price-cascade-actions.ts` - after price update
   - OpenClaw sync completion in `lib/openclaw/sync.ts`

3. Fix the delete gap in `lib/recipes/actions.ts`:
   - After removing an ingredient from a recipe, call `refreshRecipeTotalCost()` for that recipe

4. Wire `cost_needs_refresh`:
...
4. Create `getMenuCostingCoverage()` and `getRecipeCostingCoverage()`:
   ...
   - Count ingredients with non-null `cost_per_unit_cents` vs total
...
| `lib/recipes/actions.ts`                 | (a) **Export** `computeRecipeIngredientCost()` (currently private, line ~1804) and `refreshRecipeTotalCost()` (currently private, line ~1869). These must be exported so `cost-refresh-actions.ts` can import them. (b) After removing an ingredient from a recipe, add a call to `refreshRecipeTotalCost()` (currently missing). (c) When adding an ingredient, check for alias; if none, queue for matching suggestions. |
