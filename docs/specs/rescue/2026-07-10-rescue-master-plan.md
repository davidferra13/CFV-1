...
| WS5 | [2026-07-10-rescue-ws5-integrity.md](2026-07-10-rescue-ws5-integrity.md)                   | 14    | Data-integrity audit (added 2026-07-10, second discovery pass): 2 P0 cross-tenant leaks, 8 P1 (tenant/money/silent-failure), 8 P2. Not from the blueprint; audits integrity (leaks, fake success, stale cache, money double-writes) where the blueprint audited access. WS5 T1-T5 ride Wave 0/1 with the WS1 spine. |
...
- WS5 root cause (`lib/db/server.ts:7`): `createServerClient` ignores its `admin` option and there is no RLS on the Drizzle path, so every tenant wall is a hand-written `.eq('tenant_id', ...)`. The durable fix (queued for architecture review) is a lint or scoped-query helper that fails CI on a by-id read missing its tenant filter; WS5 T1-T2/T7 patch the known holes in the meantime.
- WS5 M2: `anonymize_financial_records` (SECURITY DEFINER, disables the ledger triggers) is never REVOKEd from PUBLIC; this is the one path that can mutate the immutable ledger. Additive REVOKE migration, next prefix above 20260617000001.
