...
| **PIE**                     | Pricing Intelligence Engine. ChefFlow's full pricing infrastructure. Not just a database. Five layers: (1) Data Platform (OpenClaw collects/normalizes prices from 20+ chains, government APIs, wholesale, receipts), (2) Resolution Engine (11-tier waterfall: chef override -> receipt -> API quote -> wholesale -> scrape -> flyer -> instacart -> regional avg -> market aggregate -> government -> historical -> category baseline), (3) Intelligence Layer (seasonal factors, confidence scoring, anomaly detection, sourceability), (4) Costing Engine (recipe/menu/event/quote pricing with yield, waste, labor, overhead), (5) Decision Support (shopping optimizer, price comparison, vendor scorecards, spike warnings, margin projections). Internal name: PIE. User-facing: prices just appear with confidence badges and source attribution. Never name the engine in UI. External pitch: "proprietary food pricing infrastructure, 400 market regions, real-time collection from every major grocery chain." |
| **Auto-Costing Engine**     | Legacy name for PIE's resolution engine (layer 2). Use PIE instead. 11-tier price resolution chain that automatically prices ingredients. Uses OpenClaw data, vendor prices, chef overrides. Deterministic (formula, not AI).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| **Price Catalog**         | The chef-facing UI for browsing PIE data. Shows ingredient prices from OpenClaw across local stores. Deterministic, not AI.           |
...
| Pricing database | PIE (Pricing Intelligence Engine)        | It's an engine, not a database. Five layers.            |
| Price engine     | PIE                                      | Use the acronym. Everyone knows it.                     |
