...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'staff_availability' AND column_name = 'chef_id')
     AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'staff_availability' AND column_name = 'tenant_id') THEN
...
DO $$ BEGIN CREATE POLICY ssr_update ON shift_swap_requests FOR UPDATE USING (tenant_id = get_current_tenant_id()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY ssr_delete ON shift_swap_requests FOR DELETE USING (tenant_id = get_current_tenant_id()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
