-- ============================================================
-- Migration: Quote/Event pricing integrity hardening
-- Purpose:
--   1) Harden respond_to_quote_atomic against duplicate accepted quotes.
--   2) Stamp events.converting_quote_id on quote acceptance.
--   3) Prevent accepting a quote that conflicts with locked event pricing.
--   4) Backfill converting_quote_id from existing accepted quotes.
-- ============================================================
      ...
      IF FOUND THEN
        IF v_event.status IN ('draft', 'proposed') THEN
          ...
          SET
            ...
            converting_quote_id = COALESCE(converting_quote_id, v_quote.id),
