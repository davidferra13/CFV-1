...
CREATE POLICY "modifier_groups_insert" ON product_modifier_groups
  FOR INSERT WITH CHECK (chef_id IN (SELECT id FROM chefs WHERE user_id = auth.uid()));

CREATE POLICY "modifier_groups_update" ON product_modifier_groups
  FOR UPDATE USING (chef_id IN (SELECT id FROM chefs WHERE user_id = auth.uid()));

CREATE POLICY "modifier_groups_delete" ON product_modifier_groups
  FOR DELETE USING (chef_id IN (SELECT id FROM chefs WHERE user_id = auth.uid()));
