...
1. **No external store availability signal** - from Noah Kessler - HIGH
   In-stock/low-stock indicators for stores Noah will shop.
   > Search hints: external.store, store.availability, availability.signal, external, store, availability, signal
2. **No destination-first store intelligence for travel** - from Noah Kessler - HIGH
   Before-arrival view of nearby stores, comparative prices, and likely procurement plan by destination.
   > Search hints: destination-first.store, store.intelligence, intelligence.travel, destination-first, store, intelligence, travel
...
A single consolidated build addressing all sourcing supply gaps should cover:
