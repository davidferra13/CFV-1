...
// Routed to OLLAMA (contains client names, feedback text, survey responses — all PII).
...
const SYSTEM_PROMPT = `You are a marketing consultant for a private chef. Evaluate client feedback excerpts and identify the strongest testimonials for portfolio/marketing use.
...
RULES:
- Anonymize client names to initials (e.g. "Sarah Miller" becomes "S.M.").
- Extract the BEST QUOTE from each piece of feedback (2-4 sentences max).
- Only include quotes that are genuinely compelling (score >= 60).
...
Return ONLY valid JSON:
{
  "topTestimonials": [{
    ...
    "quote": "the specific extracted excerpt",
...
export async function selectTestimonialHighlights(): Promise<TestimonialSelectionResult> {
  ...
  const userPrompt = `Evaluate these client feedback excerpts and select the strongest testimonials:

${feedbackItems.map((f, i) => `${i + 1}. [${f.eventType} | ${f.clientName}]:\n"${f.content.slice(0, 500)}"`).join('\n\n')}

Return only testimonials with score >= 60, sorted by score descending.`

  const parsed = await parseWithOllama(SYSTEM_PROMPT, userPrompt, TestimonialResponseSchema, {
    modelTier: 'standard',
    timeoutMs: 90_000,
    maxTokens: 2048,
  })
