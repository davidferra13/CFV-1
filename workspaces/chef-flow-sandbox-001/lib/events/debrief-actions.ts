 ...
 * Routed to OLLAMA (contains client name, event details — PII stays local).
 */
export async function generateDebriefDraft(
  ...
  const systemPrompt = `You are helping a private chef write private reflection notes after a dinner service. Write 2-3 sentences in the first person, casual and honest, as if jotting a quick note to yourself right after the event. Focus on the experience, what went well, what to remember next time. No em dashes. No greeting, no sign-off, no quotes. Just the notes. Return JSON: { "draft": "your reflection text" }`
...
Write the reflection notes.`

  try {
    const { z } = await import('zod')
    const { parseWithOllama } = await import('@/lib/ai/parse-ollama')
    const DebriefDraftSchema = z.object({ draft: z.string() })

    const parsed = await parseWithOllama(systemPrompt, userPrompt, DebriefDraftSchema, {
      modelTier: 'standard',
      timeoutMs: 60_000,
      maxTokens: 512,
    })
    ...
    // Check if Ollama is offline and give a clear message
    const { OllamaOfflineError } = await import('@/lib/ai/ollama-errors')
    if (err instanceof OllamaOfflineError) {
      return { error: 'Ollama is offline. Start Ollama to generate draft notes.' }
    }
